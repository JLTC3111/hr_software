...
void mainImage(out vec4 fc,in vec2 frag){
    ...
    float L=(a+b*topA)*uBeamIntensity;
    float w=vWisps(vec2(uvc.x,yPix),topA)*uBeamIntensity;
    float fog=0.0;
#if FOG_ON
...
export const LaserFlow = ({
  ...
  const showSurfacePanelRef = useRef(showSurfacePanel);
  const mouseOverPanelRef = useRef(false);
  showSurfacePanelRef.current = showSurfacePanel;
  ...
  useEffect(() => {
    ...
    renderer.setClearColor(hexToNumber(clearColor), 1);
    ...
    const isOverSurfacePanel = (clientX, clientY) => {
      if (!showSurfacePanelRef.current) return false;
      const panel = surfacePanelRef.current;
    ...
    const onPointerMove = ev => {
      const overPanel = isOverSurfacePanel(ev.clientX, ev.clientY);
      mouseOverPanelRef.current = overPanel;
      if (overPanel) {
        mouseTarget.set(0, 0);
        mouseSmooth.set(0, 0);
        return;
    ...
    const onPointerLeave = () => {
      mouseOverPanelRef.current = false;
      mouseTarget.set(0, 0);
      mouseSmooth.set(0, 0);
    };
  ...
  useEffect(() => {
    ...
    uniforms.uBeamIntensity.value = beamIntensity;
  ...
  useEffect(() => {
    rendererRef.current?.setClearColor(hexToNumber(clearColor), 1);
    renderSceneRef.current?.();
  }, [clearColor]);
