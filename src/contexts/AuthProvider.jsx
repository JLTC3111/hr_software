import _React, { useState, useEffect, useCallback, useRef, useMemo } from 'react';
  ...
  const fetchUserProfile = async (userId) => {
    ...
    try {
      ...
      const [
        ...
        // avatar_url is a base64 portrait (~200KB). It is not needed to decide
        // whether this person may enter, and downloading it here held the
        // login spinner for the whole transfer.
        supabase.from('hr_users').select('id, email, is_active, employment_status, full_name, first_name, last_name, phone, role, department, position, employee_id, manager_id, hire_date, salary').eq('id', hrUserId).single(),
      ...
      if (data && data.is_active) {
        ...
        supabase
          ...
          .eq('id', hrUserId)
          .maybeSingle()
          .then(({ data: avatarRow, error: avatarError }) => {
            if (avatarError || !avatarRow?.avatar_url) return;
            setUser((prev) => (
              prev && prev.id === data.id
                ? { ...prev, avatar_url: avatarRow.avatar_url }
                : prev
            ));
          });
