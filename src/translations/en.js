export default {
  // Navigation
  nav: {
    menu: 'Menu',
    workload: 'Work Management',
    attendanceHub: 'Attendance',
    timeClock: 'Time Clock',
    punchClock: 'Punch Clock',
    dashboard: 'Dashboard',
    employees: 'Employees',
    recruitment: "Recruitment",
    leaveManagement: 'Leave Management',
    timeTracking: 'Time Tracking',
    performance: 'Personal Goals',
    reports: 'Reports',
    notifications: 'Notifications',
    settings: 'Settings',
    translations: 'Translation Studio',
    policyControls: 'Policy Controls',
    controlPanel: 'Control Panel',
    personalGoals: 'Personal Goals',
    taskListing: 'Task Listing',
    taskReview: 'Performance Review',
    svgMorphing: 'SVG Morphing',
    fullDemo: 'Full Demo',
    showcase: 'Quick Showcase'
  },
  // Leave Management
  leave: {
    subtitle: 'Plan, request and approve time off on a shared calendar.',
    // Sheet chrome
    month: 'Month',
    byType: 'By type',
    ofNRequests: 'of {n} requests',
    pendingNote: 'Approve or reject them on their row in the ledger.',
    nothingPending: 'Nothing is waiting on a decision.',
    requestLeave: 'Request Leave',
    requestsTitle: 'Leave Requests',
    noRequests: 'No leave requests yet.',
    everyone: 'Everyone',
    mine: 'My Leave',
    today: 'Today',
    clearSelection: 'Clear selection',
    cmdClickHint: '{mod} or Shift-click another day to add it, or a selected day to skip it.',
    selectStartHint: 'Tip: click a day to start a leave request, then click the end day.',
    selectEndHint: 'Now click the end date (or the same day for a single day).',
    selectionRange: 'Selected',
    selectDatesFirst: 'Click a start and end date on the calendar first.',
    selectedDates: 'Selected Dates',
    adjustOnCalendar: 'Close this dialog to re-pick dates on the calendar.',
    invalidRange: 'End date cannot be before start date.',
    selectEmployee: 'Please select an employee.',
    type: 'Type',
    allTypes: 'All Types',
    employee: 'Employee',
    allEmployees: 'All Employees',
    days: 'days',
    halfDay: 'Half day',
    halfDayTag: '[Half day]',
    startTime: 'Start Time',
    endTime: 'End Time',
    reason: 'Reason',
    reasonPlaceholder: 'Add an optional note for your manager...',
    submitRequest: 'Submit Request',
    requestSubmitted: 'Leave request submitted successfully!',
    requestsSubmitted: '{n} leave requests submitted successfully!',
    requestApproved: 'Leave request approved.',
    requestRejected: 'Leave request rejected.',
    rejectReasonPrompt: 'Reason for rejecting this request (optional):',
    approve: 'Approve',
    reject: 'Reject',
    other: 'Other',
    totalRequests: 'Total Requests',
    pending: 'Pending',
    approvedDaysYear: 'Approved Days (Year)',
    onLeaveToday: 'On Leave Today',
    stepPickStart: 'Click your first day',
    stepPickEnd: 'Click your last day',
    rangeReady: 'Dates selected — ready to request leave',
    requestLeaveNow: 'Request Leave Now',
    rangeStart: 'Start',
    rangeEnd: 'End',
    clickToSelect: 'Click to select',
    filtersTitle: 'Filters & Legend',
    statusFilter: 'Status',
    allStatuses: 'All',
    legendTitle: 'Legend',
    addForEmployee: 'Add for Employee',
    addLeaveForEmployee: 'Add Leave for Employee',
    autoApprove: 'Approve immediately',
    autoApproveHint: 'Skip the pending queue and mark this request as approved.',
    submitAndApprove: 'Submit & Approve',
    submitAndApproved: 'Leave added and approved.',
    requestsSubmitAndApproved: '{n} leave requests added and approved.',
    manualDates: 'Enter start and end dates.',
    onBehalfNote: 'You are submitting leave on behalf of this employee.',
    rejectTitle: 'Reject Leave Request',
    rejectConfirm: 'Reject leave for {{name}}?',
    rejectReasonLabel: 'Reason (optional)',
    rejectReasonPlaceholder: 'Explain why this request is being rejected...',
    confirmReject: 'Reject Request',
    status: 'Status',
    approved: 'Approved',
    rejected: 'Rejected',
    editRequest: 'Edit Leave Request',
    requestUpdated: 'Leave request updated.',
    approvedRemovesGenerated: 'Approving removes the generated 09:00–17:00 standard hours on the covered weekdays; hand-entered attendance stays.',
    revertApproval: 'Revert',
    revertApprovalHint: 'Send the request back to pending',
    revertTitle: 'Revert Approval',
    revertConfirm: 'Send the approved leave for {{name}} ({{range}}) back to pending?',
    confirmRevert: 'Revert to Pending',
    approvalReverted: 'Approval reverted; the request is pending again.',
    approvalRevertedRestored: 'Approval reverted. {n} standard-hour entries restored.',
    restoreStandardHours: 'Restore standard hours',
    restoreStandardHoursHint: 'Re-create the generated 09:00–17:00 entries on the released weekdays. Hand-entered attendance and overtime are never touched.',
    restoreStandardHoursEditHint: 'Re-create the generated 09:00–17:00 entries on weekdays this change releases from approved leave. Hand-entered attendance and overtime are never touched.'
  },
  // Header
  header: {
    title: 'HR Management',
    welcome: 'Welcome Back,',
    welcomePhrase1: 'Welcome back, {name}',
    welcomePhrase2: 'Great to see you, {name}',
    welcomePhrase3: 'Hello, {name}!',
    welcomePhrase4: 'Ready to manage HR, {name}?',
    welcomePhrase5: "Let's make today productive, {name}",
    welcomePhrase6: 'Good to have you here, {name}',
    user: 'HR Team',
    logout: 'Logout',
    notifications: 'Notifications',
    // The steel band's session cell: a tracked label over the user's name.
    welcomeBack: 'Welcome back',
    // Under the ICUE mark, at 9px/.2em.
    consoleTag: 'Workforce Console',
    // The eyebrow above every page title: "SHEET 01".
    sheet: 'Sheet'
  },
  // Session lifecycle: the idle countdown shown before an automatic sign-out.
  session: {
    idleWarningTitle: 'Still there?',
    idleWarningBody: 'You\'ve been inactive for a while. For security, we\'ll sign you out automatically.',
    idleWarningCountdown: 'Signing out in',
    idleWarningSeconds: 'seconds',
    staySignedIn: 'Stay signed in',
    signOutNow: 'Sign out now',
  },
  // Login
  login: {
    idleLogoutMessage: 'You were signed out after a period of inactivity. Please sign in again.',
    title: 'HR Manager',
    subtitle: 'Sign in to access your dashboard',
    email: 'Email Address',
    password: 'Password',
    emailPlaceholder: 'you@example.com',
    passwordPlaceholder: '••••••••',
    emailRequired: 'Email is required',
    emailInvalid: 'Email is invalid',
    passwordRequired: 'Password is required',
    passwordTooShort: 'Password must be at least 6 characters',
    invalidCredentials: 'Invalid email or password',
    rememberMe: 'Remember me',
    forgotPassword: 'Forgot password?',
    signIn: 'Sign In',
    signingIn: 'Signing in...',
    orContinueWith: 'Or continue with',
    continueWithGithub: 'Continue with GitHub',
    redirecting: 'Redirecting to GitHub...',
    githubError: 'Failed to login with GitHub',
    tryDemoLoading: 'Loading demo...',
    tryDemo: "Try Demo Mode",
    requestDemo: 'Request Demo',
    requestDemoNote: 'Email support@icue.vn to request a demo',
    forgotPasswordModal: {
      title: 'Reset Password',
      description: 'Enter your email address and we\'ll send you a link to reset your password.',
      emailLabel: 'Email Address',
      emailPlaceholder: 'you@example.com',
      emailRequired: 'Email is required',
      emailInvalid: 'Please enter a valid email',
      sendReset: 'Send Reset Link',
      cancel: 'Cancel',
      success: 'Password reset email sent. Please check your inbox.',
      error: 'Failed to send reset email. Please try again.',
      noAccount: 'No account found with this email address',
      rateLimitError: 'Too many requests. Please try again later',
      emailRateLimitError: 'Email sending limit reached. Please try again later.',
      networkError: 'Network error. Please check your connection'
    }
  },
  // Reset Password Page
  resetPassword: {
    title: 'Reset Password',
    subtitle: 'Enter your new password below',
    newPassword: 'New Password',
    confirmPassword: 'Confirm Password',
    newPasswordPlaceholder: 'Enter new password',
    confirmPasswordPlaceholder: 'Confirm new password',
    allFieldsRequired: 'All fields are required',
    passwordTooShort: 'Password must be at least 6 characters',
    passwordsDontMatch: 'Passwords do not match',
    error: 'Failed to reset password. Please try again.',
    success: 'Password reset successfully!',
    resetting: 'Resetting...',
    verifying: 'Verifying reset link...',
    resetButton: 'Reset Password',
    backToLogin: 'Back to Login',
    invalidLink: 'Invalid or expired reset link. Please request a new password reset.',
    signIn: 'Sign In',
    signingIn: 'Signing in...',
    orContinueWith: 'Or continue with',
    continueWithGithub: 'Continue with GitHub',
    redirecting: "Redirecting to login...",
    githubError: 'Failed to login with GitHub',
    noAccount: "Don't have an account?",
    signUp: 'Sign up',
    footer: '© 2024 HR Manager. All rights reserved.',
    tryDemoLoading: 'Loading demo...'
  },
  // Global Errors
  errors: {
    invalidFileType: 'Please select an image file',
    fileTooLarge: 'File size must be less than 5MB',
    fileReadError: 'Error reading file. Please try again.'
  },
  // Sidebar
  sidebar: {
    main: 'MAIN',
    analytics: 'ANALYTICS',
    settings: 'SETTINGS',
    collapse: 'Collapse sidebar',
    expand: 'Expand sidebar'
  },
  // Dashboard
  dashboard: {
    workforce: 'Workforce',
    headcount: 'Headcount',
    attrition: 'Attrition',
    hours: 'Hours',
    leave: 'Leave',
    performance: 'Performance',
    approvals: 'Approvals',
    total: 'Total',
    regularAndOvertimeByEmployee: 'Regular & Overtime by Employee',
    employeeLabel: 'Employee',
    currentMonth: 'Current Month',
    liveData: 'Live Data from Supabase',
    noData: 'No data available',
    title: 'Dashboard Overview',
    pendingApprovals: 'Pending Approvals',
    overview: 'Organization Overview',
    lastUpdate: 'Last update',
    totalEmployees: 'Total Employees',
    totalRegularHours: 'Total Regular Hours',
    activeEmployees: 'Active Employees',
    pendingApplications: 'Pending Applications',
    avgPerformance: 'Average Performance',
    totalOvertime: 'Total Overtime',
    totalLeave: 'Total Leave Days',
    employeePerformance: 'Employee Performance',
    departmentDist: 'Department Distribution',
    regularHoursByEmployee: 'Regular Hours by Employee',
    overtimeHoursByEmployee: 'Overtime Hours by Employee',
    performanceRating: 'Performance Rating',
    regularHoursLegend: 'Regular Hours',
    totalOvertimeLegend: 'Total Overtime',
    workLeaveComp: 'Work vs Leave Days',
    topPerformers: 'Top Performers',
    overtime: 'Over Time',
    totalWorkDays: 'Total Work Days',
    acrossEmployees: 'Across all employees this month',
    activeApplications: 'Active Applications',
    pendingReview: 'Pending review',
    pendingRequests: 'Pending Requests',
    leaveRequests: 'Leave requests to review',
    recentActivity: 'Recent Activity',
    viewAll: 'View All',
    // Organization Overview console
    company: 'Company',
    outOf: 'of',
    reviews: 'reviews',
    high: 'High',
    low: 'Low',
    filterByDept: 'Filter by department',
    needsDecision: 'Needs a Decision',
    items: 'items',
    queueClear: 'Queue is clear',
    oldestWaited: 'Oldest has waited',
    overdue: 'Overdue',
    nothingPending: 'Nothing is waiting on you right now.',
    more: 'more',
    noApplications: 'No active applications',
    pendingApprovalsTotal: 'pending across all approvals',
    pipeline: {
      screening: 'Screening',
      shortlisted: 'Shortlisted',
      interview: 'Interview',
      offer: 'Offer'
    },
    activities: {
      newEmployee: "New employee added: ",
      interviewScheduled: "Interview scheduled with: ",
      performanceReview: "Performance review completed for: "
    },
    timeAgo: {
      twoHours: '2 hours ago',
      fourHours: '4 hours ago',
      oneDay: '1 day ago'
    }
  },
  // Employees
  employees: {
    title: 'Employee Management',
    directory: 'Directory',
    cards: 'Cards',
    viewCards: 'Card view',
    viewDirectory: 'Directory view',
    addNew: 'Add New Employee',
    quickStats: 'Quick Stats',
    searchPlaceholder: 'Search employees...',
    addEmployee: 'Add Employee',
    employee: 'Employee',
    name: 'Name',
    position: 'Position',
    department: 'Department',
    email: 'Email',
    phone: 'Phone',
    startDate: 'Start Date',
    salary: 'Salary',
    status: 'Status',
    location: 'Location',
    performance: 'Performance',
    actions: 'Actions',
    view: 'View',
    edit: 'Edit',
    delete: 'Delete',
    active: 'Active',
    inactive: 'Inactive',
    onLeave: 'On Leave',
    employeeDetails: 'Employee Details',
    contactInformation: 'Contact Information',
    employmentDetails: 'Employment Details',
    started: 'Started',
    editEmployee: 'Edit Employee',
    uploadPhoto: 'Upload photo',
    viewDeleted: 'Restore',
    deletedEmployees: 'Deleted Employees',
    restore: 'Restore',
    confirmRestore: 'Are you sure you want to restore this employee?',
    restoreSuccess: 'Employee restored successfully!',
    restoreError: 'Failed to restore employee',
    updateSuccess: 'Employee details have been successfully updated!',
    orgNetwork: 'Org network',
    orgNetworkDesc: 'Departments connected to your HR hub.',
    topPerformers: 'Top performers',
    avgRating: 'Avg rating',
    departments: 'departments',
    livePulse: 'Live pulse'
  },
  // Employee Directory — ledger, spec plates and the employee record
  employeeDirectory: {
    title: 'Employee directory',
    // Ticker
    headcount: 'Headcount',
    working: 'Working',
    onLeaveToday: 'On leave today',
    joinedThisMonth: 'Joined this month',
    missingRecords: 'Missing records',
    tickerCode: 'ID',
    tickerUnit: 'Unit',
    openWork: 'Open work',
    // Title row and filter strip
    peopleInUnits: '{n} people in {u} units',
    joinedThisMonthLong: '{n} joined this month',
    missingRecordsLong: '{n} records missing documents',
    platesSubtitle: 'Card mode · for recognising people, not for comparing them',
    exportList: 'Export list',
    exported: 'Exported {n} people.',
    scope: 'Scope',
    scopeAll: 'All',
    scopeStarting: 'Starting',
    scopeLeft: 'Left',
    searchPlaceholder: 'Search by name, email or unit',
    groupedByUnit: 'Grouped by unit',
    sortedByRating: 'Sorted by rating',
    viewMode: 'View',
    // Ledger
    contact: 'Contact',
    tenure: 'Tenure',
    thisWeek: 'This week',
    mean: 'Mean',
    nPeople: '{n} people',
    nStarting: '{n} starting',
    nAwaiting: '{n} awaiting approval',
    nMissingDocs: '{n} missing documents',
    ledgerFooter: 'Showing {rows} / {total} people · {open} / {units} units',
    expandUnits: 'Show all units',
    collapseUnits: 'Collapse all units',
    nothingMatches: 'No one matches this filter.',
    // Person facts
    noMailbox: 'No mailbox yet',
    noPhone: 'No phone on file',
    noUnit: 'Unassigned',
    joinedOn: 'joined',
    startsOn: 'starts',
    since: 'Since',
    noStartDate: 'No start date',
    yearsShort: 'yr',
    monthsShort: 'mo',
    inNDays: 'in {n} days',
    notStartedYet: 'Not started',
    noTimesheet: 'No timesheet',
    onLeaveNote: 'On approved leave',
    noOvertime: 'No overtime',
    overtimeShort: 'OT',
    pendingHours: '{h} awaiting approval',
    documentsComplete: 'Complete',
    documentsMissing: 'Missing',
    recordsOnFile: 'Records on file',
    recordsMissing: 'No documents',
    // State words
    stateWorking: 'Working',
    stateOnLeave: 'On leave',
    statePending: 'Awaiting approval',
    stateStarting: 'Starts',
    stateLeft: 'Left',
    // Plates
    code: 'ID',
    joined: 'Joined',
    records: 'Records',
    sendMail: 'Mail',
    viewHours: 'Hours',
    openRecord: 'Open record',
    outOfFive: 'out of 5.0',
    aboveMean: 'out of 5.0 · {d} above the company mean {m}',
    belowMean: 'out of 5.0 · {d} below the company mean {m}',
    noRatingTitle: 'Not rated this cycle',
    noRatingBody: 'No score has been filed, so the bar is left empty rather than guessed.',
    noReviewYetTitle: 'No review cycle yet',
    noReviewYetBody: 'Starts {date} — the first cycle opens after onboarding.',
    plateFooter: 'Showing {shown} / {total} of {unit} · sorted by rating',
    compareInLedger: 'Compare in the ledger',
    photoUpdated: 'Photo updated.',
    // Decision column
    needsAction: 'Needs action',
    nItems: '{n} items',
    attachedToPeople: 'Attached to people, not to cards',
    pointersOnly: 'Pointers only · the decisions live in the ledger',
    queueEmpty: 'Nothing is waiting on this roster.',
    pendingTag: 'pending',
    starterBody: 'Starts in {n} days in {unit}. Records are {docs} — a missing document blocks the first-week onboarding.',
    pendingBody: '{h} filed this week are still unapproved, {ot} of it overtime against a {cap}h monthly cap. Approving writes it into the open payroll month.',
    startsInDays: 'Starts in {n} days · {unit}',
    overtimeOf: 'Overtime · {name}',
    pendingAgainstCap: '{h} pending · {cap}h monthly cap',
    timesheetExceptions: '{n} timesheets awaiting approval',
    acrossUnits: 'Across {n} units',
    reviewsUnsigned: '{n} reviews not signed off',
    reviewCycleNote: 'Current cycle',
    leavePending: '{n} leave requests pending',
    leavePendingNote: 'Waiting on an approver',
    docsMissing: '{n} records missing documents',
    justJoined: 'Just joined',
    justJoinedNote: 'This month · {n} records still missing documents',
    noRecentJoiners: 'No one has joined in the last two months.',
    topRated: 'Top rated',
    ofNRated: '{n} rated',
    podiumNote: 'Against a company mean of {m}',
    noRatings: 'Nobody on this roster has been rated yet.',
    unitSpread: 'Unit spread',
    unitsAndPeople: '{u} units · {n} people',
    emptyUnits: 'Nobody in {list}',
    everyUnitStaffed: 'Every department has someone in it',
    report: 'Report',
    unitNetwork: 'Unit network',
    unitNetworkNote: 'The {n} largest units and their headcount.',
    // Record — index
    allPeople: 'All',
    searchInUnit: 'Search {n} people',
    shownOf: '{shown} / {total} people',
    viewCards: 'Cards',
    unitSize: '{n} in the same unit',
    fullProfile: 'Full profile',
    editRecord: 'Edit record',
    deleteRecord: 'Delete record',
    assignWork: 'Assign work',
    // Record — panel 01
    panelEmployment: 'Employment',
    workplace: 'Workplace',
    noWorkplace: 'Not recorded',
    overtimeCap: 'Overtime cap',
    perMonth: 'month',
    used: 'used',
    leaveBalance: 'Leave balance',
    leaveBalanceValue: '{left} / {total} days · {used} taken this year',
    documentsCompleteLong: 'Complete · document on file',
    documentsMissingLong: 'Missing · nothing on file',
    // Record — panel 02
    panelTimesheet: 'This week',
    openTimesheet: 'Open timesheet',
    againstContract: 'against {h}h contracted · {ot} overtime',
    noEntriesThisWeek: 'No time entries filed between {from} and {to}.',
    ownPending: '{n} of these entries are still waiting for approval.',
    noPendingHere: 'Nothing on this record is waiting for approval · {n} entries elsewhere are.',
    // Record — panel 03
    panelReview: 'Review',
    cycle: 'Cycle',
    noCycle: 'No cycle filed',
    selfAssessment: 'Self-assessment',
    managerReview: 'Manager review',
    unassignedReviewer: 'No reviewer named',
    signOff: 'Sign-off',
    filed: 'Filed',
    notFiled: 'Not filed',
    goalsMet: 'Goals met this cycle',
    openReview: 'Open review',
    openGoals: 'Goals',
    reviewStatus: {
      draft: 'Draft',
      submitted: 'Awaiting sign-off',
      approved: 'Signed off',
      acknowledged: 'Acknowledged',
      rejected: 'Returned'
    },
    // Record — panel 04
    panelWork: 'Open work',
    openTaskBook: 'Task book',
    noOpenWork: 'Nothing is assigned to this record.',
    due: 'Due',
    taskOpen: 'In progress',
    taskToday: 'Today',
    taskLate: 'Late',
    taskDone: 'Done',
    openWorkNote: '{n} open — every one of them belongs to a record, so nothing here is free-floating.',
    priority: {
      low: 'Low',
      medium: 'Medium',
      high: 'High',
      urgent: 'Urgent'
    }
  },
  // Add Employee Modal
  addEmployee: {
    title: 'Add New Employee',
    submit: 'Add Employee',
    // Draft sheet (2d)
    directory: 'Directory',
    step: 'Step',
    stepPersonal: 'Personal',
    stepEmployment: 'Employment',
    stepReview: 'Review',
    fullName: 'Full name',
    recordTitle: 'New employee record',
    draft: 'Draft',
    addNew: 'Add new',
    autosaved: 'Autosaved',
    notSavedYet: 'Not saved yet',
    draftSaved: 'Draft saved',
    reservedId: 'Reserved record id',
    stepOf: 'Step {n} of {total}',
    inProgress: 'In progress',
    done: 'Done',
    next: 'Next',
    locked: 'Locked',
    requiredCounter: '{filled} of {total} required fields',
    allPresent: 'all present',
    nationalId: 'National ID',
    nationalIdRequired: 'National ID is required',
    addPhoto: 'Add photo',
    photoSpec: '4:5 · min 600px',
    removePhoto: 'Remove photo',
    fromOffer: 'From offer',
    prefillFromReq: 'Prefill from requisition →',
    recordPreview: 'Record preview',
    unnamed: 'Unnamed record',
    starts: 'Starts',
    contract: 'Contract',
    fullTime: 'Full time',
    partTime: 'Part time',
    perMonth: 'mo',
    reportsTo: 'Reports to',
    headcountAfter: 'Headcount after',
    beforeSubmit: 'Before submit',
    checkPersonal: 'Personal information complete',
    checkOffer: 'Signed offer on file',
    checkOfferMissing: 'Signed offer on file',
    checkEmployment: 'Department and start date',
    checkPayroll: 'Payroll and tax details',
    checkEquipment: 'Equipment request',
    onSubmit: 'On submit',
    onSubmitBody: 'Creates the employee record, adds them to the directory and the time-tracking roster, and notifies you when it lands.',
    onSubmitManager: 'They will report to {name}.',
    continueToEmployment: 'Continue to employment →',
    continueToReview: 'Continue to review →',
    createRecord: 'Create employee record',
    saveAndClose: 'Save and close',
    createFailed: 'Failed to create employee. Please try again.',
    unexpectedError: 'An unexpected error occurred.',
    nameRequired: 'Name is required',
    positionRequired: 'Position is required',
    departmentRequired: 'Department is required',
    emailRequired: 'Email is required',
    emailInvalid: 'Email is invalid',
    phoneRequired: 'Phone is required',
    dobRequired: 'Date of birth is required',
    addressRequired: 'Address is required',
    startDateRequired: 'Start date is required',
    salaryRequired: 'Salary is required',
    performanceInvalid: 'Performance must be between 0 and 5',
    namePlaceholder: 'Enter employee name',
    positionPlaceholder: 'e.g., senior_developer',
    departmentPlaceholder: 'e.g., engineering, finance',
    emailPlaceholder: 'employee@example.com',
    phonePlaceholder: '+84 909 999 999',
    addressPlaceholder: 'City, Country',
    salaryPlaceholder: 'Enter employee salary',
    dob: 'Date of Birth',
    address: 'Address',
    personalInfo: 'Personal Information',
    employmentInfo: 'Employment Information',
    review: 'Review & Submit',
    personalInformation: 'Personal Information',
    employmentInformation: 'Employment Information',
    reviewAndSubmit: 'Review & Submit',
    selectPosition: 'Select Position',
    selectDepartment: 'Select Department',
    photoTooLarge: 'Photo must be less than 5MB',
    invalidPhotoType: 'Only image files are allowed',
    successMessage: 'Employee added successfully!',
    submitError: 'Failed to add employee',
    dobPlaceholder: 'Select date of birth'
  },
  // Employee Detail Modal
  employeeDetailModal: {
    download: 'Download',
    uploadPdf: 'Upload PDF',
    uploading: 'Uploading...',
    delete: 'Delete',
    basicInfo: 'Basic Information',
    contact: 'Contact',
    documents: 'Documents',
    pdfTab: 'PDF',
    requestDocsTab: 'Requests',
    requestDocsRefresh: 'Refresh',
    requestDocsOpen: 'Open',
    requestDocsPreview: 'Preview',
    requestDocsDelete: 'Delete',
    requestDocsOpenInNewTab: 'Open in new tab',
    requestDocsClosePreview: 'Close preview',
    requestDocsLoading: 'Loading...',
    uploadRequestDoc: 'Upload Document',
    docCategory: 'Category',
    categoryLeave: 'Leave request',
    categoryOther: 'Other',
    noRequestDocs: 'No documents uploaded',
    requestDocsHint: 'Upload leave-request evidence, certificates, or other supporting files.',
    requestDocsRestricted: 'Only Admin/HR/Manager can upload request documents.',
    requestDocUploaded: 'Document uploaded successfully!',
    quickStats: 'Quick Stats',
    status: 'Status',
    workDuration: 'Work Duration',
    workDurationNA: 'N/A',
    yearUnit: 'year',
    yearsUnit: 'years',
    monthUnit: 'month',
    monthsUnit: 'months',
    durationUnitSeparator: ' ',
    durationPartSeparator: ' ',
    performance: 'Performance',
    fullName: 'Full Name',
    department: 'Department',
    position: 'Position',
    dateOfBirth: 'Date of Birth',
    startDate: 'Start Date',
    salary: 'Salary',
    email: 'Email',
    phone: 'Phone',
    address: 'Address',
    uploadDocument: 'Upload Document',
    downloading: 'Downloading...',
    viewDocument: 'View Document',
    deleteDocument: 'Delete Document',
    confirmDelete: 'Are you sure you want to delete this document?',
    documentDeleted: 'Document deleted successfully',
    documentDeleteError: 'Failed to delete document',
    noDocument: 'No document uploaded',
    uploadInstructions: 'Click or drag a PDF file here to upload',
    maxFileSize: 'Maximum file size: 10MB'
  },
  // Search
  search: {
    placeholder: 'Search employees...'
  },
  // Employee Department (for display in cards)
  employeeDepartment: {
    all: 'All',
    legal_compliance: 'Legal Compliance',
    technology: 'Technology',
    internal_affairs: 'Internal Affairs',
    human_resources: 'Human Resources',
    office_unit: 'Office Unit',
    operations: 'Operations',
    board_of_directors: 'Board of Directors',
    finance: 'Finance',
    engineering: 'Engineering',
    sales: 'Sales',
    marketing: 'Marketing',
    design: 'Design',
    part_time_employee: 'Part-Time Employee',
    // Also support capitalized versions for compatibility
    Management: 'Management',
    Engineering: 'Engineering',
    Design: 'Design',
    Marketing: 'Marketing',
    Sales: 'Sales',
    Finance: 'Finance',
    'Human Resources': 'Human Resources',
    Technology: 'Technology',
    Operations: 'Operations',
    Support: 'Support',
    Legal: 'Legal',
    IT: 'IT',
    it: 'IT',
    product: 'Product',
    customer_support: 'Customer Support'
  },
  // Months
  months: {
    january: 'January',
    february: 'February',
    march: 'March',
    april: 'April',
    may: 'May',
    june: 'June',
    july: 'July',
    august: 'August',
    september: 'September',
    october: 'October',
    november: 'November',
    december: 'December'
  },
  // Recruitment
  recruitment: {
    rating: 'Rating',
    total: 'Total',
    shortListed: 'Shortlisted',
    interviews: 'Interviews',
    offers: 'Offers',
    hired: 'Hired',
    rejected: 'Rejected',
    searchPlaceholder: 'Search candidates...',
    evaluation: 'Evaluation',
    notRated: 'Not rated',
    notesPlaceholder: 'Add interview notes or feedback...',
    scheduleInterview: 'Schedule Interview',
    selectDate: 'Please select an interview date',
    interviewDate: 'Date',
    interviewTime: 'Time',
    interviewTypeLabel: 'Interview Type',
    duration: 'Duration (min)',
    meetingLink: 'Meeting Link',
    confirmSchedule: 'Confirm Schedule',
    interviewNotesPlaceholder: 'Agenda, interviewers, things to prepare...',
    interviewType: {
      phone: 'Phone',
      video: 'Video',
      inPerson: 'In Person',
      technical: 'Technical',
      hr: 'HR'
    },
    title: 'Recruitment Pipeline',
    postNewJob: 'Post New Job',
    editJob: 'Edit job',
    confirmDeleteJob: 'Delete "{title}"? Applications for this job will also be removed.',
    deleteFailed: 'Failed to delete job posting',
    applications: 'Applications',
    candidate: 'Candidate',
    position: 'Position',
    department: 'Department',
    appliedDate: 'Applied Date',
    experience: 'Experience',
    statusLabel: "Status",
    actions: "Actions",
    stage: 'Stage',
    interviewScheduled: "Interview scheduled successfully!",
    underReview: 'Under Review',
    offerExtended: 'Offer Extended',
    screening: 'Screening',
    technical: 'Technical',
    offer: 'Offer',
    years: 'years',
    yearsExperience: 'years experience',
    // Post Job Modal
    jobTitle: 'Job Title',
    enterJobTitle: 'Enter job title',
    location: 'Location',
    enterLocation: 'Enter location',
    employmentType: 'Employment Type',
    fullTime: 'Full Time',
    partTime: 'Part Time',
    contract: 'Contract',
    internship: 'Internship',
    experienceLevel: 'Experience Level',
    enterExperience: 'e.g., 3-5 years',
    salaryMin: 'Minimum Salary',
    salaryMax: 'Maximum Salary',
    description: 'Job Description',
    enterDescription: 'Enter job description',
    requirements: 'Requirements',
    enterRequirements: 'Enter job requirements',
    postJob: 'Post Job',
    jobPosted: 'Job posted successfully!',
    statusUpdated: 'Status updated successfully!',
    // Detail Modal
    jobDetails: 'Job Details',
    applicantInfo: 'Applicant Information',
    currentCompany: 'Current Company',
    currentPosition: 'Current Position',
    education: 'Education',
    links: 'Links',
    viewResume: 'View Resume',
    linkedinProfile: 'LinkedIn Profile',
    notes: 'Notes',
    status: {
      interviewscheduled: 'Interview Scheduled',
      underreview: 'Under Review',
      offerextended: 'Offer Extended',
      hired: 'Hired',
      rejected: 'Rejected',
      shortlisted: 'Shortlisted',
      pending: 'Pending'
    },
    // Pipeline View
    subtitle: 'Manage your hiring pipeline and track candidates',
    pipelineView: 'Pipeline',
    tableView: 'Table',
    jobsView: 'Jobs',
    noJobPostings: 'No job postings found',
    postedDate: 'Posted',
    salaryRange: 'Salary',
    searchJobsPlaceholder: 'Search jobs',
    jobStatuses: {
      open: 'Open',
      active: 'Active',
      published: 'Published',
      closed: 'Closed',
      draft: 'Draft',
    },
    candidates: 'candidates',
    moveTo: 'Move to',
    reject: 'Reject',
    // Pipeline stages
    pipeline: {
      title: 'Recruitment Pipeline',
      clickToExpand: 'Click stages to view candidates',
      noCandidates: 'No candidates in this stage',
      screening: 'Screening',
      screeningDesc: 'Initial application review',
      shortlisted: 'Shortlisted',
      shortlistedDesc: 'Candidates passed initial screening',
      interview: 'Interview',
      interviewDesc: 'Interview scheduled or in progress',
      offer: 'Offer',
      offerDesc: 'Job offer extended',
      hired: 'Hired',
      hiredDesc: 'Successfully hired',
      rejected: 'Rejected',
      rejectedDesc: 'Not proceeding'
    },
    // Pipeline metrics
    metrics: {
      totalCandidates: 'Candidates',
      activeInPipeline: 'Active',
      conversionRate: 'Conversion',
      avgTimeToHire: 'Time to hire',
      offersOut: 'Offers out',
      openReqs: 'Open reqs'
    },
    // Board (2b)
    boardView: 'Board',
    viewMode: 'View',
    noApplications: 'No applications found',
    fieldsNotStored: 'Job posted, but these fields are not in your job_postings table and were not saved: {fields}',
    board: {
      allDepartments: 'All departments',
      openReqs: '{n} open requisitions',
      inPlay: '{n} candidates in play',
      acceptedThisWeek: '{n} offers accepted this week',
      passRate: '{n}% pass',
      awaitingReply: '{n} awaiting reply',
      ofTarget: 'of {n} target',
      more: '{n} more →',
      showLess: 'Show less ↑',
      yearToDate: 'Year to date',
      ytdHires: '{n} hires',
      ytdAvg: '{n}d avg',
      createRecord: 'Create record →',
      scorecard: 'Scorecard',
      move: 'Move',
      chase: 'Chase',
      revise: 'Revise',
      panelToday: 'Panel today {time}',
      panelOn: 'Panel {date} {time}',
      panelUnscheduled: 'Panel not scheduled',
      awaitingSent: 'Awaiting reply · sent {n}d ago',
      tagCvParsed: 'CV parsed',
      tagNeedsReview: 'Needs review',
      tagAwaiting: 'Awaiting',
      tagNegotiating: 'Negotiating',
      tagSigned: 'Signed',
      tagScore: 'Score {n}'
    }
  },
  // Time Tracking
  timeTracking: {
    noRecords: 'No attendance records found for this period',
    detailedBreakdown: 'Detailed Time Tracking Breakdown',
    title: 'Time Tracking Dashboard',
    overview: 'Overview',
    leaveRequests: 'Leave Requests',
    overviewTitle: 'Company Overview',
    clockIn: 'Clock In',
    clockOut: 'Clock Out',
    amountHours: 'Hours',
    hourType: 'Hour Type',
    time: 'Time',
    notes: 'Notes',
    createdAt: 'Created At',
    summary: 'Summary',
    quickActions: 'Quick Actions',
    workDays: 'Work Days',
    overtime: 'Overtime Hours',
    wfh: 'Working From Home',
    holidayOvertime: 'Holiday Overtime',
    hours: 'hours',
    days: 'days',
    thisMonth: 'This Month',
    totalHours: 'Total Hours',
    regularHours: 'Regular Hours',
    overtimeHours: 'Overtime Hours',
    hrs: 'hrs',
    attendanceRate: 'Attendance Rate',
    requestLeave: 'Request Leave',
    logOvertime: 'Log Overtime',
    vacation: 'Vacation',
    sickLeave: 'Sick Leave',
    personal: 'Personal Leave',
    unpaid: 'Unpaid Leave',
    leaveDays: "Leave Days",
    leaveType: "Leave Type",
    status: "Status",
    requestedBy: 'Requested By',
    approvedBy: 'Approved By',
    leaveRequestManagement: 'Leave Request Management',
    pending: 'Pending',
    approved: 'Approved',
    rejected: 'Rejected',
    cancelled: 'Cancelled',
    enteredByAdmin: 'Entered by admin:',
    // Renamed to avoid collision with status label
    requestStatus: {
      pending: 'Pending',
      approved: 'Approved',
      rejected: 'Rejected',
      cancelled: 'Cancelled'
    },
    // Hour types for overtime
    weekend: 'Weekend Overtime',
    holiday: 'Holiday Overtime',
    bonus: 'Bonus Hours',
    regular: 'Regular Hours',
    on_leave: 'On Leave',
    startDate: 'Start Date',
    endDate: 'End Date',
    reason: 'Reason',
    reasonPlaceholder: 'Briefly explain your leave request...',
    overtimePlaceholder: 'Describe the work performed during overtime...',
    date: 'Date',
    leaveSuccess: 'Leave request submitted successfully!',
    leaveError: 'Error submitting leave request',
    approveSuccess: 'Leave request approved',
    rejectSuccess: 'Leave request rejected',
    actionError: 'Error updating leave request',
    approve: 'Approve',
    reject: 'Reject',
    rejectReasonPrompt: 'Please enter a reason for rejection (optional):',
    overtimeSuccess: 'Overtime logged successfully!',
    overtimeError: 'Error logging overtime',
    exportSuccess: 'Report exported successfully!',
    includesPending: '*incl. pending',
    totalRegularHours: 'Total Regular Hours (All Employees)',
    pendingApprovals: 'Pending Approvals',
    regularHoursChart: 'Regular Hours by Employee',
    overtimeHoursChart: 'Overtime Hours by Employee',
    regularHoursLegend: 'Regular Hours',
    totalOvertimeLegend: 'Total Overtime',
    employee: 'Employee',
    daysWorked: 'Days Worked',
    totalHoursLabel: 'Total Hours',

    // ---- Time Tracking 2a: load against contract ----
    view: 'View',
    load: 'Load',
    ofContract: 'of',
    timesheets: 'Timesheets',
    topTen: 'Top 10',
    daysShort: 'Days',
    regularShort: 'Regular',
    overtimeShort: 'Overtime',
    totalShort: 'Total',
    employeesLower: 'employees',
    workingDaysLower: 'working days',
    workingDaysInPeriod: 'Working days in this period',
    contractLower: 'contract',
    dayShort: 'day',
    monthLower: 'month',
    loadAgainstContract: 'Load against contract',
    everyEmployee: 'Every employee, sorted by total hours. Dashed line is the contract.',
    loadCaption: '{over} of {total} employees finished above the {contract}h contract; overtime is the segment stacked on top.',
    filter: {
      missingPunch: 'Missing punch-out',
      overCap: 'Over cap',
      unapprovedOvertime: 'Unapproved overtime',
      underContract: 'Under contract'
    },

    // ---- Exceptions column ----
    exceptions: 'Exceptions',
    fixBeforePayroll: 'Fix before payroll closes',
    noExceptions: 'Nothing is blocking payroll for this period.',
    people: 'ppl',
    others: 'others',
    missingPunch: 'Missing punch',
    missingPunchOut: 'Missing punch-out',
    autoClose: 'Auto-close {time}',
    autoCloseConfirm: 'Set clock-out to {time} on {count} open entries? This updates the timesheets.',
    autoCloseSuccess: '{count} entries closed at {time}',
    reviewEach: 'Review each',
    overOvertimeCap: 'Over overtime cap',
    aboveCap: 'Above {cap}h cap',
    topIs: 'highest',
    openCases: 'Open cases',
    raiseCap: 'Raise cap',
    raiseCapPrompt: 'Monthly overtime cap, in hours:',
    unapprovedOvertime: 'Unapproved overtime',
    acrossTimesheets: '{hours}h across {count} timesheets',
    underContractHours: 'Under contract hours',
    attendanceThisWeek: 'Attendance, this week',
    onTime: 'On time',
    onTimeThisWeek: 'On-time arrivals this week',
    lateArrival: 'Late arrival',
    absentUnreported: 'Absent, unreported'
  },
  // Work Days Modal
  workDaysModal: {
    title: 'Work Days Details',
    entries: 'entries',
    totalHours: 'total hours',
    search: 'Search...',
    exportExcel: 'Export Excel',
    date: 'Date',
    clockIn: 'Clock In',
    clockOut: 'Clock Out',
    hours: 'Hours',
    notes: 'Notes',
    noData: 'No work days recorded for this period'
  },
  // Admin Time Entry
  adminTimeEntry: {
    title: 'Admin Time Entry',
    description: 'Enter time entries for employees (Admin/Manager only)',
    selectEmployee: 'Select Employee',
    selectEmployees: 'Select Employees',
    searchEmployees: 'Search employees...',
    noEmployees: 'No employees found',
    date: 'Date',
    clockIn: 'Clock In',
    clockOut: 'Clock Out',
    hourType: 'Hour Type',
    regularHours: 'Regular Hours',
    weekendOvertime: 'Weekend/Overtime',
    holiday: 'Holiday',
    bonusHours: 'Bonus Hours',
    notes: 'Notes',
    notesPlaceholder: 'Add any notes about this time entry...',
    submitButton: 'Submit Time Entry',
    submitBulkEntries: 'Submit Entries for Employees',
    submitting: 'Submitting...',
    accessDenied: 'Access Denied: You don\'t have permission to manage time entries for other employees.',
    errorLoadEmployees: 'Failed to load employees',
    success: 'Time entry added successfully',
    error: 'Failed to create time entry',
    selected: 'selected',
    selectedEmployees: 'Selected Employees',
    selectAtLeastOne: 'Please select at least one employee',
    entryAddedSuccess: 'Time entry added successfully for',
    entriesAddedSuccess: 'Time entries added successfully for',
    employees: 'employees',
    skippedEmployees: 'Skipped {count} employee(s) with existing entries: {names}',
    searchHint: 'Search by name, email or position, then pick everyone this entry is filed for.',
    willFile: 'This will file',
    totalHoursFiled: 'Total hours filed',
    autoApprovedNote: '* Entries filed here are approved on submission',
    bulkScopeNote: '* {n} employees on file',
    hourTypes: {
      regular: 'Regular Hours',
      holiday: 'Holiday',
      weekend: 'Weekend/Overtime',
      overtime: 'Overtime',
      bonus: 'Bonus Hours',
      wfh: 'Working From Home (Online)',
      onLeave: 'On Leave'
    },
    errors: {
      checkFailed: 'Failed to check for existing entries',
      allDuplicates: 'All selected employees already have time entries for {date}: {names}',
      allOverlapping: 'All selected employees have overlapping {hourType} time entries for {date}: {names}'
    },
    bulkStandardHours: {
      title: 'Bulk Standard Hours',
      description: 'Automatically create 9:00 AM – 5:00 PM regular hour entries for all employees on weekdays only. Saturdays and Sundays are excluded. Existing overlapping entries and approved leave are skipped.',
      startDate: 'Start Date',
      endDate: 'End Date',
      fillButton: 'Fill 9 AM – 5 PM for All Employees',
      filling: 'Filling standard hours...',
      confirmTitle: 'Confirm Bulk Standard Hours',
      confirmMessage: 'Create 9 AM – 5 PM regular hour entries for all employees from {start} to {end}? Saturdays and Sundays will be excluded. Overlapping entries and approved leave will be skipped.',
      confirmButton: 'Yes, Fill Hours',
      cancelButton: 'Cancel',
      success: 'Created {created} standard hour entries across {dates} weekday(s) for {employees} employee(s). Skipped {skipped} slot(s) with existing entries or approved leave. Excluded {weekends} weekend day(s).',
      noEntriesCreated: 'No entries were created. Every weekday already has an overlapping entry or approved leave.',
      noWeekdaysInRange: 'No entries were created. The selected range contains only weekends, which are excluded automatically.',
      invalidRange: 'End date must be on or after start date',
      invalidDate: 'Invalid date range.',
      rangeTooLarge: 'Date range cannot exceed {max} days',
      noEmployeesFound: 'No employees found to fill hours for.',
      errorGeneric: 'Failed to fill standard hours. Please try again.',
      errorWithDetail: 'Failed to fill standard hours: {message}'
    }
  },
  // Time Clock Entry
  timeClock: {
    title: 'New Time Entry',
    uploadFile: 'Click to upload File',
    fileTypes: 'Supports: Images, PDF, Documents (Max 50MB)',
    newEntry: 'New Entry',
    history: 'History',
    subtitle: 'Enter hours and upload attendance proof',
    selectDate: "Select Date",
    clockIn: 'Clock In',
    clockOut: 'Clock Out',
    hourType: 'Hour Type',
    proof: 'Proof of Work',
    optional: 'Optional',
    notes: 'Notes',
    notesPlaceholder: 'Add any notes about your work...',
    submit: 'Submit Time Entry',
    submitting: 'Submitting...',
    success: 'Time entry submitted successfully!',
    confirmDelete: 'Are you sure you want to delete this time entry?',
    deleteSuccess: 'Time entry deleted successfully',
    deleteOptions: 'Choose delete option',
    deleteEntry: 'Delete entire time entry',
    deleteProofOnly: 'Delete proof file only',
    enterChoice: 'Enter 1 or 2',
    invalidChoice: 'Invalid choice. Please enter 1 or 2.',
    proofDeleteSuccess: 'Proof file deleted successfully',
    proofDeleteError: 'Failed to delete proof file',
    requestLeave: 'Request Leave',
    weeklySummary: 'This Week',
    monthlySummary: 'This Month',
    leaveDays: 'Leave Days',
    thisWeek: 'This Week',
    thisMonth: 'This Month',
    total: 'Total',
    hrs: 'hrs',
    days: 'days',
    includesPending: '* Includes pending & approved',
    // 3a screen chrome — ticker, scope switch, entry consequence, ledger foot
    awaitingApproval: 'Awaiting approval',
    missingProof: 'Missing proof',
    dayShort: 'd',
    scopeMine: 'Mine',
    scopeAdmin: 'Admin',
    scopeBulk: 'Bulk',
    entryHint: 'Hours are filed for approval. Attach proof now, or from the ledger below.',
    autoDetectNote: 'Weekend and holiday follow from the date — change the type only when the day is an exception.',
    adminTag: 'Admin',
    filingFor: 'Filing for: {name}',
    changeEmployee: 'Change employee',
    willRecord: 'This entry will record',
    weekBecomes: 'This week becomes',
    remainingOf: 'Remaining of {n}h',
    fillStandard: 'Fill {from} – {to}',
    midnightNote: 'Shifts crossing midnight are counted against the day they started.',
    proofMissing: 'Missing',
    proofLoadFailed: 'This file could not be displayed.',
    showingCount: 'Showing {n} of {total} entries',
    showAll: 'Show all',
    otherTypes: '{n} other types',
    bulkExplain: 'File a standard {from}–{to} day for every active employee across a date range. Weekends are skipped.',
    openBulk: 'Open bulk entry',
    bulkScope: '{n} active employees on file',
    hourTypes: {
      regular: 'Regular Hours',
      holiday: 'Holiday',
      weekend: 'Weekend/Overtime',
      overtime: 'Overtime',
      bonus: 'Bonus Hours',
      wfh: 'Working From Home (Online)',
      onLeave: 'On Leave'
    },
    errors: {
      dateRequired: 'Date is required',
      clockInRequired: 'Clock in time is required',
      clockOutRequired: 'Clock out time is required',
      clockOutAfterClockIn: 'Clock out must be after clock in',
      submitFailed: 'Failed to submit time entry',
      deleteFailed: 'Failed to delete time entry',
      overlapping: 'This time entry overlaps with an existing entry. Please choose a different time.',
      duplicateEntry: 'You already have a {hourType} time entry for this date. Please edit the existing entry or choose a different hour type.',
      overlappingEntry: 'This {hourType} time entry overlaps with an existing entry ({existingIn} - {existingOut}). Please choose a different time.',
      fileTooLarge: 'File size must be less than 50MB',
      invalidFileType: 'Only images, PDF, and document files are allowed'
    },
    proofUploadSuccess: 'Proof file uploaded successfully',
    proofUploadError: 'Failed to upload proof file',
    uploadProof: 'Upload proof file',
    viewProof: 'View proof image',
    downloadProof: 'Download proof file',
    approve: 'Approve',
    approvalSuccess: 'Time entry approved successfully',
    approvalError: 'Failed to approve entry',
    viewEntries: 'View Entries',
    filterByStatus: 'Status',
    allStatuses: 'All Statuses',
    reviewPendingBanner: 'Showing time entries awaiting your review',
    noPendingEntries: 'No pending time entries to review',
    myEntries: 'My Entries',
    allEmployees: 'All Employees',
    specificEmployee: 'Specific Employee',
    employee: "Employee",
    date: 'Date',
    time: 'Time',
    hours: 'Hours',
    type: 'Type',
    status: {
      pending: 'Pending',
      approved: 'Approved',
      rejected: 'Rejected',
      cancelled: 'Cancelled',
    },
    actions: 'Actions',
    noEntries: 'No time entries found'
  },
  // Personal Goals
  personalGoals: {
    title: 'Personal Goals',
    overallRating: 'Overall Rating',
    reviewPeriod: 'Review Period',
    goals: 'Goals & Objectives',
    achievements: 'Key Achievements',
    strengths: 'Strengths',
    areasForImprovement: 'Areas for Improvement',
    skillsAssessment: 'Skills Assessment',
    teamwork: 'Teamwork',
    problemSolving: 'Problem Solving',
    rating: 'Rating',
    excellent: 'Excellent',
    good: 'Good',
    average: 'Average',
    needsImprovement: 'Needs Improvement',
    comments: 'Comments',
    save: 'Save Appraisal',
    lastReview: 'Last Review',
    nextReview: 'Next Review Due',
    q1_2025: 'Q1 2025',
    q2_2025: 'Q2 2025',
    q3_2025: 'Q3 2025',
    q4_2025: 'Q4 2025',
    q1_2026: 'Q1 2026',
    q2_2026: 'Q2 2026',
    q3_2026: 'Q3 2026',
    q4_2026: 'Q4 2026',
    overallPerformance: 'Overall Performance',
    goalsCompleted: 'Goals Completed',
    reviewsThisPeriod: 'Reviews This Period',
    avgSkillRating: 'Avg Skill Rating',
    completed: 'Completed',
    inProgress: 'In Progress',
    pending: 'Pending',
    // Review Types
    weekly: 'Weekly',
    monthly: 'Monthly',
    quarterly: "Quarterly",
    midYear: "Mid-Year",
    annual: "Annual",
    probation: 'Probation',
    adHoc: 'Ad-Hoc',
    // Period Types
    periodType: 'Period Type',
    byMonth: 'By Month',
    byQuarter: 'By Quarter',
    byYear: 'By Year',
    year: 'Year',
    quarter: 'Quarter',
    month: 'Month',
    q1: 'Q1 (Jan-Mar)',
    q2: 'Q2 (Apr-Jun)',
    q3: 'Q3 (Jul-Sep)',
    q4: 'Q4 (Oct-Dec)',
    reviewPeriodPlaceholder: 'e.g., Q4 2024',
    reviewType: 'Review Type',
    ratingUpdated: 'Performance rating updated successfully!',
    ratingUpdateError: 'Failed to update performance rating',
    skillUpdateError: 'Failed to update skill rating',
    beginner: "Beginner",
    intermediate: "Intermediate",
    advanced: "Advanced",
    technicalSkills: 'Technical Skills',
    // Tabs
    overview: 'Overview',
    goalsTab: 'Goals',
    reviewsTab: 'Reviews',
    // Goals section
    currentGoals: 'Current Goals',
    addGoal: 'Add Goal',
    addNewGoal: 'Add New Goal',
    performanceGoals: 'Performance Goals',
    progress: 'Progress',
    in_progress: 'In Progress',
    clickToRate: 'Click stars to update employee rating',
    deadline: 'Deadline',
    due: 'Due',
    complete: 'complete',
    viewDetails: 'View Details',
    // Reviews section
    performanceReviews: 'Performance Reviews',
    newReview: "New Performance Review",
    by: 'by',
    viewFullReview: 'View Full Review',
    // General
    status: 'Status',
    actions: 'Actions',
    edit: 'Edit',
    view: 'View',
    editGoal: 'Edit Goal',
    goalUpdatedSuccess: 'Goal updated successfully!',
    goalUpdatedError: 'Failed to update goal',
    progressSaved: 'Progress saved successfully!',
    progressSaveError: 'Failed to save progress',
    progressPercentage: 'Progress',
    goalCreatedSuccess: 'Goal created successfully!',
    goalCreatedError: 'Failed to create goal',
    reviewCreatedSuccess: "Review created successfully!",
    reviewCreatedError: "Failed to create review",
    goalTitle: "Goal Title",
    goalTitlePlaceholder: "Enter goal title",
    goalDescription: 'Description',
    goalDescriptionPlaceholder: 'Describe the goal objectives',
    category: 'Category',
    priority: 'Priority',
    low: 'Low',
    medium: 'Medium',
    high: 'High',
    critical: 'Critical',
    targetDate: 'Target Date',
    cancelled: 'Cancelled',
    onHold: 'On Hold',
    reviewDetails: 'Performance Review Details',
    reviewer: 'Reviewer',
    reviewDate: 'Review Date',
    ratings: 'Ratings',
    reviewComments: 'Review Comments',
    noComments: 'No comments yet',
    errorFetchingComments: 'Error fetching comments',
    general: "General",
    technical: "Technical Skills",
    leadership: "Leadership",
    project: "Project",
    communication: "Communication",
    professionalDevelopment: "Professional Development",
    // Missing toast/alert translation keys
    goalDeletedSuccess: 'Goal deleted successfully!',
    goalDeletedError: 'Failed to delete goal',
    goalsInProgress: 'Goals In Progress',

    // ---- Personal Goals 2c: calibrated assessment ----
    history: 'History',
    period: 'Period',
    employee: 'Employee',
    reviewDue: 'Review due',
    exportReview: 'Export review',
    withCompany: 'with the company',
    yearsShort: 'y',
    monthsShort: 'm',
    assessmentLead: 'Self-rating as fill, manager as marker, company median dashed.',
    self: 'Self',
    manager: 'Manager',
    median: 'Median',
    mgrShort: 'mgr',
    noManagerShort: 'no mgr rating',
    adjustRatings: 'Adjust ratings',
    gapsToDiscuss: '{count} gaps to discuss',
    ratedAboveSelf: 'rated above self',
    ratedBelowSelf: 'below',
    noGaps: 'Self and manager agree within half a point across every skill.',
    noManagerRatings: 'No manager ratings for this period yet.',
    enterManagerRatings: 'Enter manager ratings',
    saveManagerReview: 'Save manager review',
    managerReviewSaved: 'Manager review saved.',
    managerReviewSaveError: 'Failed to save manager review',
    needManagerRating: 'Rate at least one skill before saving.',
    managerAssessmentLead: 'The fill is their self-rating. Place your mark on the same track.',
    submitForCalibration: 'Submit for calibration',
    submittedForCalibration: 'Submitted for calibration.',
    submitCalibrationError: 'Failed to submit for calibration',
    readyToSubmitCalibration: 'Ready to submit',
    openPerformanceReviews: 'Open Performance Reviews',
    signOffOnReviews: 'Sign off on Performance Reviews',

    // Goals section
    onTrack: 'On track',
    atRisk: 'At risk',
    closed: 'Closed',
    daysLeft: '{n} days left',
    overdueDays: '{n} days overdue',
    expectedBy: 'timeline says {n}%',
    inProgressLower: 'in progress',
    completedThisYear: 'completed',
    noReviews: 'No reviews recorded yet.',

    // Right column
    reviewCycle: 'Review cycle',
    closes: 'Closes',
    byDate: 'by',
    notYet: 'Not yet',
    awaitingSchedule: 'Not scheduled',
    stepSelfAssessment: 'Self-assessment submitted',
    stepManagerRating: 'Manager rating entered',
    stepCalibration: 'Calibration meeting',
    stepSignOff: 'Sign-off & next-quarter goals',
    ratingHistory: 'Rating history',
    noRatingHistory: 'No rated quarters yet.',
    managerNote: 'Manager note',
    noManagerNote: 'No written feedback for this period yet.',
    acknowledge: 'Acknowledge',
    acknowledged: 'Acknowledged',
    reply: 'Reply',
    yourReply: 'Your reply',
    replyPrompt: 'Your response to this review:',
    replyError: 'Could not save your response'
  },
  // Reports
  reports: {
    // ── Export spec sheet ──────────────────────────────────────────
    inScope: 'In scope',
    range: 'Range',
    people: 'People',
    peopleInScope: 'people in scope',
    peopleWithoutRecords: 'People without records in range',
    withoutRecords: 'without records',
    pendingTickerHint: 'Time entries and leave still awaiting approval',
    lastExport: 'Last export',
    by: 'by',
    selected: 'selected',
    byUnit: 'By unit',
    activeOnly: 'Active only',
    activeOnlyHint: 'Exclude people who have left the company',
    quarter: 'Quarter',
    custom: 'Custom',
    workingDays: 'working days',
    byType: 'By type',
    byStatus: 'By status',
    byWeek: 'By week',
    hoursShort: 'h',
    daysShort: 'd',
    timeAndLeave: 'time & leave',
    pendingApproval: 'Pending approval',
    noApprovableRecords: 'No approvable records in scope',
    beforeYouExport: 'Before you export',
    rowsReady: 'rows ready — nothing needs attention.',
    checkEntriesPending: 'entries are still pending',
    checkLeavePending: 'leave requests are still pending',
    checkExportsAsPending: 'they will export with status PENDING.',
    checkApproveFirst: 'Approve first',
    checkOpenLeave: 'Open leave',
    checkPeopleNoRecords: 'people have no records in range',
    checkOpenList: 'Open list',
    checkLongShifts: 'days booked over 12h for one person',
    checkLongShiftsNote: 'usually a duplicate punch rather than a real shift.',
    checkLongShiftsShort: 'Over 12h',
    checkRejectedRows: 'rejected rows are in scope',
    checkRejectedNote: 'they export with status REJECTED.',
    checkReview: 'Review',
    preview: 'Preview',
    rows: 'rows',
    showing: 'Showing',
    load: 'Load',
    sortedBy: 'Sorted by',
    chooseColumns: 'Choose columns',
    previewColumns: 'Preview columns',
    previewColumnsNote: 'Affects this preview. Exports always carry every field.',
    exportCarriesEveryRow: 'the export carries every row in scope, not the preview',
    unit: 'Unit',
    approvedBy: 'Approved by',
    source: 'Source',
    sourceTime: 'Time entries',
    sourceLeave: 'Leave management',
    sourceTasks: 'Task listing',
    sourceGoals: 'Personal goals',
    writes: 'Writes',
    recordTypes: 'record types',
    estimated: 'est.',
    format: 'Format',
    regular: 'Regular',
    wfh: 'WFH',
    days: 'Days',
    bonusHours: 'Bonus Hours',
    title: 'Reports & Analytics',
    filenamePrefix: 'HR_Report_',
    leave: 'Leave Requests',
    totalLeaveDays: 'Total Leave Days',
    excel: {
      charts: {
        hoursByType: 'Hours by Type',
        statusDistribution: 'Status Distribution',
        taskStatusDistribution: 'Task Status Distribution',
        taskPriorityDistribution: 'Task Priority Distribution',
        goalStatusDistribution: 'Goal Status Distribution',
        goalCategoryDistribution: 'Goal Category Distribution'
      },
      sheets: {
        summary: 'Summary',
        performance: 'Employee Performance',
        allEmployeesOverview: 'All Employees Overview',
        charts: 'Charts & Metrics',
        timeEntries: 'Time Entries',
        tasks: 'Tasks',
        goals: 'Goals'
      },
      summaryTitle: 'HR Report Summary',
      generated: 'Generated',
      dateRange: 'Date Range',
      employee: 'Employee',
      reportLanguage: 'Report Language',
      visual: 'Visual',
      timeTracking: 'Time Tracking Summary',
      workload: 'Workload Summary',
      goals: 'Goals Summary',
      metrics: {
        totalTimeEntries: 'Total Time Entries',
        totalHours: 'Total Hours Logged',
        regularHours: 'Regular Hours',
        overtimeHours: 'Overtime Hours',
        wfhHours: 'WFH Hours',
        pendingApprovals: 'Pending Approvals',
        approvedEntries: 'Approved Entries',
        totalTasks: 'Total Tasks',
        completedTasks: 'Completed Tasks',
        inProgress: 'In Progress',
        highPriorityTasks: 'High Priority Tasks',
        estimatedHours: 'Estimated Days',
        actualHours: 'Actual Days',
        variance: 'Variance',
        totalGoals: 'Total Goals',
        completedGoals: 'Completed Goals',
        avgProgress: 'Average Progress'
      },
      performance: {
        header: 'Performance Report',
        employeeInfo: 'Employee Information',
        name: 'Name',
        department: 'Department',
        position: 'Position',
        email: 'Email',
        reportPeriod: 'Report Period',
        performanceMetrics: 'Performance Metrics',
        tableHeaders: {
          metric: 'Metric',
          value: 'Value',
          status: 'Status',
          notes: 'Notes'
        },
        approvalRate: 'Approval Rate',
        taskCompletionRate: 'Task Completion Rate',
        avgGoalProgress: 'Average Goal Progress',
        overallRating: 'Overall Performance Rating',
        overallScore: 'Overall Performance Score:',
        ratingLabel: 'Rating:'
      },
      status: {
        high: 'High',
        normal: 'Normal',
        tracked: 'Tracked',
        pending: 'Pending',
        allApproved: 'All Approved',
        active: 'Active',
        noTasks: 'No Tasks',
        excellent: 'Excellent',
        good: 'Good',
        needsImprovement: 'Needs Improvement',
        set: 'Set',
        onTrack: 'On Track',
        progressing: 'Progressing',
        behind: 'Behind'
      },
      rating: {
        outstanding: 'Outstanding',
        excellent: 'Excellent',
        good: 'Good',
        satisfactory: 'Satisfactory',
        needsImprovement: 'Needs Improvement'
      },
      entries: 'entries',
      completedShort: 'completed',
      ofTotal: 'of total',
      headers: {
        dataType: 'Data Type',
        type: 'Type',
        hours: 'Hours',
        status: 'Status',
        count: 'Count',
        priority: 'Priority',
        employee: 'Employee',
        department: 'Department',
        position: 'Position',
        date: 'Date',
        clockIn: 'Clock In',
        clockOut: 'Clock Out',
        hourType: 'Hour Type',
        notes: 'Notes',
        createdAt: 'Created At',
        taskTitle: 'Task Title',
        description: 'Description',
        dueDate: 'Due Date',
        estimatedHours: 'Estimated Days',
        actualHours: 'Actual Days',
        variance: 'Variance',
        goalTitle: 'Goal Title',
        category: 'Category',
        progress: 'Progress (%)',
        targetDate: 'Target Date',
        updatedAt: 'Updated At'
      }
    },
    // Report Types
    reportTypes: 'Report Types',
    performanceReport: 'Performance Reports',
    language: 'Language',
    timeEntry: 'Time Entries',
    performanceSummary: 'Performance Summary',
    applyFilters: 'Apply Filters',
    totalEntries: 'Total Entries',
    averageHoursPerEntry: 'Average Hours per Entry',
    highestHoursEntry: 'Highest Hours Entry',
    lowestHoursEntry: 'Lowest Hours Entry',
    exportReport: 'Export Report',
    resetFilters: 'Reset Filters',
    reportGenerated: 'Report generated successfully!',
    reportError: 'Failed to generate report',
    personalGoals: "Personal Goals",
    personalGoalsReport: 'Personal Goals Report',
    recruitmentReport: 'Recruitment Report',
    pdf: {
      visualAnalytics: 'Visual Analytics',
      charts: {
        hoursByType: 'Hours by Type',
        statusDistribution: 'Time Entry Status Distribution',
        taskStatusDistribution: 'Task Status Distribution',
        taskPriorityDistribution: 'Task Priority Distribution',
        goalStatusDistribution: 'Goal Status Distribution',
        goalCategoryDistribution: 'Goal Category Distribution'
      },
      headers: {
        employee: 'Employee',
        department: 'Department',
        position: 'Position',
        date: 'Date',
        clockIn: 'Clock In',
        clockOut: 'Clock Out',
        hours: 'Hours',
        hourType: 'Hour Type',
        status: 'Status',
        notes: 'Notes',
        createdAt: 'Created At',
        taskTitle: 'Task Title',
        description: 'Description',
        priority: 'Priority',
        dueDate: 'Due Date',
        estimatedHours: 'Est. days',
        actualHours: 'Actual days',
        variance: 'Variance',
        goalTitle: 'Goal Title',
        category: 'Category',
        progress: 'Progress (%)',
        targetDate: 'Target Date',
        updatedAt: 'Updated At'
      }
    },
    performanceReportSubtitle: "Generate detailed reports on employee performance, attendance, and recruitment metrics",
    individualReport: 'Individual Reports',
    individualMetrics: 'Individual Metrics',
    comprehensiveReports: 'Comprehensive Reports',
    exportData: 'Export Data',
    exportDataSubtitle: 'Export detailed data for time entries, tasks, and personal goals',
    selectReportType: "Select Report Type",
    selectDateRange: "Select Date Range",
    selectFilters: "Select Filters",
    previewData: "Preview Data",
    exportCSV: "Export to CSV",
    exportingIncludes: "*Exporting includes all filtered data, not just previewed records",
    categories: "Report Categories",
    performanceReports: 'Performance Reports',
    attendanceReports: "Attendance Reports",
    recruitmentReports: "Recruitment Reports",
    salaryReports: "Salary Reports",
    employeeGrowth: "Employee Growth",
    departmentDistribution: "Department Distribution",
    performanceMetrics: "Performance Metrics",
    salaryReport: "Salary Report",
    generateReport: "Generate Report",
    exportPDF: 'Export PDF',
    exportExcel: 'Export Excel',
    filterBy: 'Filter By',
    allDepartments: 'All Departments',
    performance: "Performance",
    salary: "Salary",
    attendance: "Attendance",
    recruitment: "Recruitment",
    department: "Department",
    all: 'All',
    // Navigation
    overview: 'Overview',
    detailedReports: 'Detailed Reports',
    filters: "Filters",
    exportAll: 'Export All',
    // Charts
    departmentOverview: 'Department Overview',
    attendanceOverview: 'Attendance Overview',
    recruitmentFunnel: 'Recruitment Funnel',
    performanceTrend: 'Performance Trend',
    // Stats
    totalEmployees: 'Total Employees',
    newHires: 'New Hires',
    turnoverRate: 'Turnover Rate',
    annualRate: 'Annual rate',
    avgSalary: 'Avg Salary',
    satisfaction: 'Satisfaction',
    productivity: 'Productivity',
    // Attendance
    present: 'Present',
    onLeave: 'On Leave',
    absent: 'Absent',
    // Recruitment labels
    totalApplications: 'Total Applications',
    interviewed: 'Interviewed',
    hired: 'Hired',
    rejected: 'Rejected',
    employees: 'employees',
    generateCustomReport: "Generate Custom Report",
    reportType: "Report Type",
    employeePerformance: "Employee Performance",
    salaryAnalysis: "Salary Analysis",
    attendanceReport: "Attendance Report",
    recruitmentMetrics: "Recruitment Metrics",
    departmentComparison: "Department Comparison",
    exportToPDF: "Export to PDF",
    exportToExcel: 'Export to Excel',
    prebuiltReports: "Pre-built Reports",
    // Pre-built report names
    monthlyPerformanceReview: 'Monthly Performance Review',
    comprehensivePerformanceAnalysis: 'Comprehensive performance analysis',
    salaryBenchmarking: 'Salary Benchmarking',
    compareSalariesAcrossDepartments: 'Compare salaries across departments',
    attendanceAnalytics: 'Attendance Analytics',
    trackAttendancePatterns: 'Track attendance patterns and trends',
    recruitmentPipeline: 'Recruitment Pipeline',
    monitorHiringProcess: 'Monitor hiring process efficiency',
    employeeTurnover: 'Employee Turnover',
    analyzeRetentionRates: 'Analyze retention and turnover rates',
    trainingEffectiveness: 'Training Effectiveness',
    measureTrainingSuccess: 'Measure training program success',
    generate: 'Generate →',
    fromLastPeriod: 'from last period',
    lastWeek: 'Last Week',
    lastQuarter: 'Last Quarter',
    lastYear: 'Last Year',
    // Report Display
    reportResults: 'Report Results',
    totalReviews: 'Total Reviews',
    averageRating: 'Average Rating',
    rating: 'Rating',
    reviewDate: 'Review Date',
    reviewer: 'Reviewer',
    position: 'Position',
    averageSalary: 'Average Salary',
    exportToCSV: "Export to CSV",
    exportAllData: 'Export All Data',
    export: 'Export',
    exportScopeNote: 'Exports carry every filtered record, not just the 50 previewed.',
    leaveType: 'Type',
    rate: 'Rate',
    // Date Range Options
    today: 'Today',
    thisWeek: 'This Week',
    thisMonth: 'This Month',
    lastMonth: "Last Month",
    thisQuarter: "This Quarter",
    thisYear: 'This Year',
    customRange: "Custom Range",
    timeEntries: "Time Entries Report",
    tasks: 'Tasks',
    goals: 'Personal Goals',
    employee: "Employee",
    dateRange: "Date Range",
    dataType: 'Data Type',
    quickFilters: "Quick Filters",
    allEmployees: "All Employees",
    startDate: 'Start Date',
    endDate: 'End Date',
    recordsFound: "records found",
    noData: 'No data found',
    adjustFilters: 'Try adjusting your filters or date range',
    dataPreview: 'Data Preview',
    showingFirst50: 'Showing first 50 records',
    showingFirst50of: 'Showing first 50 of',
    records: 'records',
    exportForAll: 'Export to CSV to get all records.',
    liveData: 'Live data from Supabase',
    subtitle: "Export comprehensive data for time entries, tasks, and personal goals",
    errorExporting: 'Error exporting data',
    exportSuccess: 'Data exported successfully',
    csvExportSuccess: 'CSV report exported successfully with all data types in one file!',
    excelExportHint: 'Export all data types with summary, charts, and detailed sheets',
    pdfExportHint: 'Export PDF with visual charts, summary, and detailed tables for all data types',
    pdfExportSuccess: 'PDF report exported successfully!',
    reportPeriod: 'Report Period',
    to: 'to',
    totalHours: "Total Hours",
    regularHours: 'Regular Hours',
    overtime: 'Overtime',
    leaveDays: 'Leave Days',
    daysWorked: 'Days Worked',
    tasksDone: 'Tasks Done',
    completion: 'Completion Rate',
    goalProgress: 'Goal Progress',
    approved: 'Approved',
    pending: 'Pending',
    completed: 'Completed',
    inProgress: 'In Progress',
    completionRate: 'Completion Rate',
    avgProgress: 'Avg Progress',
    totalGoals: 'Total Goals',
    type: 'Type',
    details: 'Details',
    status: 'Status',
    date: 'Date',
    for: 'for',
    from: 'from',
    // PDF Export specific translations
    generated: 'Generated ',
    period: 'Period',
    summaryOverview: 'Summary Overview',
    totalRecords: 'Total Records',
    achievedGoals: 'Achieved Goals',
    completedTasks: 'Completed Tasks',
    unknown: 'Unknown',
    task: 'Task',
    priority: 'Priority',
    dueDate: 'Due Date',
    goal: 'Goal',
    category: 'Category',
    progress: 'Progress',
    clockIn: 'Clock In',
    clockOut: 'Clock Out',
    hours: 'Hours',
    est: 'Est.',
    actual: 'Actual',
    targetDate: 'Target Date',
    page: 'Page',
    of: 'of',
    generatedBy: 'Generated by HR Management System',
    achieved: 'Achieved',
    priorityLow: 'Low',
    priorityMedium: 'Medium',
    priorityHigh: 'High',
    statusApproved: 'Approved',
    statusRejected: 'Rejected',
    statusPending: 'Pending',
    statusCompleted: 'Completed',
    statusInProgress: 'In Progress',
    statusNotStarted: 'Not Started'
  },
  // Status translations
  status: {
    approved: 'Approved',
    pending: 'Pending',
    rejected: 'Rejected',
    completed: 'Completed',
    active: 'Active',
    delayed: 'Delayed',
    'in-progress': 'In Progress',
    'not-started': 'Not Started',
    achieved: 'Achieved',
    'on-hold': 'On Hold'
  },
  // Data Type Labels
  dataType: {
    timeEntry: 'Time Entry',
    task: 'Task',
    goal: 'Goal'
  },
  // Common
  datePicker: {
    placeholder: 'Select date',
    calendar: 'Calendar',
    prevMonth: 'Previous month',
    nextMonth: 'Next month',
    today: 'Today',
    clear: 'Clear'
  },
  timePicker: {
    period: 'AM/PM',
    hours: 'hours',
    minutes: 'min',
    done: 'Done',
    clear: 'Clear',
    placeholder: '--:--',
    picker: 'Time picker'
  },
  routeError: {
    message: 'This screen could not be opened. The rest of the app is still working — pick another page from the menu, or reload.'
  },

  common: {
    refresh: 'Refresh',
    reload: 'Reload',
    previous: 'Previous',
    retry: 'Try Again',
    results: 'Results',
    total: 'Total',
    update: 'Update',
    updating: 'Updating...',
    name: 'Name',
    email: 'Email',
    phone: 'Phone',
    address: 'Address',
    dateOfBirth: 'Date of Birth',
    startDate: 'Start Date',
    salary: 'Salary',
    performance: 'Performance',
    searchPlaceholder: 'Search...',
    employee: 'Employee',
    department: "Department",
    position: "Position",
    date: 'Date',
    actions: 'Actions',
    clockIn: 'Clock In',
    clockOut: 'Clock Out',
    hours: 'Hours',
    hourType: 'Hour Type',
    notes: 'Notes',
    status: 'Status',
    type: 'Type',
    viewDetails: "View Details",
    leaveRequest: 'Leave Request',
    search: 'Search',
    filter: 'Filter',
    next: 'Next',
    back: 'Back',
    sort: 'Sort',
    save: 'Save',
    saving: 'Saving...',
    submit: 'Submit',
    submitting: 'Submitting...',
    cancel: 'Cancel',
    delete: 'Delete',
    edit: 'Edit',
    view: 'View',
    add: 'Add',
    close: 'Close',
    loading: 'Loading...',
    noData: 'No data available',
    error: 'An error occurred',
    success: 'Operation successful',
    confirm: 'Are you sure?',
    yes: 'Yes',
    no: 'No',
    accessDenied: 'Access Denied',
    noPermission: 'You do not have permission to access this page.',
    goBack: 'Go Back',
    days: 'days',
    day: 'day',
    notAvailable: 'N/A',
    select: 'Select',
    selectDate: 'Select Date',
    selectEmployee: 'Select Employee',
    uploadFile: 'Upload File',
    dropFileHere: 'or drop file here',
    changeFile: 'Change File',
    removeFile: 'Remove File',
    browseFiles: 'Browse Files',
    fileTooLarge: 'File size exceeds the limit',
    invalidFileType: 'Invalid file type',
    download: 'Download',
    viewFile: 'View File',
    fetching: 'Fetching',
    ready: 'Ready',
    ms: 'ms',
    approve: 'Approve',
    decline: 'Decline',
    pending: 'Pending',
    unknown: 'Unknown',
    done: 'Done',
    noDescription: 'No description available',
    export: 'Export',
    all: 'All'
  },
  // Theme
  theme: {
    light: 'Light Mode',
    dark: 'Dark Mode',
    toggle: 'Toggle Theme'
  },
  // Language
  language: {
    select: 'Select Language',
    current: 'Current Language'
  },
  // Time Tracking Actions
  timeTrackingActions: {
    recordTime: 'Record Time',
    requestLeave: 'Enter Leave Days',
    logOvertime: 'Log Overtime',
    exportReport: 'Export Report'
  },
  // Recruitment Actions
  recruitmentActions: {
    view: 'View',
    schedule: 'Schedule',
    reject: 'Reject'
  },
  // Employee Status
  employeeStatus: {
    active: 'Active',
    inactive: 'Inactive',
    onLeave: 'On Leave',
    pending: 'Pending',
    outSourceContractor: 'Outsource Contractor'
  },
  employeePosition: {
    general_manager: 'General Manager',
    senior_developer: 'Senior Developer',
    hr_specialist: 'HR Manager',
    accountant: 'Chief Accountant',
    contract_manager: 'Contract Manager',
    managing_director: 'Managing Director',
    support_staff: 'Support Staff',
    expertGroup: 'Experts Group',
    employee: 'Employee'
  },
  // Recruitment Status
  recruitmentStatus: {
    active: 'Active',
    interviewscheduled: 'Interview Scheduled',
    interviewScheduled: 'Interview Scheduled',
    underreview: 'Under Review',
    underReview: 'Under Review',
    offerextended: 'Offer Extended',
    offerExtended: 'Offer Extended',
    offer: 'Offer',
    hired: 'Hired',
    rejected: 'Rejected',
    screening: 'Screening',
    technical: 'Technical',
    shortlisted: 'Shortlisted'
  },
  // Performance Goals
  goals: {
    completeReactProject: 'Complete React Project',
    improveCodeQuality: 'Improve Code Quality',
    mentoringJuniorDevelopers: 'Mentoring Junior Developers',
    apiDevelopment: 'API Development',
    databaseOptimization: 'Database Optimization',
    backendDevelopment: 'Backend Development',
    teamCollaboration: 'Team Collaboration'
  },
  // Review Types
  reviewTypes: {
    quarterlyReview: 'Quarterly Review',
    midYearReview: 'Mid-year Review',
    annualReview: 'Annual Review'
  },
  // Skill Categories
  skillCategories: {
    technical: 'Technical',
    soft: 'Soft Skills',
    leadership: 'Leadership',
    communication: 'Communication'
  },
  // Notifications
  notifications: {
    title: 'Notifications',
    // Ledger chrome
    shown: 'Shown',
    nShownOf: '{shown} of {total} shown',
    ofN: 'of {n}',
    byCategory: 'By category',
    byType: 'By type',
    leavingMarksRead: 'Leaving this screen marks everything shown as read.',
    employeeAdded: 'Employee Added',
    addedTo: 'added to',
    unreadCount: '{0} unread',
    allCaughtUp: 'You\'re all caught up!',
    refresh: 'Refresh',
    filters: 'Filters',
    markAllRead: 'Mark all as read',
    deleteAll: 'Delete all',
    confirmDeleteAll: 'Are you sure you want to delete all notifications?',
    total: 'Total',
    unread: 'Unread',
    errors: 'Errors',
    warnings: 'Warnings',
    filterBy: 'Filter By',
    status: 'Status',
    type: 'Type',
    category: 'Category',
    allNotifications: 'All Notifications',
    unreadOnly: 'Unread Only',
    readOnly: 'Read Only',
    allTypes: 'All Types',
    info: 'Info',
    success: 'Success',
    warning: 'Warning',
    error: 'Error',
    allCategories: 'All Categories',
    general: 'General',
    timeTracking: 'Time Tracking',
    performance: 'Performance',
    employee: 'Employee',
    recruitment: 'Recruitment',
    system: 'System',
    noNotifications: 'No notifications',
    noNotificationsFilter: 'No notifications match your filters',
    noNotificationsYet: 'You don\'t have any notifications yet',
    justNow: 'Just now',
    minutesAgo: '{0}m ago',
    hoursAgo: '{0}h ago',
    daysAgo: '{0}d ago',
    markAsRead: 'Mark as read',
    delete: 'Delete',
    actionFailedMarkRead: 'Could not mark notification as read. Please try again.',
    actionFailedMarkAllRead: 'Could not mark all notifications as read. Please try again.',
    actionFailedDelete: 'Could not delete notification. Please try again.',
    actionFailedDeleteAll: 'Could not delete all notifications. Please try again.',
    viewAll: 'View all notifications',
    clearFilters: 'Clear filters',
    loadMore: 'Load more',
    loadingMore: 'Loading...',
    markReadOnLeave: 'Mark visible as read when leaving',
    // Notification titles
    pendingApprovals: 'Pending Approvals',
    timeEntryApproved: 'Time Entry Approved',
    timeEntryRejected: 'Time Entry Rejected',
    newEmployeeAdded: 'New Employee Added',
    performanceReview: 'Performance Review',
    systemUpdate: 'System Update',
    // Notification messages
    timeEntriesAwaiting: 'You have {0} time entries awaiting approval',
    reviewNow: 'Review Now',
    viewDetails: 'View Details'
  },
  // Settings
  settings: {
    title: 'Settings',
    subtitle: 'Manage your preferences and account settings',
    saved: 'Saved!',
    export: 'Export',
    import: 'Import',
    reset: 'Reset',
    saveChanges: 'Save Changes',
    confirmReset: 'Are you sure you want to reset all settings to default?',
    importSuccess: 'Settings imported successfully!',
    importError: 'Failed to import settings',
    // Tabs
    notifications: 'Notifications',
    appearance: 'Appearance',
    language: 'Language',
    privacy: 'Privacy',
    work: 'Work Preferences',
    // Notification Settings
    notificationPreferences: 'Notification Preferences',
    emailNotifications: 'Email Notifications',
    emailNotificationsDesc: 'Receive notifications via email',
    pushNotifications: 'Push Notifications',
    pushNotificationsDesc: 'Receive push notifications in the app',
    desktopNotifications: 'Desktop Notifications',
    desktopNotificationsDesc: 'Show browser notifications on desktop',
    desktopDenied: 'Notifications are blocked for {origin}. In the address bar, set Notifications to Allow, then turn this on again.',
    desktopEmbedded: 'This window cannot show desktop notifications. Open {origin} in Chrome or Safari, then allow notifications.',
    desktopInsecure: 'Desktop notifications need http://localhost or https. This page is {origin}.',
    desktopUnsupported: 'This browser cannot show desktop notifications.',
    desktopTestTitle: 'Desktop notifications are on',
    desktopTestBody: 'You will see alerts like this when new notifications arrive.',
    desktopPermissionRequired: 'Notification permission was not granted for {origin}. Allow it in this browser’s site controls, then try again.',
    notificationSound: 'Notification Sound',
    notificationSoundDesc: 'Play a sound when new notifications arrive',
    notificationFrequency: 'Notification Frequency',
    realtime: 'Real-time',
    daily: 'Daily Digest',
    weekly: 'Weekly Summary',
    notifyMeAbout: 'Notify me about',
    timeTrackingNotifications: 'Time Tracking',
    performanceNotifications: 'Performance Reviews',
    employeeNotifications: 'Employee Updates',
    recruitmentNotifications: 'Recruitment',
    systemNotifications: 'System Updates',
    // Appearance Settings
    appearanceSettings: 'Appearance Settings',
    theme: 'Theme',
    dateFormat: 'Date Format',
    timeFormat: 'Time Format',
    itemsPerPage: 'Items Per Page',
    // Language & Region
    languageRegion: 'Language & Region',
    timezone: 'Timezone',
    timezoneNote: 'Currently showing first 50 timezones',
    // Privacy Settings
    privacySettings: 'Privacy Settings',
    profileVisibility: 'Profile Visibility',
    visibilityAll: 'Everyone',
    visibilityTeam: 'My Team',
    visibilityManagers: 'Managers Only',
    visibilityPrivate: 'Private',
    contactVisibility: 'Contact Information Visibility',
    showEmail: 'Show Email Address',
    showPhone: 'Show Phone Number',
    // Work Preferences
    workPreferences: 'Work Preferences',
    defaultDashboard: 'Default Dashboard View',
    overviewView: 'Overview',
    detailedView: 'Detailed',
    compactView: 'Compact',
    autoClockOut: 'Auto Clock Out',
    autoClockOutDesc: 'Automatically clock out at a specific time',
    autoClockOutTime: 'Auto Clock Out Time',
    weeklyReport: 'Weekly Report',
    weeklyReportDesc: 'Receive a weekly summary of your work activities',
    // Spec-sheet chrome
    on: 'On',
    off: 'Off',
    sections: 'Settings sections',
    unsaved: 'Unsaved',
    unsavedChanges: 'Unsaved changes',
    nUnsaved: '{n} unsaved',
    allSaved: 'All changes saved',
    nItems: '{n} items',
    nothingUntilSaved: 'Nothing takes effect until saved',
    queueEmpty: 'The draft and your saved settings agree. Change a row to open one.',
    revert: 'Revert',
    account: 'Account',
    saveN: 'Save {n} changes',
    nothingToSave: 'Nothing to save',
    loadFailed: 'Your settings could not be loaded.',
    themes: {
      light: 'Light',
      dark: 'Dark',
      system: 'System'
    },
    scope: {
      notifications: 'Scope: this account · applies on your next sign-in',
      topics: 'Scope: every channel above · a topic that is off is never sent',
      appearance: 'Scope: this browser · saved to your account',
      language: 'Scope: the whole interface · takes effect on save',
      privacy: 'Scope: what colleagues see on your directory card',
      work: 'Scope: your own time records and landing page'
    },
    note: {
      email: 'Delivered to {email}',
      desktop: 'The browser asks for permission the first time this is turned on',
      frequency: 'Real-time delivers as events happen; a digest batches them into one message',
      timeTracking: 'Approvals, missing punches and proof requests',
      performance: 'Review cycles, goals and self-assessments',
      employee: 'Joiners, leavers and directory changes',
      recruitment: 'New applications and stage changes',
      system: 'Maintenance windows and release notes',
      theme: 'System follows the operating system setting',
      dateFormat: 'Used by every date on screen and in exports',
      timeFormat: 'Applies to the clock, the ledgers and the punch history',
      itemsPerPage: 'Every table in the app pages at this size',
      language: 'Text without a translation falls back to English',
      timezone: 'Every timestamp on screen is rendered in this zone',
      visibility: 'Managers and admins always keep access for HR purposes',
      showEmail: 'Turning this off hides it from the directory, not from HR',
      showPhone: 'Turning this off hides it from the directory, not from HR',
      dashboard: 'The view the Organisation Overview opens in',
      autoClockOutTime: 'An open punch left running past this time is closed at it'
    }
  },
  // Control Panel
  controlPanel: {
    demoModeDisabled: 'Function is locked',
    title: 'Control Panel',
    subtitle: 'Manage your account and system settings',
    role: 'Role',
    userUuid: 'User UUID',
    employeeId: 'Employee ID',
    changePassword: 'Change Password',
    readManual: 'Read Manual',
    logout: 'Log Out',
    confirmLogout: 'Are you sure you want to log out?',
    allFieldsRequired: 'All fields are required',
    passwordTooShort: 'Password must be at least 6 characters',
    passwordsDontMatch: 'Passwords do not match',
    passwordChanged: 'Password changed successfully!',
    passwordChangeError: 'Error changing password',
    currentPassword: 'Current Password',
    newPassword: 'New Password',
    confirmPassword: 'Confirm Password',
    needHelp: 'Need Help?',
    helpText: 'Check out the manual for detailed instructions on using the system.',
    standardAccess: 'Standard user access',
    roles: {
      admin: 'Admin',
      hrManager: 'HR Manager',
      manager: 'Manager',
      employee: 'Employee',
      viewer: 'Viewer',
      demoAdmin: 'Demo Admin',
      hrAdmin: 'HR Admin'
    },
    roleDesc: {
      admin: 'Full system access with all administrative privileges including user management, system settings, and complete data control',
      hrAdmin: 'Complete HR management with employee administration, payroll access, and system configuration',
      hrManager: 'Manage employees, performance reviews, recruitment, and HR operations',
      manager: 'Supervise team members, approve time tracking, and manage performance reviews',
      employee: 'Access personal information, submit time entries, and view own performance data',
      viewer: 'View-only access to reports, dashboards, and limited system information',
      demoAdmin: 'Demo mode uses localstorage to simulate admin access, official version uses backend databse to sync data'
    },
    uploadAvatar: 'Upload avatar',
    avatarUpdated: 'Avatar updated successfully!',
    avatarError: 'Error uploading avatar',
    resetUserPassword: 'Reset User Password',
    selectUser: 'Select User',
    chooseUser: '-- Choose a user --',
    enterNewPassword: 'Enter new password',
    confirmNewPassword: 'Confirm new password',
    resetPassword: 'Reset Password',
    selectUserFirst: 'Please select a user first',
    userNotFound: 'User not found',
    confirmResetPassword: 'Are you sure you want to reset password for',
    passwordResetSuccess: 'Password reset successfully for',
    passwordResetError: 'Error resetting password. You may need admin service role access.',
    errorFetchingUsers: 'Error loading users',
    warning: 'Warning',
    adminResetWarning: 'This will change the password for the selected user. They will need to use the new password to log in.',
    resetEmployeePassword: 'Reset Employee Password',
    selectEmployee: 'Select Employee',
    chooseEmployee: '-- Choose an employee --',
    employeeNotFound: 'Employee not found',
    confirmResetEmployeePassword: 'Are you sure you want to reset password for employee',
    passwordResetSuccessEmployee: 'Password reset successfully for employee',
    errorFetchingEmployees: 'Error loading employees',
    noEmployeesFound: 'No employees found',
    manageEmails: 'Manage User Emails',
    visitAnalytics: 'Visit analytics',
    refreshVisitAnalytics: 'Refresh visit analytics',
    refresh: "Refresh",
    refreshing: "Refreshing...",
    visit: {
      total: 'Total',
      last24h: 'Last 24h',
      distinctIps: 'Distinct IPs',
      noVisits: 'No visits logged yet.',
      unknownIp: 'unknown IP',
      ref: 'Ref',
      demoCount: 'Demo Counts',
      authorized: 'Authorised Sessions'
    },
    changeOwnPassword: "Change Own Password",
    resetOtherUserPassword: "Reset Other Employee Password",
    resetEmployeeTablePassword: 'Reset Employee Table Password',
    restoreDemoData: 'Restore Demo Data',
    restoreDemoDataDescription: 'Restore default demo data for specific data types. This will reset any changes you made.',
    demoTimeEntries: 'Time Entries',
    demoGoals: 'Goals',
    demoTasks: 'Tasks',
    demoReviews: 'Reviews',
    demoSkills: 'Skills',
    demoLeaveRequests: 'Leave Requests',
    demoDataRestored: '{type} restored to defaults',
    allDemoDataRestored: 'All demo data restored to defaults',
    restoreAllDemoData: 'Restore All Demo Data',
    switchToPrefix: 'Switch to',
    demoAdminLabel: 'Demo Admin',
    demoEmployeeLabel: 'Demo Employee',
    switchToDemoEmployee: 'Switch to Demo Employee',
    switchToDemoAdmin: 'Switch to Demo Admin',
    demoRoleOnly: 'This feature is only available in demo mode',
    // Industry-system Control Panel: scope and consequence lines.
    mode: 'Mode',
    modeDemo: 'Demo',
    modeLive: 'Live',
    demoSession: 'Demo session — actions are simulated',
    liveSession: 'Live session',
    signedInAs: 'Signed in as',
    identity: 'Identity',
    identityScope: 'Scope: this sign-in only',
    credentials: 'Credentials',
    credentialsScope: 'Scope: your sign-in only',
    credentialsScopeAdmin: 'Scope: your sign-in · plus any account you administer',
    change: 'Change',
    ownPasswordNote: 'Takes effect immediately; this device stays signed in',
    demoLockedNote: 'Locked in demo mode — nothing is written to the database',
    userPasswordNote: '{n} accounts in scope — they sign in with the new password immediately',
    userPasswordNoteIdle: 'Signs the chosen account out of every device it is using',
    employeePasswordNote: '{n} employee records carry a sign-in of their own',
    employeePasswordNoteIdle: 'Only employee records linked to a user account can be reset',
    togglePassword: 'Show or hide password',
    visitScope: 'Scope: every session that reached the app',
    session: 'Session',
    demoSessionNote: 'Demo data lives in this browser. Nothing here reaches the database.',
    liveSessionNote: 'These actions leave the screen. Everything else above is yours alone.',
    switchRole: 'Switch role',
    endSession: 'End session',
    logoutNote: 'Signs this device out. Unsaved forms above are discarded.'
  },
  // Policy Controls — settings as a spec sheet. Every rule carries a value, a
  // scope and a consequence; nothing commits until the single publish button.
  policyControls: {
    title: 'Policy Controls',
    version: 'Policy version',
    lastEditedBy: 'last edited by',
    appliesTo: 'applies to {n} employees',
    versionHistory: 'Version history',
    publishN: 'Publish {n} changes',
    nothingToPublish: 'Nothing to publish',
    sections: 'Policy sections',
    policy: 'Policy',
    users: 'Users',
    roles: 'Roles',
    pendingChanges: 'Pending changes',
    lastEdit: 'Last edit',
    organisation: 'Organisation',
    nItems: '{n} items',
    nothingUntilPublished: 'Nothing takes effect until published',
    queueEmpty: 'The published policy and the draft agree. Edit a rule to open a change.',
    approve: 'Approve',
    decline: 'Decline',
    accepted: 'Accepted',
    revert: 'Revert',
    queued: 'Queued',
    auditTrail: 'Audit trail',
    on: 'On',
    off: 'Off',
    affects: 'Affects {n}',
    lowRisk: 'Low risk',
    yourEdit: 'Your edit',
    yourEditReason: 'Edited on this screen — it takes effect when you publish.',
    you: 'You',
    openCases: 'open the cases',
    weekday: 'Weekday',
    weekend: 'Weekend',
    holiday: 'Holiday',
    thirdWorkingDay: '3rd working day',
    elevenDays: '11 days',
    file: 'File',
    encoding: 'Encoding',
    item: {
      otCap: 'Overtime cap',
      otCapReason: 'Requested by Trần Thị Lan Anh — the Sơn Trà line is running six-day weeks until the September order ships.',
      lateGrace: 'Late grace period',
      lateGraceReason: 'Requested by Đỗ Bảo Long — the 07:30 shuttle arrives late on rainy mornings.',
      teamLead: 'Role · Team lead',
      teamLeadGain: 'Gains "Approve overtime" · 9 people'
    },
    sec: {
      workingTime: 'Working time',
      overtime: 'Overtime & pay',
      leave: 'Leave policy',
      roles: 'Roles & access',
      cycles: 'Review cycles',
      notifications: 'Notifications',
      data: 'Data & retention'
    },
    scope: {
      allDepartments: 'Scope: all departments',
      overtime: 'Scope: all departments · 1 override on Engineering',
      leave: 'Scope: all departments · statutory minimum applies',
      roles: 'Scope: 6 roles · 248 employees',
      cycles: 'Scope: all departments · H1 2026 open',
      notifications: 'Scope: all employees · email and in-app',
      data: 'Scope: all records · Vietnam data residency'
    },
    row: {
      standardDay: 'Standard working day',
      standardDayNote: 'Anything past it is overtime — 41 people crossed it yesterday',
      workingDays: 'Working days per month',
      workingDaysNote: 'Divides every monthly figure on the dashboard — July 2026 resolves to 26',
      coreHours: 'Core hours',
      coreHoursNote: 'Meetings outside it need an override — 3 were booked this week',
      lateGrace: 'Late grace',
      lateGraceNote: '7 people used it this week',
      autoClose: 'Auto-close punches',
      autoCloseNote: 'Closed 14 forgotten punches at midnight last month',
      weekendWork: 'Weekend counts as work',
      weekendWorkNote: 'Turning it on moves the month from 26 to 30 working days',
      otCap: 'Monthly overtime cap',
      otCapNote: '3 people are over it this month',
      otApproval: 'Overtime requires approval',
      otApprovalNote: '23 managers hold the decision · 41 requests cleared this month',
      multipliers: 'Overtime multipliers',
      multipliersNote: 'Set by contract — payroll reads these, this screen only reports them',
      payPeriod: 'Pay period closes',
      payPeriodNote: 'Entries filed after it land in the following month — 6 did in July',
      annualLeave: 'Annual leave entitlement',
      annualLeaveNote: '9 people have already spent all of it this year',
      carryOver: 'Carry-over cap',
      carryOverNote: '12 people will lose days on 31 December at this cap',
      leaveNotice: 'Notice required',
      leaveNoticeNote: '4 requests bypassed it this quarter',
      sickNote: 'Sick note required after',
      sickNoteNote: '17 notes were filed against this rule this year',
      holidays: 'Public holidays',
      holidaysNote: 'Vietnam statutory calendar — 11 days, none in August',
      rolesInUse: 'Roles in use',
      rolesInUseNote: 'Admin · HR · Manager · Team lead · Employee · Viewer',
      managersLeave: 'Managers can approve leave',
      managersLeaveNote: '23 managers hold it · 41 approvals this month',
      teamLeadOt: 'Team leads can approve overtime',
      teamLeadOtNote: '9 people would gain it — queued for publication',
      session: 'Session length',
      sessionNote: '148 sessions were cut short at this length last month',
      twoFactor: 'Two-factor for admins',
      twoFactorNote: '4 admin accounts are enrolled; turning it off unenrols them all',
      digest: 'Daily digest',
      digestNote: '231 people receive it; 17 have opted out',
      punchReminder: 'Punch reminder after',
      punchReminderNote: 'Sent 34 times this week',
      escalate: 'Escalate unanswered requests after',
      escalateNote: '6 requests escalated to a department head this month',
      weekendNotify: 'Weekend notifications',
      weekendNotifyNote: 'Turning it on reaches 248 people on a rest day',
      timeEntries: 'Time entries kept for',
      timeEntriesNote: 'The oldest entry on file is from March 2023 — shortening this deletes it',
      leaveRecords: 'Leave records kept for',
      leaveRecordsNote: 'Labour code requires 60 months — below that the export stops being audit-safe',
      auditLog: 'Audit log kept for',
      auditLogNote: '4,812 entries are stored under this window',
      anonymise: 'Anonymise leavers',
      anonymiseNote: '3 leavers fall due next month once this runs',
      exportFormat: 'Export format',
      exportFormatNote: 'Payroll reads this file every 3rd working day'
    }
  },
  prodHelp: {
    title: "Production Help Center",
    highlight: "Production",
    subtitle: "Production Tips & Playbooks",
    lede: "A concise set of production-only tips: shipping safely, keeping the lights on, and reacting fast when things go sideways.",
    checklist: "Operator Checklist",
    cards: "Production Guides",
    item: {
      observability: "Dashboards & Alerts Active",
      rollbacks: "Rollback Plan Tested",
      backups: "Backups Verified"
    },
    observability: {
      title: "Observability: Logs, Metrics, Traces",
      description: "Ensure error budgets, structured logs, 95th/99th percentile latency dashboards, and trace sampling for production."
    },
    rollouts: {
      title: "Safe Rollouts & Feature Flags",
      description: "Use gradual rollouts with health checks, rollback playbooks, and flags to quickly disable risky code paths."
    },
    security: {
      title: "Security & Secret Hygiene",
      description: "Rotate keys, enforce least privilege, enable audit logs, and require MFA for admin roles."
    },
    performance: {
      title: "Performance Budgets",
      description: "Monitor cold start and hot path times. Set budgets for API latency, SQL queries, and frontend TTI."
    },
    resilience: {
      title: "Backups & Resilience",
      description: "Verify automated backups, DR strategy, rate limiting, and circuit breakers for downstream dependencies."
    },
    tags: {
      reliability: "Reliability",
      metrics: "Metrics",
      release: "Release",
      safety: "Safety",
      security: "Security",
      performance: "Performance",
      resilience: "Resilience"
    }
  },
  // Help Center / Advanced Guide
  help: {
    containerLabel: 'Help Center',
    title: 'Advanced Feature Guide',
    subtitle: 'Walkthrough of key features and pro tips',
    videoSection: 'Video Section',
    videos: {
      title: 'Video Tutorials',
      subtitle: 'Learn through short guides and advanced tips.'
    },
    demoRestrictions: 'Demo Restrictions',
    demo: {
      title: 'IMPORTANT: Help Center!',
      body: 'DATA IS SAVED LOCALLY - Does Not Sync Across Devices. Data Can Be Restored In Control Panel.'
    },
    features: {
      section: 'Advanced & Contextual Features',
      defaultTitle: 'Hidden Features & Pro Tips',
      contextTitle: 'Contextual Tips for {context}',
      restricted: 'RESTRICTED',
      empty: 'No specific tips found for context: "{context}". All features are shown below.',
      feature_1: {
        title: 'Hidden Feature: Batch Editing',
        description: 'Did you know you can edit up to 50 records at once? Select the checkboxes next to the entries and click the edit icon at the top of the table. This is disabled in the current demo.'
      },
      feature_2: {
        title: 'Contextual Tip: Quick Filter',
        description: 'Use the magnifying glass icon next to a column header to instantly filter by the unique values in that column. No need to open the full filter panel!'
      },
      feature_3: {
        title: 'Demo Restriction: Data Persistence',
        description: 'All changes you make (create, edit, delete) will be lost when refreshing the page. This is an important restriction in the demo environment.'
      },
      feature_4: {
        title: 'Hard-to-Find: User Preferences',
        description: 'Access your dashboard layout settings by clicking your profile avatar in the top right. You can enable dark mode and adjust table densities.'
      }
    },
    tags: {
      advanced: 'Advanced',
      hidden: 'Hidden',
      demorestricted: 'Demo Restricted',
      contextual: 'Contextual',
      quickaccess: 'Quick Access',
      restriction: 'Restriction',
      demo: 'Demo',
      'hard-to-find': 'Hard to Find',
      personalization: 'Personalization'
    },
    showcase: {
      section: 'Animated product walkthrough',
      label: 'Animated product walkthrough',
      badge: 'Live app walkthrough',
      title: 'See the Platform in Motion',
      subtitle: 'Follow the flow from time capture to reporting, with data moving across modules in real time.',
      control: 'Control panel',
      sync: 'Everything stays in sync',
      syncDesc: 'Entries, files, and goals update dashboards and reports instantly.',
      live: 'Live',
      time: {
        title: 'Time management',
        desc: 'Track time entries, approvals, overtime, and worklogs in one place.'
      },
      upload: {
        title: 'Upload files',
        desc: 'Securely store contracts, IDs, and evidence with audit-ready history.'
      },
      dashboard: {
        title: 'Dashboard & charts',
        desc: 'Visualize utilization, attendance, and health scores in real time.'
      },
      tasks: {
        title: 'Tasks & goals',
        desc: 'Assign owners, due dates, and checklists that sync with people data.'
      },
      reporting: {
        title: 'Reporting',
        desc: 'Export PDFs/CSV and schedule recurring digests for managers.'
      }
    }
  },
  // Email Management
  emailManagement: {
    title: 'User Email Management',
    description: 'Manage multiple email addresses for each user. Each email can have its own password but authenticates as the same user.',
    addEmail: 'Add Email',
    addEmailFor: 'Add Email for',
    emailAddress: 'Email Address',
    authUserId: 'Auth User ID',
    authUserIdHelp: 'Get this from Supabase Auth dashboard after creating the account',
    linkEmail: 'Link Email',
    unlink: 'Unlink',
    primary: 'Primary',
    setPrimary: 'Set as primary',
    confirmUnlink: 'Are you sure you want to unlink {{email}}?',
    cannotRemoveLastEmail: 'Cannot remove the last email',
    emailLinked: 'Email linked successfully',
    emailUnlinked: 'Email unlinked successfully',
    primaryEmailSet: 'Primary email updated successfully',
    errorLinking: 'Error linking email',
    errorUnlinking: 'Error unlinking email',
    errorSettingPrimary: 'Error setting primary email',
    errorLoadingUsers: 'Error loading users',
    fillAllFields: 'Please fill all fields'
  },
  // Task Listing
  taskListing: {
    title: 'Task Listing',
    assignedBy: 'Assigned By',
    assignDate: 'Assign Date',
    filterBy: 'Filter By',
    allStatuses: 'All Statuses',
    allPriorities: 'All Priorities',
    viewTask: 'View Task',
    taskDetails: 'Task Details',
    team: 'Team',
    individual: 'Individual',
    teamWorkload: 'Team Workload',
    completedTasks: 'Completed Tasks',
    pendingTasks: 'Pending Tasks',
    overdueTasks: 'Overdue Tasks',
    searchEmployee: 'Search employee...',
    noEmployees: 'No employees found',
    employee: 'Employee',
    tasksAssigned: 'Tasks Assigned',
    tasksCompleted: 'Tasks Completed',
    filterByEmployee: 'Filter by Employee',
    filterByStatus: 'Filter by Status',
    searchTasks: "Search Tasks...",
    organization: 'Organization',
    totalTasks: "Total Tasks",
    progress: "Progress",
    avgQuality: "Avg Quality",
    avgProgress: "Avg Progress",
    myTasks: 'My Tasks',
    manageTasks: "Manage Tasks",
    addTask: "Add Task",
    assignTask: "Assign Task",
    editTask: "Edit Task",
    taskTitle: "Task Title",
    description: "Description",
    dueDate: "Due Date",
    startDate: "Start Date",
    priority: "Priority",
    priorityLow: "Low",
    priorityMedium: "Medium",
    priorityHigh: "High",
    low: 'Low',
    medium: 'Medium',
    high: 'High',
    status: "Status",
    statusPending: "Pending",
    statusInProgress: "In Progress",
    statusCompleted: "Completed",
    pending: 'Pending',
    'in-progress': 'In Progress',
    completed: "Completed",
    selfAssessment: 'Self Assessment',
    selfAssessmentPlaceholder: 'How did you perform on this task?',
    qualityRating: 'Quality Rating',
    noTasks: 'No tasks yet. Add your first task!',
    employeeProgress: 'Employee Progress',
    confirmDelete: 'Are you sure you want to delete this task?',
    assignTo: "Assign To",
    selectEmployee: "Select Employee",
    completionBeforeStart: 'Completion date cannot be before start date',
    tasks: 'Tasks',
    quality: 'Quality',
    completionDate: 'Completion Date',
    createdAt: 'Created At',
    updatedAt: 'Updated At',

    // ── Task ledger / task detail (Industry redesign) ──────────────
    titleRequired: 'Task title is required',
    taskCreated: 'Task created',
    taskCreateError: 'Failed to create task',
    taskUpdated: 'Task updated',
    taskUpdateError: 'Failed to update task',
    taskDeleted: 'Task deleted',
    taskDeleteError: 'Failed to delete task',
    newTask: 'New task',
    export: 'Export',
    exported: '{n} rows exported',
    loadFailed: 'Failed to load tasks',
    notYours: 'This task belongs to someone else',
    markComplete: 'Mark complete',
    markedComplete: 'Task marked complete',
    reopen: 'Reopen',
    reopened: 'Task reopened',
    openTask: 'Open',
    assignedTo: 'Assigned To',
    noMatches: 'Nothing matches that search',

    // Ticker
    open: 'Open',
    dueToday: 'Due today',
    overdue: 'Overdue',
    unscheduled: 'No due date',
    closedOnTime: 'Closed on time',
    closedOnTimeHint: 'Of the closed tasks that had a due date',
    task: 'Task',
    state: 'State',
    recordOpen: 'Record open',
    grouping: 'Grouping',
    byRecord: 'By record',
    byDue: 'By due window',
    byPriority: 'By priority',

    // Title row
    headOpen: '{n} open of {total} task(s) across {r} employee record(s)',
    noneInSegment: 'No tasks in this view — the scope holds {n}, all in other states',
    showClosedN: 'Show the {n} closed',
    clearSearch: 'Clear the search',
    unlinkedViewer: 'This sign-in is not linked to an employee record, so there is no record to list tasks against. An administrator can link it from User Management.',
    headModel: 'every one was opened on a record, none are free-standing',
    headMine: 'you own {n}',

    // Filter strip
    segAllOpen: 'All open',
    segMine: 'Mine',
    segOverdue: 'Overdue',
    segClosed: 'Closed',
    searchPlaceholder: 'Search tasks, owners or the record that opened them',
    groupRecord: 'Grouped by record',
    groupDue: 'Grouped by due window',
    groupPriority: 'Grouped by priority',
    change: 'Change',

    // Table
    colTask: 'Task',
    colSource: 'Source',
    colOwner: 'Owner',
    colDue: 'Due',
    colState: 'State',
    done: 'Done',
    emittedBy: 'every task below was opened on this employee record',
    nMoreOnRecord: '{n} more on this record',
    showingRows: 'Showing {shown} of {total} · {groups} of {allGroups} group(s)',
    showAll: 'Show everything',
    collapse: 'Collapse',

    // Group bands
    bandOverdue: 'Past due',
    bandOverdueNote: 'the due date has already gone by',
    bandToday: 'Due today',
    bandTodayNote: 'closes today or joins the overdue count tomorrow',
    bandWeek: 'Next seven days',
    bandWeekNote: 'inside the week — still schedulable',
    bandLater: 'Later',
    bandLaterNote: 'beyond the week',
    bandNoDate: 'No due date',
    bandNoDateNote: 'nothing can chase these until a date is set',
    bandHighNote: 'chased first by the decision column',
    bandMediumNote: 'normal queue',
    bandLowNote: 'picked up when there is room',

    // States
    stateOpen: 'Open',
    stateInProgress: 'In progress',
    stateDone: 'Done',
    stateCancelled: 'Cancelled',
    stateOverdue: 'Overdue',
    stateUnscheduled: 'No due date',

    // Source
    sourceSelf: 'Self-set',
    sourceAssigned: 'Assigned',
    sourceSystem: 'System',
    selfSetValue: 'The owner',
    unknownOwner: 'Unknown record',

    // Row second line
    byWho: 'by {who}',
    bySelf: 'by the owner',
    openedOn: 'opened {date} {by}',
    openedUnknown: 'opening date not recorded',
    closedOn: 'Closed {date}',
    onTime: 'on time',
    lateClose: 'closed after the due date',
    ratedN: 'rated {n}/5',
    notRated: 'not rated — the record keeps no quality figure for it',
    cancelledLine: 'Cancelled',
    oneDayLate: '1 day past due',
    nDaysLate: '{n} days past due',
    noDueLine: 'No due date — it cannot be chased and it never reaches this ledger’s overdue count',
    dueTodayLine: 'Due today',
    dueInDays: 'Due in {n} days',
    assessmentLogged: 'self-assessment logged',

    // Due cell
    noDue: 'No date',
    today: 'Today',
    inNDays: 'In {n} d',
    oneDayLateShort: '1 day late',
    nDaysLateShort: '{n} days late',

    // Decision column
    needsDecision: 'Needs a decision',
    nItems: '{n} items',
    decisionSummary: '{o} overdue · {u} with no due date to chase them by',
    nothingToDecide: 'Nothing is open. Every task on the visible records is closed.',
    costOverdue: 'It has been past due for {n} day(s) and is the oldest open item on this record.',
    costToday: 'It is due today; tomorrow it joins the overdue count.',
    costNoDate: 'It carries no due date, so nothing will chase it.',
    costSoon: 'Due in {n} day(s), and marked {p} priority.',
    costContext: '{owner} has {n} other open task(s) on the same record.',
    yourDay: 'Your day',
    yourDayCount: '{n} open · {d} due today',
    yourDayEmpty: 'Nothing is assigned to you right now.',
    closedAt: 'Closed {date}',

    // Detail — title row and blocker
    owner: 'owner',
    dueLower: 'due',
    moveDueDate: 'Move due date',
    setDueDate: 'Set a due date',
    blockerOverdue: 'It was due {date}. Until it closes it stays in the overdue count on this record, alongside {n} other open task(s).',
    blockerNoDateHead: 'No due date — it cannot be chased',
    blockerNoDate: 'Nothing schedules, escalates or reports on a task without a date. Set one, or close it if it is already done.',

    // Detail — 01 Steps
    panelSteps: 'Steps',
    stepsNote: '{done} of {total} done · {left} to go',
    waiting: 'Waiting',
    stepOpened: 'Task opened on the record',
    stepOpenedDetail: 'Emitted by the {who} record · {src}',
    srcSelfSet: 'set by the owner',
    srcAssignedBy: 'assigned by {creator}',
    stepScheduled: 'Due date set',
    stepScheduledDone: 'Due {date} · everything below is measured against it',
    stepScheduledOpen: 'Without a date this task cannot be chased, counted as overdue, or reported on',
    stepStarted: 'Work started',
    stepStartedDone: 'The owner moved it off Pending',
    stepStartedOpen: 'Follows the due date · the owner moves it to In progress',
    stepClosed: 'Marked complete',
    stepClosedDone: 'Closed {date}',
    stepClosedOpen: 'Follows the work · closing it is what clears the record',
    stepAssessed: 'Assessed and rated',
    stepAssessedDone: 'Rated {n}/5 · feeds the quality average on this record',
    stepAssessedOpen: 'Follows completion · until it exists the record carries no quality figure for this task',

    // Detail — 02 Chain
    panelChain: 'Chain',
    relOpenedBy: 'Opened by',
    relBelongsTo: 'Belongs to',
    relSitsWith: 'Sits with',
    recordHolds: 'This record holds {n} task(s) in total',
    nOpenTasks: '{n} open task(s) on the same record',
    nOfThemOverdue: '{n} of them already overdue — open the record',
    noneOverdue: 'none of them overdue — open the record',

    // Detail — 03 Elapsed
    panelElapsed: 'Elapsed',
    againstWindow: 'against the {n} day window the due date allows',
    noWindow: 'no start date — there is no window to measure against',
    measuredToClose: 'measured to the close stamp',
    measuredToToday: 'measured to today',
    withinWindow: 'Inside the window',
    pastDue: 'Past the due date',
    openedOnDate: 'Opened {date}',

    // Detail — right column
    record: 'Record',
    recordNote: 'Everything this task carries',
    recordField: 'Record',
    setOnClose: 'Set when the task closes',
    priorityHighNote: 'Chased first',
    priorityMediumNote: 'Normal queue',
    priorityLowNote: 'When there is room',
    activity: 'Activity',
    actRated: 'Quality rated {n}/5',
    actClosed: 'Marked complete',
    actChanged: 'Last changed · now {state}',
    actAssessed: 'Self-assessment written',
    actOpened: 'Task opened on the {who} record',
    notes: 'Notes',
    addNote: 'Add a note or a decision…',
    noteAdded: 'Note added'
  },
  // Task Review
  taskReview: {
    title: 'Performance Reviews',
    notRated: 'Not Rated',
    subtitle: 'Review cycle progress, score distribution and sign-offs',
    individual: 'Individual',
    team: 'Team',
    review: 'Review',
    employeeTasks: 'Employee Tasks',
    teamTasks: 'Team Tasks',
    reviewTask: 'Review Task',
    description: 'Description',
    qualityRatingStars: 'Quality Rating (1-5 stars)',
    managerEvaluationComments: 'Manager Evaluation Comments',
    submitReview: 'Submit Review',
    updateStatus: 'Update Status',
    feedbackPlaceholder: 'Provide feedback on task progress and quality...',
    total: 'Total',
    month: 'Month',
    selectMonth: "Select Month",
    employee: 'Employee',
    totalTasks: 'Total Tasks',
    completed: 'Completed',
    inProgress: 'In Progress',
    overdue: 'Overdue',
    completionRate: 'Completion Rate',
    avgQuality: 'Avg Quality',
    onTimePerformance: 'On-Time Performance',
    onTimeCompletions: 'On-Time Completions',
    onTimeRate: 'on-time rate',
    priorityDistribution: 'Priority Distribution',
    highPriority: 'High Priority',
    mediumPriority: 'Medium Priority',
    lowPriority: 'Low Priority',
    qualityAssessment: 'Quality Assessment',
    ratedTasks: 'Rated Tasks',
    noTasks: 'No tasks found for this month',
    noTeamTasks: 'No team tasks found for this month',
    evaluateTask: 'Evaluate Task',
    selfAssessment: 'Self Assessment',
    qualityRating: 'Quality Rating',
    managerComments: 'Manager Comments',
    commentPlaceholder: 'Provide feedback on task quality and personalGoals...',
    taskDetails: 'Task Details',
    currentRating: 'Current Rating',
    submitEvaluation: 'Submit Evaluation',
    cancel: 'Cancel',
    evaluate: 'Evaluate',
    managerEvaluation: 'Manager Evaluation',
    created: 'Created',
    due: 'Due',
    all: 'All',
    pending: 'Pending',
    createGoal: 'Create Goal',
    createGoalSubtitle: 'Set a new goal for an employee',
    createPersonalGoal: 'Set a new personal goal',
    assignTo: 'Assign To',
    goalTitle: 'Goal Title',
    goalTitlePlaceholder: 'Enter goal title...',
    goalDescription: 'Description',
    goalDescriptionPlaceholder: 'Describe the goal in detail...',
    goalPermissionAdmin: 'As an admin/manager, you can create goals for any employee',
    goalPermissionEmployee: 'You can only create goals for yourself',
    high: 'High',
    medium: 'Medium',
    low: 'Low',
    employees: 'Employees',
    completion: 'Completion',
    quality: 'Quality',
    task: 'Task',
    progress: 'Progress',
    organization: 'Organization',
    chooseEmployee: 'Choose Employee',
    selectEmployeeToView: 'Select an employee to view',
    tasks: 'Tasks',
    assignedTo: 'Assigned To',
    assignedBy: 'Assigned By',
    dueDate: "Due Date",
    status: "Status",
    priority: "Priority",
    actions: 'Actions',
    noTasksFound: 'No tasks found',
    selectEmployee: "Select Employee",
    taskList: 'Task List',
    employeeSelfAssessment: 'Employee Self Assessment',
    employeeBreakdown: 'Employee Breakdown',
    expandAll: 'Expand All',
    collapseAll: 'Collapse All',
    noActiveTasks: 'No active tasks for this employee.',
    category: 'Category',
    targetDate: 'Target Date',
    notes: 'Notes',
    createdAt: 'Created At',
    updatedAt: 'Updated At',
    // Review cycle console
    cycle: 'Cycle',
    company: 'Company',
    scope: 'Scope',
    inScope: 'In scope',
    inScopeCount: '{n} employees in scope',
    signedOff: 'Signed off',
    signedOffThisWeek: 'Signed off in the last 7 days',
    avgScore: 'Avg score',
    calibration: 'calibration',
    calibrationLabel: 'Calibration',
    daysOut: '{n} days out',
    daysPast: '{n} days past',
    cycleCompletion: 'Cycle completion',
    ofSignedOff: '{n} of {total} signed off',
    thisWeek: 'this week',
    projectionDone: 'Every review in scope is signed off.',
    projectionRemaining: '{n} still to sign off before calibration on {date}.',
    projectionRate: 'At this rate the cycle closes',
    beforeCalibration: '{n} days before calibration.',
    afterCalibration: '{n} days after calibration.',
    stageSelf: 'Self-assessment',
    stageManager: 'Manager review',
    stageCalibration: 'Calibration',
    stageSignedOff: 'Signed off',
    complete: 'complete',
    nOutstanding: '{n} outstanding',
    nNotSubmitted: '{n} not submitted',
    nOverdue: '{n} overdue',
    nToFile: '{n} to file',
    session: 'session',
    nWaitOnYou: '{n} wait on you',
    noneWaitOnYou: 'none wait on you',
    scoreDistribution: 'Score distribution',
    nReviews: '{n} reviews',
    distributionCaption: 'The {avg} average peaks at {peak} — {pct}% of reviews sit at or above it',
    distributionEmptyCaption: 'No scored reviews in this cycle yet',
    noScores: 'Nothing has been scored in this cycle yet.',
    averageByDepartment: 'Average by department',
    scaleRange: 'scale {from} – {to}',
    departmentCaption: 'The vertical rule is the org average, {avg} — {n} department(s) sit under it',
    departmentEmptyCaption: 'Department averages appear once reviews are scored',
    noDepartmentScores: 'No department has a scored review in this cycle.',
    awaitingSignOff: 'Awaiting your sign-off',
    oldestWaited: 'Oldest has waited {n} day(s)',
    calibrationCloses: 'calibration closes {date}',
    nothingAwaiting: 'Nothing is waiting on your signature for this cycle.',
    sinceLastCycle: 'since',
    scoreUnchanged: 'score unchanged',
    flaggedForPromotion: 'flagged for promotion',
    signOff: 'Sign off',
    sendBack: 'Send back',
    waitedDays: 'waited {n} day(s)',
    overdueManagerReviews: 'Overdue manager reviews',
    noOverdueReviews: 'Every manager review is still inside its deadline.',
    nLate: '{n} late',
    toBeReviewedBy: 'To be reviewed by {name}',
    remindAll: 'Remind all {n}',
    remindSelfAll: 'Remind {n} to self-assess',
    remindNoAccounts: 'None of those managers has an account to notify.',
    remindNoEmployeeAccounts: 'None of those employees has an account to notify.',
    reminderTitle: 'Manager reviews outstanding',
    reminderBody: '{n} review(s) for {dept} are past their deadline for {cycle}.',
    remindedMessage: 'Reminded {n} manager(s).',
    selfReminderTitle: 'Self-assessment outstanding',
    selfReminderBody: 'Please submit your self-assessment for {cycle}.',
    remindedEmployeesMessage: 'Reminded {n} employee(s).',
    skippedSelfMessage: '{name}: continued without a self-assessment.',
    skippedSelfNote: 'This cycle is continuing without a self-assessment.',
    continueWithoutSelf: 'Continue without self-assessment',
    signedOffMessage: '{name} signed off.',
    sentBackMessage: '{name} sent back for revision.',
    overallRating: 'Overall',
    strengths: 'Strengths',
    areasForImprovement: 'Areas for improvement',
    achievements: 'Achievements',
    noWrittenReview: 'No written feedback was recorded on this review.',
    viewPending: 'View pending',
    pendingReviews: 'Pending reviews',
    pendingCaption: '{n} still to complete this cycle',
    nothingPending: 'Every review in this cycle is signed off.',
    nPeople: '{n} people',
    statusSelfOutstanding: 'Self-assessment outstanding',
    statusSelfSkipped: 'Self-assessment not submitted',
    statusManagerToFile: 'Manager review to file',
    statusAwaitingCalibration: 'Awaiting calibration',
    selfOutstandingEmpty: 'Every self-assessment in this cycle is in.',
    managerToFileEmpty: 'Every manager review in this cycle is filed.',
    calibrationEmpty: 'Every scored review has reached calibration.',
    reviewNotFiled: 'This review has not been filed yet.',
    openPersonalGoals: 'Open Personal Goals',
    enterManagerRatings: 'Enter manager ratings',
    submitForCalibration: 'Submit for calibration',
    submittedForCalibrationMessage: '{name} submitted for calibration.',
    readyForCalibration: 'This review is ready to send to calibration.',
    competency: {
      technical_skills_rating: 'Technical',
      communication_rating: 'Communication',
      leadership_rating: 'Leadership',
      teamwork_rating: 'Teamwork',
      problem_solving_rating: 'Problem solving'
    }
  },
  // 3d — the live punch clock screen
  punchClock: {
    title: 'Punch clock',
    shift: 'shift',
    scope: 'Scope',
    scopeMine: 'Mine',
    scopeFloor: 'Whole floor',
    sync: 'Sync',
    // ticker
    onTheFloor: 'On the floor',
    ofHeadcount: '{n} of {total} employees',
    onBreak: 'On break',
    hoursToday: 'Hours today',
    lateToday: 'Late',
    unclosed: 'Unclosed',
    // hero
    countingFrom: 'Counting from {time}',
    notPunchedIn: 'Not punched in',
    onClock: 'on the clock {n}',
    breakTotal: 'break {n}',
    readyToPunch: 'nothing recorded yet today',
    regularHours: 'Regular hours',
    overtimeAfter: 'Overtime after {time}',
    startBreak: 'Start break',
    endBreak: 'End break',
    punchIn: 'Punch in',
    punchOut: 'Punch out',
    consequenceIn: 'Punching in starts the count against your {time} shift',
    consequencePlain: 'Punching out now files {n} for approval',
    consequenceOvertime: 'Punching out now records {n} overtime',
    punchedIn: 'Punched in at {time}',
    punchedOut: 'Punched out. {n} filed for approval.',
    noEmployee: 'This account is not linked to an employee record.',
    filedLive: 'Live punch',
    filedWithBreak: 'Live punch · {n} break',
    dayStripLabel: 'Today from 08:00 to 19:00',
    // week
    thisWeek: 'This week',
    ofContract: 'of {n}h',
    weekMine: 'Your hours',
    weekFloor: 'Averaged per person who worked',
    weekCaption: 'the rule is the {n}h contract day',
    contractDay: 'Contract day, 8h',
    dayMon: 'MON',
    dayTue: 'TUE',
    dayWed: 'WED',
    dayThu: 'THU',
    dayFri: 'FRI',
    daySat: 'SAT',
    daySun: 'SUN',
    // log
    log: 'Punch log',
    logCaption: 'Every event keeps its source; corrections never overwrite the original',
    logEmpty: 'No punches recorded today.',
    colTime: 'Time',
    colEvent: 'Event',
    colSource: 'Source',
    colNote: 'Note',
    eventIn: 'Clock in',
    eventOut: 'Clock out',
    eventBreakStart: 'Break start',
    eventBreakEnd: 'Break end',
    eventRunning: 'Running',
    onTime: 'On time',
    sourceFiled: 'Entered by hand',
    sourceLive: 'Punched here',
    pendingApproval: '{n} entr(ies) awaiting approval',
    // floor column
    floorStatus: 'Floor status',
    nWorking: '{n} working',
    needDecision: '{n} punch(es) need someone to decide',
    allClean: 'Nothing on the floor needs a decision.',
    impossibleSpan: 'Span too long to be a real shift',
    closeAt: 'Close at {time}',
    enterByHand: 'Enter by hand',
    wouldClose: 'Closing a punch needs the 3a review screen.',
    wouldEdit: 'Manual correction lives on the 3a screen.',
    lateMeta: '{n} people · longest {m}',
    noneLate: 'Everyone was inside the grace period',
    noPunch: 'No punch today',
    everyonePunched: 'Everyone on the roster has punched',
    workingByDept: 'Working by department',
    rationale: 'The 76px clock and one solid button carry the screen: everything else is evidence for the single decision of when to punch.'
  },
  performanceAppraisalPage: {
    title: "Performance Appraisal",
    overallPerformance: "Overall Performance",
    goalsAchieved: "Goals Achieved",
    reviewsThisPeriod: "Reviews This Period",
    avgSkillRating: "Avg Skill Rating",
    currentGoals: "Current Goals",
    addGoal: "Add Goal",
    overview: "Overview",
    goals: "Goals",
    reviews: "Reviews"
  },
  employeeDetailTabs: {
    basicInformation: "Basic Information",
    contact: "Contact",
    documents: "Documents"
  },
  employeeEditForm: {
    editEmployee: "Edit Employee",
    contactInformation: "Contact Information",
    employmentDetails: "Employment Details",
    email: "Email",
    phone: "Phone",
    telephone: "Telephone",
    address: "Address",
    department: "Department",
    abteilung: "Department",
    startDate: "Start Date",
    startdatum: "Start Date",
    dateOfBirth: "Date of Birth",
    performance: "Performance",
    leistung: "Performance",
    cancel: "Cancel",
    abbrechen: "Cancel",
    save: "Save",
    speichern: "Save"
  },
  // Department Pie Chart Names
  departments: {
    all: 'All',
    legal_compliance: 'Legal Compliance',
    technology: 'Technology',
    internal_affairs: 'Internal Affairs',
    human_resources: 'Human Resources',
    office_unit: 'Office Unit',
    board_of_directors: 'Board of Directors',
    finance: 'Finance',
    engineering: 'Engineering',
    sales: 'Sales',
    marketing: 'Marketing',
    design: 'Design',
    part_time_employee: 'Experts Group'
  },
  // Morphing Showcase
  morphingShowcase: {
    title: 'Flubber Morphing Integration',
    subtitle: 'Smooth SVG morphing animations powered by Flubber',
    interactiveTimeIcon: 'Interactive Time Icon',
    clickToMorph: 'Click to morph between clock and calendar',
    clock: 'Clock',
    calendar: 'Calendar',
    autoMorphingIcon: 'Auto-Morphing Icon',
    autoMorphDescription: 'Automatically morphs between clock and calendar every 3 seconds',
    statusIndicators: 'Status Indicators',
    uiElements: 'UI Elements',
    useCases: 'Use Cases in HR Software',
    timeTracking: 'Time Tracking',
    timeTrackingDesc: 'Morph between clock in/out states, show time transitions',
    statusUpdates: 'Status Updates',
    statusUpdatesDesc: 'Smooth transitions between task states and indicators',
    uiFeedback: 'UI Feedback',
    uiFeedbackDesc: 'Engaging animations for actions and state changes',
    features: 'Features',
    smoothPathInterpolation: 'Smooth path interpolation',
    autoPlaySupport: 'Auto-play support',
    themeAwareColors: 'Theme-aware colors',
    optimizedPerformance: 'Optimized performance',
    manualControl: 'Manual control',
    responsiveDesign: 'Responsive design',
    customizableTiming: 'Customizable timing',
    multipleEasingFunctions: 'Multiple easing functions'
  },
  // Flubber Demo
  flubberDemo: {
    title: 'Flubber SVG Morphing Demo',
    subtitle: 'Smooth SVG path transitions with Flubber interpolation',
    basicShapes: 'Basic Shapes',
    timeCalendar: 'Time & Calendar',
    uiElements: 'UI Elements',
    statusIcons: 'Status Icons',
    workRelated: 'Work Related',
    smoothTransitions: 'Smooth Transitions',
    smoothTransitionsDesc: 'Flubber creates smooth interpolations between SVG paths, handling complex shapes with ease.',
    autoPlay: 'Auto Play',
    autoPlayDesc: 'Automatic cycling through shapes with customizable intervals and durations.',
    customizable: 'Customizable',
    customizableDesc: 'Control colors, sizes, animations, and easing functions for perfect UI integration.',
    usageExample: 'Usage Example',
    features: 'Features',
    smoothPathInterpolation: 'Smooth path interpolation using Flubber',
    autoPlayIntervals: 'Auto-play with customizable intervals',
    themeAwareColors: 'Theme-aware colors',
    optimizedPerformance: 'Optimized performance with requestAnimationFrame',
    manualShapeSelection: 'Manual shape selection',
    responsiveAccessible: 'Responsive and accessible',
    customizableDuration: 'Customizable animation duration',
    easingFunctions: 'Easing functions for natural motion'
  },
  // Demo employee display values (used for demo mode translations)
  demoEmployees: {
    'demo-emp-1': {
      name: 'Admin',
      location: 'Headquarters'
    },
    'demo-emp-2': {
      name: 'Engineer',
      location: 'Headquarters'
    },
    'demo-emp-3': {
      name: 'UI/UX Designer',
      location: 'Remote'
    },
    'demo-emp-4': {
      name: 'Marketing Specialist',
      location: 'Headquarters'
    },
    'demo-emp-5': {
      name: 'Sales Representative',
      location: 'Remote'
    }
  },
  // Location names
  locations: {
    headquarters: 'Headquarters',
    remote: 'Remote'
  },
  // Demo tasks (for demo mode translations)
  demoTasks: {
    'task-1': {
      title: 'Q4 Report Analysis',
      description: 'Analyze Q4 performance metrics and prepare presentation - NOTE: This demo task cannot be edited or deleted.'
    },
    'task-2': {
      title: 'Update Employee Handbook',
      description: 'Review and update the 2024 employee handbook - NOTE: This demo task cannot be edited or deleted.'
    },
    'task-3': {
      title: 'Fix Login Bug',
      description: 'Investigate reported login issues on mobile devices - NOTE: This demo task cannot be edited or deleted.'
    },
    'task-4': {
      title: 'Design New Dashboard',
      description: 'Create wireframes and mockups for the analytics dashboard - NOTE: This demo task cannot be edited or deleted.'
    },
    'task-5': {
      title: 'Campaign Performance Review',
      description: 'Analyze Q3 marketing campaign results and ROI - NOTE: This demo task cannot be edited or deleted.'
    },
    'task-6': {
      title: 'Client Onboarding',
      description: 'Complete onboarding for new enterprise client - NOTE: This demo task cannot be edited or deleted.'
    },
    'task-7': {
      title: 'API Documentation',
      description: 'Write comprehensive API documentation for v2.0 release - NOTE: This demo task cannot be edited or deleted.'
    }
  },
  // Demo goals (for demo mode translations)
  demoGoals: {
    'goal-1': {
      title: 'Improve Team Efficiency',
      description: 'Implement new project management workflow - NOTE: This demo goal cannot be edited or deleted.'
    },
    'goal-2': {
      title: 'Learn React Native',
      description: 'Complete advanced React Native course - NOTE: This demo goal cannot be edited or deleted.'
    },
    'goal-3': {
      title: 'UX Certification',
      description: 'Obtain Google UX Design Professional Certificate - NOTE: This demo goal cannot be edited or deleted.'
    },
    'goal-4': {
      title: 'Increase Lead Generation',
      description: 'Achieve 30% increase in qualified leads through marketing campaigns - NOTE: This demo goal cannot be edited or deleted.'
    },
    'goal-5': {
      title: 'Close Enterprise Deals',
      description: 'Close 5 enterprise-level deals worth $500K+ each - NOTE: This demo goal cannot be edited or deleted.'
    }
  },
  demoReviews: {
    'review-1': {
      reviewPeriod: 'Q4 2024',
      strengths: 'Excellent technical skills, great team player',
      areasForImprovement: 'Could improve on documentation',
      achievements: 'Led the migration to new architecture',
      comments: 'Outstanding performance this quarter'
    },
    'review-2': {
      reviewPeriod: 'Q4 2024',
      strengths: 'Excellent communication and problem-solving',
      areasForImprovement: 'More proactive in meetings',
      achievements: 'Improved UI design system',
      comments: 'Great contribution to the team'
    }
  },
  demoSkills: {
    'skill-1': {
      name: 'React',
      category: 'Technical',
      proficiencyLevel: 'Expert'
    },
    'skill-2': {
      name: 'TypeScript',
      category: 'Technical',
      proficiencyLevel: 'Advanced'
    },
    'skill-3': {
      name: 'UI/UX Design',
      category: 'Design',
      proficiencyLevel: 'Expert'
    }
  },
  demoFeedback: {
    'feedback-1': {
      type: 'Peer Review',
      text: 'Great collaboration on the project. Very helpful and knowledgeable.'
    },
    'feedback-2': {
      type: 'Manager Review',
      text: 'Excellent work on the new design system. Keep it up!'
    }
  },
  demoJobs: {
    'job-1': {
      title: 'Senior Frontend Developer',
      description: 'We are looking for an experienced Frontend Developer to join our team and help build amazing web applications.',
      location: 'Remote',
      salaryRange: '$90k - $120k',
      requirements: {
        0: 'React',
        1: 'TypeScript',
        2: 'Tailwind CSS'
      }
    },
    'job-2': {
      title: 'Product Designer',
      description: 'Join our design team to create amazing user experiences and shape the future of our products.',
      location: 'New York',
      salaryRange: '$80k - $100k',
      requirements: {
        0: 'Figma',
        1: 'UI/UX',
        2: 'Prototyping'
      }
    }
  },
  demoApplicants: {
    'app-1': {
      firstName: 'John',
      lastName: 'Doe',
      education: 'Bachelor of Computer Science',
      currentPosition: 'Senior Developer'
    },
    'app-2': {
      firstName: 'Jane',
      lastName: 'Smith',
      education: 'Master of Design',
      currentPosition: 'UX Lead'
    }
  },
  demoApplications: {
    'appl-1': {
      status: 'Under Review',
      notes: 'Strong candidate with excellent technical background'
    },
    'appl-2': {
      status: 'Interview Scheduled',
      notes: 'Excellent portfolio and design skills'
    }
  },
  demoInterviews: {
    'int-1': {
      status: 'Scheduled',
      type: 'Technical Interview',
      notes: 'Portfolio review and technical assessment'
    }
  },
  demoNotifications: {
    'notif-1': {
      title: "Welcome to HR System",
      message: "Welcome to the demo mode of the HR System. Feel free to explore all features!",
      actionLabel: "Go to Dashboard"
    },
    'notif-2': {
      title: "New Task Assigned",
      message: "You have been assigned a new task: Review Q3 Performance",
      actionLabel: "View Task"
    },
    'notif-3': {
      title: "Performance Review Due",
      message: "Quarterly performance reviews are due next week.",
      actionLabel: "View Calendar"
    }
  },
  // Translation Studio (manual UGC translations)
  translationStudio: {
    // Ticker figures
    strings: 'Strings',
    inQueue: 'In queue',
    locales: 'Locales',
    coverage: 'Coverage',
    selected: 'Selected',
    translateAll: 'Translate all',
    confirmTranslateAll: 'Replace all existing translations for this text with machine output? Nothing is saved until you press Save.',
    sourceCountHint: 'Fully translated records / total records',
    resizePanel: 'Resize source panel',
    accessDenied: 'Access Denied: Admin privileges required',
    downloading: 'Downloading language model',
    suggestAll: 'Suggest all empty',
    suggestHint: 'Fills blank boxes only. Nothing is saved until you review it.',
    suggested: 'Suggestions filled in',
    needsDownload: 'Suggesting this language downloads a model first',
    notConfigured: 'The translations table is missing. Apply migration 018_hr_ugc_translations.sql (supabase db push) to enable saving.',
    title: 'Translation Studio',
    subtitle: 'Hand-written translations for tasks, goals, reviews and leave requests. These override the automatic ones everywhere.',
    sources: 'Sources',
    fields: 'Fields',
    sourceTasks: 'Tasks',
    sourceGoals: 'Personal Goals',
    sourceReviews: 'Performance Reviews',
    sourceLeave: 'Leave Requests',
    status: 'Status',
    filter_all: 'All',
    filter_untranslated: 'Untranslated',
    filter_translated: 'Complete',
    filter_stale: 'Out of date',
    searchPlaceholder: 'Search text, record or employee',
    empty: 'Nothing matches these filters.',
    loadError: 'Could not load translatable content.',
    selectPrompt: 'Pick a string on the left to translate it.',
    saved: 'Saved',
    removed: 'Removed',
    saveError: 'Could not save',
    stale: 'source changed',
    machineDraft: 'Machine draft',
    remove: 'Remove',
    draftUnavailable: 'No on-device model for this language yet.',
    noLocalModel: 'This browser has no on-device translator, so machine drafts are unavailable. You can still write translations by hand.',
    demoUnavailable: 'Translations are stored on the server and are unavailable in demo mode.',
    field_title: 'Title',
    field_description: 'Description',
    field_self_assessment: 'Self assessment',
    field_comments: 'Comments',
    field_notes: 'Notes',
    field_success_criteria: 'Success criteria',
    field_strengths: 'Strengths',
    field_areas_for_improvement: 'Areas for improvement',
    field_achievements: 'Achievements',
    field_employee_comments: 'Employee comments',
    field_reason: 'Reason',
    field_rejection_reason: 'Rejection reason',
  },
};
