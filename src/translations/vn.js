export default {
  // Navigation
  nav: {
    menu: 'Menu',
    workload: 'Quản Lý Công Việc',
    attendanceHub: 'Quản Lý Thời Gian',
    timeClock: 'Chấm Công',
    punchClock: 'Bấm Giờ',
    dashboard: 'Trung Tâm',
    employees: 'Nhân Viên',
    recruitment: 'Tuyển Dụng',
    leaveManagement: 'Quản Lý Nghỉ Phép',
    timeTracking: 'Tóm Tắt Thời Gian',
    performance: 'Mục Tiêu Cá Nhân',
    reports: 'Báo Cáo',
    notifications: 'Thông Báo',
    settings: 'Cài Đặt',
    translations: 'Xưởng Dịch Thuật',
    policyControls: 'Kiểm Soát Chính Sách',
    controlPanel: 'Bảng Điều Khiển',
    personalGoals: 'Mục Tiêu Cá Nhân',
    taskListing: 'Danh Sách Công Việc',
    taskReview: 'Đánh Giá Hiệu Suất',
    svgMorphing: 'SVG Biến Hình',
    fullDemo: 'Demo Đầy Đủ',
    showcase: 'Trưng Bày Nhanh'
  },
  // Leave Management
  leave: {
    subtitle: 'Lập kế hoạch, yêu cầu và phê duyệt nghỉ phép trên lịch dùng chung.',
    // Chrome của bản vẽ
    month: 'Tháng',
    byType: 'Theo loại',
    ofNRequests: 'trên {n} yêu cầu',
    pendingNote: 'Phê duyệt hoặc từ chối ngay tại dòng của yêu cầu trong sổ.',
    nothingPending: 'Không có yêu cầu nào đang chờ quyết định.',
    requestLeave: 'Yêu Cầu Nghỉ Phép',
    requestsTitle: 'Yêu Cầu Nghỉ Phép',
    noRequests: 'Chưa có yêu cầu nghỉ phép nào.',
    everyone: 'Mọi Người',
    mine: 'Nghỉ Phép Của Tôi',
    today: 'Hôm Nay',
    clearSelection: 'Xóa lựa chọn',
    selectStartHint: 'Mẹo: nhấp vào một ngày để bắt đầu yêu cầu nghỉ phép, sau đó nhấp vào ngày cuối.',
    selectEndHint: 'Bây giờ nhấp vào ngày kết thúc (hoặc cùng ngày nếu chỉ nghỉ một ngày).',
    selectionRange: 'Đã chọn',
    selectDatesFirst: 'Vui lòng nhấp vào ngày bắt đầu và ngày kết thúc trên lịch trước.',
    selectedDates: 'Ngày Đã Chọn',
    adjustOnCalendar: 'Đóng hộp thoại này để chọn lại ngày trên lịch.',
    invalidRange: 'Ngày kết thúc không thể trước ngày bắt đầu.',
    selectEmployee: 'Vui lòng chọn một nhân viên.',
    type: 'Loại',
    allTypes: 'Tất Cả Loại',
    employee: 'Nhân Viên',
    allEmployees: 'Tất Cả Nhân Viên',
    days: 'ngày',
    halfDay: 'Nửa ngày',
    halfDayTag: '[Nửa ngày]',
    startTime: 'Giờ Bắt Đầu',
    endTime: 'Giờ Kết Thúc',
    reason: 'Lý Do',
    reasonPlaceholder: 'Thêm ghi chú tùy chọn cho quản lý của bạn...',
    submitRequest: 'Gửi Yêu Cầu',
    requestSubmitted: 'Đã gửi yêu cầu nghỉ phép thành công!',
    requestApproved: 'Đã phê duyệt yêu cầu nghỉ phép.',
    requestRejected: 'Đã từ chối yêu cầu nghỉ phép.',
    rejectReasonPrompt: 'Lý do từ chối yêu cầu này (tùy chọn):',
    approve: 'Phê Duyệt',
    reject: 'Từ Chối',
    other: 'Khác',
    totalRequests: 'Tổng Yêu Cầu',
    pending: 'Đang Chờ',
    approvedDaysYear: 'Ngày Được Duyệt (Năm)',
    onLeaveToday: 'Nghỉ Hôm Nay',
    stepPickStart: 'Nhấp vào ngày đầu tiên',
    stepPickEnd: 'Nhấp vào ngày cuối cùng',
    rangeReady: 'Đã chọn ngày — sẵn sàng yêu cầu nghỉ phép',
    requestLeaveNow: 'Yêu Cầu Nghỉ Phép Ngay',
    rangeStart: 'Bắt đầu',
    rangeEnd: 'Kết thúc',
    clickToSelect: 'Nhấp để chọn',
    filtersTitle: 'Bộ lọc & chú thích',
    statusFilter: 'Trạng thái',
    allStatuses: 'Tất cả',
    legendTitle: 'Chú thích',
    addForEmployee: 'Thêm cho nhân viên',
    addLeaveForEmployee: 'Thêm nghỉ phép cho nhân viên',
    autoApprove: 'Phê duyệt ngay',
    autoApproveHint: 'Bỏ qua hàng chờ và đánh dấu đã phê duyệt.',
    submitAndApprove: 'Gửi & phê duyệt',
    submitAndApproved: 'Đã thêm và phê duyệt nghỉ phép.',
    manualDates: 'Nhập ngày bắt đầu và kết thúc.',
    onBehalfNote: 'Bạn đang gửi nghỉ phép thay cho nhân viên này.',
    rejectTitle: 'Từ chối yêu cầu nghỉ phép',
    rejectConfirm: 'Từ chối nghỉ phép của {{name}}?',
    rejectReasonLabel: 'Lý do (tùy chọn)',
    rejectReasonPlaceholder: 'Giải thích lý do từ chối yêu cầu này...',
    confirmReject: 'Từ chối yêu cầu'
  },
  // Video Player translations used by VideoPlayer component
  videoPlayer: {
    container: 'Trình Phát Video',
    currentTitle: 'Tiêu Đề Hiện Tại',
    counter: '{current}/{total}',
    unsupported: 'Trình duyệt của bạn không hỗ trợ thẻ video.',
    prev: 'Video Trước',
    play: 'Phát',
    pause: 'Tạm Dừng',
    playToggle: 'Phát / Tạm dừng',
    next: 'Video Tiếp Theo',
    status: {
      playing: 'Đang phát',
      paused: 'Đã tạm dừng'
    },
    name: 'Tên video'
  },
  // Header
  header: {
    title: 'Quản lý HR',
    welcome: 'Xin Chào,',
    welcomePhrase1: 'Chào mừng trở lại, {name}',
    welcomePhrase2: 'Rất vui được gặp bạn, {name}',
    welcomePhrase3: 'Xin chào, {name}!',
    welcomePhrase4: 'Sẵn sàng quản lý nhân sự, {name}?',
    welcomePhrase5: 'Hãy làm việc hiệu quả hôm nay, {name}',
    welcomePhrase6: 'Rất vui có bạn ở đây, {name}',
    user: 'Đội ngũ HR',
    logout: 'Đăng xuất',
    notifications: 'Thông báo',
    // Ô phiên làm việc trên dải thép: nhãn giãn chữ phía trên tên người dùng.
    welcomeBack: 'Chào mừng trở lại',
    // Dưới ký hiệu ICUE, cỡ 9px/.2em.
    consoleTag: 'Bảng điều khiển nhân sự',
    // Nhãn phía trên tiêu đề trang: "TỜ 01".
    sheet: 'Tờ'
  },
  // Session lifecycle: the idle countdown shown before an automatic sign-out.
  session: {
    idleWarningTitle: 'Bạn còn ở đó không?',
    idleWarningBody: 'Bạn đã không hoạt động một lúc. Vì lý do bảo mật, hệ thống sẽ tự động đăng xuất.',
    idleWarningCountdown: 'Đăng xuất sau',
    idleWarningSeconds: 'giây',
    staySignedIn: 'Tiếp tục đăng nhập',
    signOutNow: 'Đăng xuất ngay',
  },
  // Login
  login: {
    idleLogoutMessage: 'Bạn đã bị đăng xuất sau khi không hoạt động trong một thời gian. Vui lòng đăng nhập lại.',
    tryDemo: 'Thử Chế Độ Demo',
    requestDemo: 'Yêu Cầu Dùng Thử',
    requestDemoNote: 'Gửi email đến support@icue.vn để yêu cầu bản demo',
    success: 'Email đặt lại mật khẩu đã được gửi. Vui lòng kiểm tra hộp thư đến của bạn.',
    title: 'Quản lý HR',
    subtitle: 'Đăng nhập để truy cập bảng điều khiển',
    email: 'Địa chỉ Email',
    password: 'Mật khẩu',
    emailPlaceholder: 'emailcuaban@example.com',
    passwordPlaceholder: '••••••••',
    emailRequired: 'Email là bắt buộc',
    emailInvalid: 'Email không hợp lệ',
    passwordRequired: 'Mật khẩu là bắt buộc',
    passwordTooShort: 'Mật khẩu phải có ít nhất 6 ký tự',
    invalidCredentials: 'Email hoặc mật khẩu không hợp lệ',
    rememberMe: 'Ghi nhớ đăng nhập',
    forgotPassword: 'Quên mật khẩu?',
    forgotPasswordModal: {
      title: 'Đặt Lại Mật Khẩu',
      description: 'Nhập địa chỉ email của bạn và chúng tôi sẽ gửi cho bạn liên kết để đặt lại mật khẩu.',
      emailLabel: 'Địa Chỉ Email',
      emailPlaceholder: 'ban@vidu.com',
      emailRequired: 'Email là bắt buộc',
      emailInvalid: 'Vui lòng nhập email hợp lệ',
      sendReset: 'Gửi Liên Kết Đặt Lại',
      cancel: 'Hủy',
      success: 'Email đặt lại mật khẩu đã được gửi. Vui lòng kiểm tra hộp thư của bạn.',
      error: 'Không thể gửi email đặt lại. Vui lòng thử lại.',
      noAccount: 'Không tìm thấy tài khoản với địa chỉ email này',
      rateLimitError: 'Quá nhiều yêu cầu. Vui lòng thử lại sau',
      networkError: 'Lỗi mạng. Vui lòng kiểm tra kết nối của bạn'
    },
    signIn: 'Đăng Nhập',
    signingIn: 'Đang đăng nhập...',
    orContinueWith: 'Hoặc tiếp tục với',
    continueWithGithub: 'Tiếp tục với GitHub',
    redirecting: 'Đang chuyển hướng đến GitHub...',
    githubError: 'Đăng nhập với GitHub thất bại',
    noAccount: 'Chưa có tài khoản?',
    signUp: 'Đăng ký',
    footer: '© 2024 Quản lý HR. Tất cả quyền được bảo lưu.',
    tryDemoLoading: 'Đang tải bản demo...'
  },
  // Reset Password Page
  resetPassword: {
    title: 'Đặt Lại Mật Khẩu',
    subtitle: 'Nhập mật khẩu mới của bạn bên dưới',
    newPassword: 'Mật Khẩu Mới',
    confirmPassword: 'Xác Nhận Mật Khẩu',
    newPasswordPlaceholder: 'Nhập mật khẩu mới',
    confirmPasswordPlaceholder: 'Xác nhận mật khẩu mới',
    allFieldsRequired: 'Tất cả các trường là bắt buộc',
    passwordTooShort: 'Mật khẩu phải có ít nhất 6 ký tự',
    passwordsDontMatch: 'Mật khẩu không khớp',
    error: 'Không thể đặt lại mật khẩu. Vui lòng thử lại.',
    success: 'Đặt lại mật khẩu thành công!',
    redirecting: 'Đang chuyển hướng đến trang đăng nhập...',
    resetting: 'Đang đặt lại...',
    verifying: 'Đang xác minh liên kết đặt lại...',
    resetButton: 'Đặt Lại Mật Khẩu',
    backToLogin: 'Quay Lại Đăng Nhập',
    invalidLink: 'Liên kết đặt lại không hợp lệ hoặc đã hết hạn. Vui lòng yêu cầu đặt lại mật khẩu mới.'
  },
  sidebar: {
    main: 'Chính',
    analytics: 'Phân Tích',
    settings: 'Cài Đặt',
    collapse: 'Thu Gọn Thanh Bên',
    expand: 'Mở Rộng Thanh Bên'
  },
  // Dashboard
  dashboard: {
    workforce: 'Nhân sự',
    headcount: 'Quân số',
    attrition: 'Nghỉ việc',
    hours: 'Giờ',
    leave: 'Nghỉ phép',
    performance: 'Hiệu suất',
    approvals: 'Phê duyệt',
    total: 'Tổng',
    regularAndOvertimeByEmployee: 'Giờ Làm Thường & Giờ Làm Thêm',
    currentMonth: 'Tháng Hiện Tại',
    employeeLabel: 'Nhân Viên',
    liveData: 'Dữ liệu trực tiếp từ Công Ty',
    noData: 'Không có dữ liệu khả dụng',
    title: 'Tổng Quan Bảng Điều Khiển',
    overview: 'Tổng Quan Tổ Chức',
    lastUpdate: 'Cập nhật',
    totalEmployees: 'Tổng Số Nhân Viên',
    totalRegularHours: 'Tổng Giờ Làm Thường',
    pendingApprovals: 'Phê Duyệt Đang Chờ',
    activeEmployees: 'Nhân Viên Hoạt Động',
    newHires: 'Nhân Viên Mới Tháng Này',
    pendingApplications: 'Đơn Ứng Tuyển Chờ Xử Lý',
    avgPerformance: 'Hiệu Suất Trung Bình',
    totalOvertime: 'Tổng Giờ Làm Thêm',
    totalLeave: 'Tổng Ngày Nghỉ Phép',
    employeePerformance: 'Hiệu Suất Nhân Viên',
    departmentDist: 'Phân Bố Phòng Ban',
    regularHoursByEmployee: 'Giờ Làm Thường',
    overtimeHoursByEmployee: 'Giờ Làm Thêm',
    performanceRating: 'Đánh Giá Hiệu Suất',
    regularHoursLegend: 'Giờ Làm Thường',
    totalOvertimeLegend: 'Tổng Giờ Làm Thêm',
    workLeaveComp: 'So Sánh Ngày Làm và Nghỉ',
    topPerformers: 'Nhân Viên Xuất Sắc',
    overtime: 'Giờ Làm Thêm',
    totalWorkDays: 'Tổng Ngày Làm Việc',
    acrossEmployees: 'Tất Cả Nhân Viên Trong Tháng',
    activeApplications: 'Đơn Ứng Tuyển Đang Xử Lý',
    pendingReview: 'Chờ Duyệt',
    pendingRequests: 'Yêu Cầu Chờ Xử Lý',
    leaveRequests: 'Đơn Xin Nghỉ Phép Chờ Duyệt',
    recentActivity: 'Hoạt Động Gần Đây',
    viewAll: 'Xem Tất Cả',
    // Bảng Điều Khiển Tổng Quan Tổ Chức
    company: 'Toàn Công Ty',
    outOf: 'trên',
    reviews: 'lượt đánh giá',
    high: 'Cao Nhất',
    low: 'Thấp Nhất',
    filterByDept: 'Lọc theo phòng ban',
    needsDecision: 'Cần Quyết Định',
    items: 'mục',
    queueClear: 'Không còn mục nào chờ xử lý',
    oldestWaited: 'Mục lâu nhất đã chờ',
    overdue: 'Quá Hạn',
    nothingPending: 'Hiện không có mục nào chờ bạn xử lý.',
    more: 'mục khác',
    noApplications: 'Không có hồ sơ ứng tuyển',
    pendingApprovalsTotal: 'mục chờ duyệt trên tất cả loại yêu cầu',
    pipeline: {
      screening: 'Sàng Lọc',
      shortlisted: 'Vào Danh Sách Ngắn',
      interview: 'Phỏng Vấn',
      offer: 'Đề Nghị Tuyển'
    },
    activities: {
      newEmployee: "Nhân Viên Mới Vào Làm: ABC ",
      interviewScheduled: "Lên Lịch Phỏng Vấn Với: XYZ",
      performanceReview: "Đánh Giá Hiệu Suất Đã Hoàn Thành Cho Nhân Viên: Đỗ Bảo Long"
    },
    timeAgo: {
      twoHours: "2 Giờ Trước",
      fourHours: "4 Giờ Trước",
      oneDay: "1 Ngày Trước"
    }
  },
  // Search
  search: {
    placeholder: 'Tìm Kiếm Nhân Viên...'
  },
  employeeDepartment: {
    all: 'Tất Cả',
    legal_compliance: 'Pháp Lý',
    technology: 'Công Nghệ',
    internal_affairs: 'Quản Lý Nội Bộ',
    human_resources: 'Nhân Sự',
    office_unit: 'Khối Văn Phòng',
    board_of_directors: 'Hội Đồng Quản Trị',
    finance: 'Tài Chính',
    engineering: 'Kỹ Thuật',
    sales: 'Bán Hàng',
    marketing: 'Tiếp Thị',
    design: 'Thiết Kế',
    operations: 'Vận Hành',
    part_time_employee: 'Khối Chuyên Gia',
    Management: 'Quản Lý',
    Engineering: 'Kỹ Thuật',
    Design: 'Thiết Kế',
    Marketing: 'Tiếp Thị',
    Sales: 'Bán Hàng',
    Finance: 'Tài Chính',
    'Human Resources': 'Nhân Sự',
    Technology: 'Công Nghệ',
    Operations: 'Vận Hành',
    Support: 'Hỗ Trợ',
    Legal: 'Pháp Lý',
    IT: 'Tin Học',
    it: 'Công Nghệ Thông Tin',
    product: 'Sản Phẩm',
    customer_support: 'Hỗ Trợ Khách Hàng'
  },
  // Months
  months: {
    january: 'Tháng Một',
    february: 'Tháng Hai',
    march: 'Tháng Ba',
    april: 'Tháng Tư',
    may: 'Tháng Năm',
    june: 'Tháng Sáu',
    july: 'Tháng Bảy',
    august: 'Tháng Tám',
    september: 'Tháng Chín',
    october: 'Tháng Mười',
    november: 'Tháng Mười Một',
    december: 'Tháng Mười Hai'
  },
  // Time Tracking
  timeTracking: {
    noRecords: 'Không tìm thấy bản ghi chấm công nào cho kỳ này',
    approve: 'Phê duyệt',
    reject: 'Từ chối',
    approveSuccess: 'Đã phê duyệt yêu cầu nghỉ phép',
    rejectSuccess: 'Đã từ chối yêu cầu nghỉ phép',
    actionError: 'Lỗi khi cập nhật yêu cầu nghỉ phép',
    rejectReasonPrompt: 'Vui lòng nhập lý do từ chối (không bắt buộc):',
    on_leave: 'Đang Nghỉ Phép',
    detailedBreakdown: 'Phân Tích Chi Tiết Theo Dõi Thời Gian',
    title: 'Bảng Điều Khiển',
    overview: 'Tổng quan',
    leaveRequests: 'Quản Lý Nghỉ Phép',
    clockIn: 'Giờ Vào',
    clockOut: 'Giờ Ra',
    amountHours: 'Số Giờ',
    hours: 'Giờ',
    time: 'Thời Gian',
    hourType: 'Loại Giờ',
    notes: 'Ghi Chú',
    createdAt: 'Ngày Tạo',
    summary: 'Tóm Tắt',
    quickActions: 'Chức Năng',
    workDays: 'Ngày Làm Việc',
    overtime: 'Giờ Làm Thêm',
    wfh: 'Làm Việc Tại Nhà',
    holidayOvertime: 'Làm thêm ngày lễ',
    days: 'Ngày',
    thisMonth: 'Tháng này',
    totalHours: 'Tổng số giờ',
    regularHours: 'Giờ Làm Thường',
    overtimeHours: 'Giờ Làm Thêm',
    hrs: 'giờ',
    attendanceRate: 'Tỷ lệ chuyên cần',
    requestLeave: 'Xin nghỉ phép',
    logOvertime: 'Ghi nhận làm thêm',
    vacation: 'Nghỉ Phép Có Lương',
    sickLeave: 'Nghỉ Ốm',
    personal: 'Nghỉ Cá Nhân',
    unpaid: 'Nghỉ Không Lương',
    leaveDays: "Số Ngày Nghỉ",
    leaveType: "Loại Nghỉ Phép",
    status: "Trạng Thái",
    requestedBy: 'Người Yêu Cầu',
    approvedBy: 'Người Phê Duyệt',
    leaveRequestManagement: 'Quản Lý Đơn Xin Nghỉ Phép',
    pending: 'Đang chờ',
    approved: 'Đã phê duyệt',
    rejected: 'Từ chối',
    cancelled: 'Đã hủy',
    enteredByAdmin: 'Được nhập bởi quản trị viên:',
    // Renamed to avoid collision with status label
    requestStatus: {
      pending: 'Đang chờ xử lý',
      approved: 'Đã phê duyệt',
      rejected: 'Bị từ chối',
      cancelled: 'Đã hủy'
    },
    // Hour types for overtime
    weekend: 'Làm thêm cuối tuần',
    holiday: 'Làm thêm ngày lễ',
    bonus: 'Giờ Làm Hiệu Suất Cao',
    regular: 'Giờ Làm Thường',
    startDate: 'Ngày bắt đầu',
    endDate: 'Ngày kết thúc',
    reason: 'Lý do',
    reasonPlaceholder: 'Giải thích ngắn gọn lý do xin nghỉ...',
    overtimePlaceholder: 'Mô tả công việc thực hiện trong giờ làm thêm...',
    date: 'Ngày',
    leaveSuccess: 'Yêu cầu nghỉ phép đã được gửi thành công!',
    leaveError: 'Lỗi khi gửi yêu cầu nghỉ phép',
    overtimeSuccess: 'Đã ghi nhận tăng ca thành công!',
    overtimeError: 'Lỗi khi ghi nhận tăng ca',
    exportSuccess: 'Báo cáo đã được xuất thành công!',
    timeClockError: 'Lỗi chấm công',
    timeClockSuccess: 'Chấm công thành công!',
    includesPending: '*bao gồm đang chờ',
    totalRegularHours: 'Tổng giờ làm việc thường (Tất cả nhân viên)',
    pendingApprovals: 'Chờ phê duyệt',
    regularHoursChart: 'Giờ Làm Thường Bởi Nhân Viên',
    overtimeHoursChart: 'Giờ Làm Thêm Bởi Nhân Viên',
    overviewTitle: 'Tổng quan công ty',
    regularHoursLegend: 'Giờ Làm Việc Thường',
    totalOvertimeLegend: 'Tổng Giờ Tăng Ca',
    employee: 'Nhân Viên',
    daysWorked: 'Ngày Làm Việc',
    totalHoursLabel: 'Tổng Số Giờ',

    // ---- Time Tracking 2a: tải trọng so với hợp đồng ----
    view: 'Chế Độ Xem',
    load: 'Tải Trọng',
    ofContract: 'trên',
    timesheets: 'Bảng Chấm Công',
    topTen: '10 Đầu',
    daysShort: 'Ngày',
    regularShort: 'Thường',
    overtimeShort: 'Tăng Ca',
    totalShort: 'Tổng',
    employeesLower: 'nhân viên',
    workingDaysLower: 'ngày làm việc',
    workingDaysInPeriod: 'Số ngày làm việc trong kỳ',
    contractLower: 'hợp đồng',
    dayShort: 'ngày',
    monthLower: 'tháng',
    loadAgainstContract: 'Tải trọng so với hợp đồng',
    everyEmployee: 'Toàn bộ nhân viên, sắp xếp theo tổng giờ. Đường đứt nét là mức hợp đồng.',
    loadCaption: '{over} trên {total} nhân viên vượt mức hợp đồng {contract}h; giờ tăng ca là phần xếp chồng phía trên.',
    filter: {
      missingPunch: 'Thiếu giờ ra',
      overCap: 'Vượt trần',
      unapprovedOvertime: 'Tăng ca chưa duyệt',
      underContract: 'Dưới hợp đồng'
    },

    // ---- Cột ngoại lệ ----
    exceptions: 'Ngoại Lệ',
    fixBeforePayroll: 'Xử lý trước khi chốt lương',
    noExceptions: 'Không có gì cản trở việc chốt lương kỳ này.',
    people: 'người',
    others: 'người khác',
    missingPunch: 'Thiếu giờ ra',
    missingPunchOut: 'Thiếu giờ ra',
    autoClose: 'Tự chốt {time}',
    autoCloseConfirm: 'Đặt giờ ra {time} cho {count} bản ghi còn mở? Thao tác này cập nhật bảng chấm công.',
    autoCloseSuccess: 'Đã chốt {count} bản ghi lúc {time}',
    reviewEach: 'Xem từng ca',
    overOvertimeCap: 'Vượt trần tăng ca',
    aboveCap: 'Vượt trần {cap}h',
    topIs: 'cao nhất',
    openCases: 'Mở hồ sơ',
    raiseCap: 'Nâng trần',
    raiseCapPrompt: 'Trần tăng ca hàng tháng, tính theo giờ:',
    unapprovedOvertime: 'Tăng ca chưa duyệt',
    acrossTimesheets: '{hours}h trên {count} bản ghi',
    underContractHours: 'Dưới giờ hợp đồng',
    attendanceThisWeek: 'Chuyên cần, tuần này',
    onTime: 'Đúng giờ',
    onTimeThisWeek: 'Số lần đến đúng giờ trong tuần',
    lateArrival: 'Đến muộn',
    absentUnreported: 'Vắng, không báo'
  },
  // Work Days Modal
  workDaysModal: {
    title: 'Chi Tiết Ngày Làm Việc',
    entries: 'mục',
    totalHours: 'tổng giờ',
    search: 'Tìm kiếm...',
    exportExcel: 'Xuất Excel',
    date: 'Ngày',
    clockIn: 'Giờ Vào',
    clockOut: 'Giờ Ra',
    hours: 'Số Giờ',
    notes: 'Ghi Chú',
    noData: 'Không có dữ liệu ngày làm việc cho khoảng thời gian này'
  },
  // Admin Time Entry
  adminTimeEntry: {
    title: 'Nhập Giờ Làm Việc (Quản Trị)',
    description: 'Nhập giờ làm việc cho nhân viên (Chỉ Quản trị/Quản lý)',
    selectEmployee: 'Chọn Nhân Viên',
    selectEmployees: 'Chọn Nhân Viên',
    searchEmployees: 'Tìm kiếm nhân viên...',
    noEmployees: 'Không tìm thấy nhân viên',
    date: 'Ngày',
    clockIn: 'Giờ Vào',
    clockOut: 'Giờ Ra',
    hourType: 'Loại Giờ',
    regularHours: 'Giờ Làm Thường',
    weekendOvertime: 'Cuối Tuần/Tăng Ca',
    holiday: 'Ngày Lễ',
    bonusHours: 'Giờ Cộng Thêm',
    notes: 'Ghi Chú',
    notesPlaceholder: 'Thêm Ghi Chú: VD: Làm tại VP, Đi họp tại...Đi công tác ở...',
    submitButton: 'Gửi Mục Nhập Giờ',
    submitBulkEntries: 'Gửi Mục Nhập Cho Nhân Viên',
    submitting: 'Đang gửi...',
    accessDenied: 'Truy cập bị từ chối: Bạn không có quyền quản lý mục nhập giờ của nhân viên khác.',
    errorLoadEmployees: 'Không thể tải danh sách nhân viên',
    success: 'Đã thêm mục nhập giờ thành công',
    error: 'Không thể tạo mục nhập giờ',
    selected: 'Đã chọn',
    selectedEmployees: 'Nhân Viên Đã Chọn',
    selectAtLeastOne: 'Vui lòng chọn ít nhất một nhân viên',
    entryAddedSuccess: 'Đã thêm mục nhập giờ thành công cho',
    entriesAddedSuccess: 'Đã thêm mục nhập giờ thành công cho',
    employees: 'Nhân viên',
    skippedEmployees: 'Đã bỏ qua {count} nhân viên có mục nhập hiện tại: {names}',
    searchHint: 'Tìm theo tên, email hoặc vị trí, rồi chọn những nhân viên được ghi nhận trong mục nhập này.',
    willFile: 'Mục nhập này sẽ ghi',
    totalHoursFiled: 'Tổng giờ sẽ ghi',
    autoApprovedNote: '* Mục nhập tại đây được duyệt ngay khi gửi',
    bulkScopeNote: '* {n} nhân viên trong hệ thống',
    hourTypes: {
      regular: 'Giờ Làm Thường',
      holiday: 'Ngày Lễ',
      weekend: 'Cuối Tuần',
      overtime: 'Giờ Làm Thêm',
      bonus: 'Giờ Làm Hiệu Suất Cao',
      wfh: 'Làm Việc Tại Nhà (Trực Tuyến)',
      onLeave: 'Nghỉ Phép'
    },
    errors: {
      checkFailed: 'Không thể kiểm tra các mục nhập hiện tại',
      allDuplicates: 'Tất cả nhân viên đã chọn đã có mục nhập giờ cho {date}: {names}'
    },
    bulkStandardHours: {
      title: 'Giờ Chuẩn Hàng Loạt',
      description: 'Tự động tạo mục nhập giờ làm thường từ 9:00 SA – 5:00 CH cho tất cả nhân viên chỉ vào ngày trong tuần. Thứ Bảy và Chủ Nhật sẽ được loại trừ. Các mục trùng lặp hiện có sẽ được bỏ qua.',
      startDate: 'Ngày Bắt Đầu',
      endDate: 'Ngày Kết Thúc',
      fillButton: 'Điền 9 SA – 5 CH cho Tất Cả Nhân Viên',
      filling: 'Đang điền giờ chuẩn...',
      confirmTitle: 'Xác Nhận Điền Giờ Hàng Loạt',
      confirmMessage: 'Tạo mục nhập giờ làm thường 9 SA – 5 CH cho tất cả nhân viên từ {start} đến {end}? Thứ Bảy và Chủ Nhật sẽ được loại trừ. Nhân viên có mục trùng lặp sẽ được bỏ qua.',
      confirmButton: 'Có, Điền Giờ',
      cancelButton: 'Hủy',
      success: 'Đã tạo {created} mục giờ chuẩn trong {dates} ngày trong tuần cho {employees} nhân viên. Đã bỏ qua {skipped} ô có mục hiện tại. Đã loại trừ {weekends} ngày cuối tuần.',
      noEntriesCreated: 'Không có mục nào được tạo. Tất cả nhân viên đã có mục trùng lặp cho các ngày đã chọn.',
      noWeekdaysInRange: 'Không có mục nào được tạo. Phạm vi đã chọn chỉ gồm cuối tuần, được loại trừ tự động.',
      invalidRange: 'Ngày kết thúc phải bằng hoặc sau ngày bắt đầu',
      invalidDate: 'Phạm vi ngày không hợp lệ.',
      rangeTooLarge: 'Phạm vi ngày không được vượt quá {max} ngày',
      noEmployeesFound: 'Không tìm thấy nhân viên để điền giờ.',
      errorGeneric: 'Không thể điền giờ chuẩn. Vui lòng thử lại.',
      errorWithDetail: 'Không thể điền giờ chuẩn: {message}'
    }
  },
  // Employees
  employees: {
    title: 'Quản Lý Nhân Viên',
    directory: 'Danh Mục Nhân Viên',
    cards: 'Thẻ',
    viewCards: 'Chế độ thẻ',
    viewDirectory: 'Chế độ danh mục',
    quickStats: 'Thống Kê Nhanh',
    addNew: 'Thêm Người Mới',
    searchPlaceholder: 'Tìm Kiếm Nhân Viên...',
    addEmployee: 'Thêm Nhân Viên',
    name: 'Tên',
    position: 'Vị Trí',
    department: 'Phòng Ban',
    email: 'Email',
    phone: 'Điện thoại',
    startDate: 'Ngày bắt đầu',
    salary: 'Lương',
    status: {
      pending: 'Đang chờ',
      approved: 'Đã phê duyệt',
      rejected: 'Từ chối',
      cancelled: 'Đã hủy',
    },
    location: 'Địa điểm',
    performance: 'Hiệu suất',
    actions: 'Hành động',
    view: 'Xem',
    edit: 'Chỉnh sửa',
    delete: 'Xóa',
    active: 'Hoạt động',
    inactive: 'Không hoạt động',
    employeeDetails: 'Chi tiết nhân viên',
    contactInformation: 'Thông tin liên hệ',
    employmentDetails: 'Chi tiết công việc',
    started: 'Bắt đầu',
    editEmployee: 'Chỉnh sửa nhân viên',
    namePlaceholder: 'Họ & tên',
    emailPlaceholder: 'Email',
    phonePlaceholder: 'Số điện thoại',
    dobPlaceholder: 'Ngày sinh',
    addressPlaceholder: 'Địa chỉ',
    updateSuccess: 'Đã cập nhật thông tin nhân viên thành công!',
    onLeave: 'Đang nghỉ phép',
    orgNetwork: 'Mạng lưới tổ chức',
    orgNetworkDesc: 'Các phòng ban kết nối với trung tâm nhân sự của bạn.',
    topPerformers: 'Nhân viên xuất sắc',
    avgRating: 'Điểm TB',
    departments: 'phòng ban',
    livePulse: 'Nhịp sống'
  },
  // Danh mục nhân viên — sổ nhân sự, thẻ thông số và hồ sơ một người
  employeeDirectory: {
    title: 'Danh mục nhân viên',
    // Thanh chỉ số
    headcount: 'Nhân sự',
    working: 'Đang làm',
    onLeaveToday: 'Nghỉ hôm nay',
    joinedThisMonth: 'Vào tháng này',
    missingRecords: 'Hồ sơ thiếu',
    tickerCode: 'Mã NV',
    tickerUnit: 'Đơn vị',
    openWork: 'Nhiệm vụ mở',
    // Hàng tiêu đề và dải lọc
    peopleInUnits: '{n} người trong {u} đơn vị',
    joinedThisMonthLong: '{n} vào làm tháng này',
    missingRecordsLong: '{n} hồ sơ còn thiếu giấy tờ',
    platesSubtitle: 'Chế độ thẻ · dùng khi cần nhận mặt người mới, không dùng để so sánh',
    exportList: 'Xuất danh sách',
    exported: 'Đã xuất {n} người.',
    scope: 'Phạm vi',
    scopeAll: 'Tất cả',
    scopeStarting: 'Sắp vào làm',
    scopeLeft: 'Đã nghỉ việc',
    searchPlaceholder: 'Tìm theo tên, mã nhân viên hoặc đơn vị',
    groupedByUnit: 'Nhóm theo đơn vị',
    sortedByRating: 'Sắp theo hiệu suất',
    viewMode: 'Chế độ xem',
    // Sổ nhân sự
    contact: 'Liên hệ',
    tenure: 'Thâm niên',
    thisWeek: 'Tuần này',
    mean: 'TB',
    nPeople: '{n} người',
    nStarting: '{n} sắp vào làm',
    nAwaiting: '{n} chờ duyệt',
    nMissingDocs: '{n} thiếu giấy tờ',
    ledgerFooter: 'Hiển thị {rows} / {total} người · {open} / {units} đơn vị',
    expandUnits: 'Xem tất cả đơn vị',
    collapseUnits: 'Thu gọn tất cả',
    nothingMatches: 'Không có ai khớp bộ lọc này.',
    // Dữ kiện của một người
    noMailbox: 'Chưa cấp hộp thư',
    noPhone: 'Chưa có số điện thoại',
    noUnit: 'Chưa xếp đơn vị',
    joinedOn: 'vào làm',
    startsOn: 'vào làm',
    since: 'Từ',
    noStartDate: 'Chưa có ngày vào làm',
    yearsShort: 'năm',
    monthsShort: 'tháng',
    inNDays: 'còn {n} ngày',
    notStartedYet: 'Chưa bắt đầu',
    noTimesheet: 'Chưa chấm công',
    onLeaveNote: 'Đang nghỉ phép',
    noOvertime: 'Không NG',
    overtimeShort: 'NG',
    pendingHours: '{h} chờ duyệt',
    documentsComplete: 'Đầy đủ',
    documentsMissing: 'Còn thiếu',
    recordsOnFile: 'Đã có hồ sơ',
    recordsMissing: 'Chưa có giấy tờ',
    // Từ trạng thái
    stateWorking: 'Đang làm',
    stateOnLeave: 'Nghỉ phép',
    statePending: 'Chờ duyệt NG',
    stateStarting: 'Vào làm',
    stateLeft: 'Đã nghỉ việc',
    // Thẻ thông số
    code: 'Mã',
    joined: 'Vào làm',
    records: 'Hồ sơ',
    sendMail: 'Gửi thư',
    viewHours: 'Xem giờ',
    openRecord: 'Xem hồ sơ',
    outOfFive: 'trên 5.0',
    aboveMean: 'trên 5.0 · cao hơn trung bình công ty {m} là {d}',
    belowMean: 'trên 5.0 · thấp hơn trung bình công ty {m} là {d}',
    noRatingTitle: 'Chưa chấm điểm kỳ này',
    noRatingBody: 'Chưa có điểm nào được ghi, nên thanh đo để trống thay vì đoán.',
    noReviewYetTitle: 'Chưa có kỳ đánh giá',
    noReviewYetBody: 'Vào làm {date} — kỳ đầu tiên mở sau khi hoàn tất nhận việc.',
    plateFooter: 'Hiển thị {shown} / {total} người của {unit} · sắp theo hiệu suất',
    compareInLedger: 'So sánh trong danh sách',
    photoUpdated: 'Đã cập nhật ảnh.',
    // Cột quyết định
    needsAction: 'Cần xử lý',
    nItems: '{n} việc',
    attachedToPeople: 'Gắn với người, không phải với thẻ',
    pointersOnly: 'Chỉ là đường dẫn · quyết định nằm ở danh sách',
    queueEmpty: 'Không có việc nào đang chờ.',
    pendingTag: 'chờ duyệt',
    starterBody: 'Còn {n} ngày nữa vào làm tại {unit}. Hồ sơ {docs} — thiếu giấy tờ sẽ chặn buổi nhận việc tuần đầu.',
    pendingBody: '{h} khai trong tuần vẫn chưa duyệt, trong đó {ot} là ngoài giờ so với trần {cap}h mỗi tháng. Duyệt sẽ ghi vào kỳ lương đang mở.',
    startsInDays: 'Còn {n} ngày nữa vào làm · {unit}',
    overtimeOf: 'Ngoài giờ · {name}',
    pendingAgainstCap: '{h} chờ duyệt · trần {cap}h mỗi tháng',
    timesheetExceptions: '{n} chấm công chờ duyệt',
    acrossUnits: 'Thuộc {n} đơn vị',
    reviewsUnsigned: '{n} đánh giá chưa ký',
    reviewCycleNote: 'Kỳ hiện tại',
    leavePending: '{n} đơn nghỉ phép chờ duyệt',
    leavePendingNote: 'Đang chờ người duyệt',
    docsMissing: '{n} hồ sơ thiếu giấy tờ',
    justJoined: 'Mới vào làm',
    justJoinedNote: 'Tháng này · {n} hồ sơ còn thiếu giấy tờ',
    noRecentJoiners: 'Hai tháng qua chưa có ai vào làm.',
    topRated: 'Hiệu suất cao nhất',
    ofNRated: '{n} người có điểm',
    podiumNote: 'So với trung bình công ty {m}',
    noRatings: 'Chưa ai trong danh sách này được chấm điểm.',
    unitSpread: 'Phân bổ đơn vị',
    unitsAndPeople: '{u} đơn vị · {n} người',
    emptyUnits: 'Chưa có ai ở {list}',
    everyUnitStaffed: 'Đơn vị nào cũng đã có người',
    report: 'Báo cáo',
    unitNetwork: 'Mạng lưới đơn vị',
    unitNetworkNote: '{n} đơn vị lớn nhất và quân số của mỗi đơn vị.',
    // Hồ sơ — cột chỉ mục
    allPeople: 'Tất cả',
    searchInUnit: 'Tìm trong {n} người',
    shownOf: '{shown} / {total} người',
    viewCards: 'Xem thẻ',
    unitSize: '{n} người cùng đơn vị',
    fullProfile: 'Hồ sơ đầy đủ',
    editRecord: 'Sửa hồ sơ',
    deleteRecord: 'Xoá hồ sơ',
    assignWork: 'Giao nhiệm vụ',
    // Hồ sơ — bảng 01
    panelEmployment: 'Nhân sự',
    workplace: 'Nơi làm việc',
    noWorkplace: 'Chưa ghi nhận',
    overtimeCap: 'Trần ngoài giờ',
    perMonth: 'tháng',
    used: 'đã dùng',
    leaveBalance: 'Phép còn lại',
    leaveBalanceValue: '{left} / {total} ngày · đã nghỉ {used} ngày trong năm',
    documentsCompleteLong: 'Đầy đủ · đã có hồ sơ lưu',
    documentsMissingLong: 'Còn thiếu · chưa có hồ sơ lưu',
    // Hồ sơ — bảng 02
    panelTimesheet: 'Chấm công tuần này',
    openTimesheet: 'Mở bảng giờ',
    againstContract: 'trên hợp đồng {h}h · {ot} ngoài giờ',
    noEntriesThisWeek: 'Không có chấm công nào từ {from} đến {to}.',
    ownPending: '{n} mục trong số này vẫn đang chờ duyệt.',
    noPendingHere: 'Hồ sơ này không có gì chờ duyệt · {n} mục còn lại thuộc người khác.',
    // Hồ sơ — bảng 03
    panelReview: 'Đánh giá & mục tiêu',
    cycle: 'Kỳ',
    noCycle: 'Chưa có kỳ nào',
    selfAssessment: 'Tự đánh giá',
    managerReview: 'Quản lý đánh giá',
    unassignedReviewer: 'Chưa chỉ định người đánh giá',
    signOff: 'Chữ ký quản lý',
    filed: 'Đã gửi',
    notFiled: 'Chưa gửi',
    goalsMet: 'Mục tiêu đạt trong kỳ',
    openReview: 'Mở đánh giá',
    openGoals: 'Mục tiêu',
    reviewStatus: {
      draft: 'Bản nháp',
      submitted: 'Chưa ký',
      approved: 'Đã ký',
      acknowledged: 'Đã xác nhận',
      rejected: 'Trả lại'
    },
    // Hồ sơ — bảng 04
    panelWork: 'Nhiệm vụ đang mở',
    openTaskBook: 'Mở sổ nhiệm vụ',
    noOpenWork: 'Hồ sơ này chưa được giao việc nào.',
    due: 'Hạn',
    taskOpen: 'Đang làm',
    taskToday: 'Hôm nay',
    taskLate: 'Trễ hạn',
    taskDone: 'Xong',
    openWorkNote: '{n} nhiệm vụ mở, đều do một bản ghi phát sinh — không có việc tự do trong hồ sơ này.',
    priority: {
      low: 'Thấp',
      medium: 'Trung bình',
      high: 'Cao',
      urgent: 'Khẩn'
    }
  },
  // Add Employee Modal
  addEmployee: {
    title: 'Thêm nhân viên mới',
    submit: 'Thêm nhân viên',
    // Draft sheet (2d)
    directory: 'Danh bạ',
    step: 'Bước',
    stepPersonal: 'Cá nhân',
    stepEmployment: 'Việc làm',
    stepReview: 'Xem xét',
    fullName: 'Họ và tên',
    recordTitle: 'Hồ sơ nhân viên mới',
    draft: 'Bản nháp',
    addNew: 'Thêm mới',
    autosaved: 'Đã tự lưu',
    notSavedYet: 'Chưa lưu',
    draftSaved: 'Đã lưu bản nháp',
    reservedId: 'Mã hồ sơ đã giữ chỗ',
    stepOf: 'Bước {n} trên {total}',
    inProgress: 'Đang thực hiện',
    done: 'Hoàn tất',
    next: 'Tiếp theo',
    locked: 'Đang khoá',
    requiredCounter: '{filled} trên {total} trường bắt buộc',
    allPresent: 'đã đủ',
    nationalId: 'CCCD/CMND',
    nationalIdRequired: 'CCCD/CMND là bắt buộc',
    addPhoto: 'Thêm ảnh',
    photoSpec: '4:5 · tối thiểu 600px',
    removePhoto: 'Xoá ảnh',
    fromOffer: 'Từ thư mời',
    prefillFromReq: 'Điền từ yêu cầu tuyển dụng →',
    recordPreview: 'Xem trước hồ sơ',
    unnamed: 'Hồ sơ chưa đặt tên',
    starts: 'Bắt đầu',
    contract: 'Hợp đồng',
    fullTime: 'Toàn thời gian',
    partTime: 'Bán thời gian',
    perMonth: 'tháng',
    reportsTo: 'Báo cáo cho',
    headcountAfter: 'Nhân sự sau khi thêm',
    beforeSubmit: 'Trước khi gửi',
    checkPersonal: 'Thông tin cá nhân đầy đủ',
    checkOffer: 'Có thư mời đã ký',
    checkOfferMissing: 'Có thư mời đã ký',
    checkEmployment: 'Phòng ban và ngày bắt đầu',
    checkPayroll: 'Thông tin lương và thuế',
    checkEquipment: 'Yêu cầu thiết bị',
    onSubmit: 'Khi gửi',
    onSubmitBody: 'Tạo hồ sơ nhân viên, thêm vào danh bạ và bảng chấm công, và thông báo cho bạn khi hoàn tất.',
    onSubmitManager: 'Nhân viên sẽ báo cáo cho {name}.',
    continueToEmployment: 'Tiếp tục sang thông tin việc làm →',
    continueToReview: 'Tiếp tục sang bước xem xét →',
    createRecord: 'Tạo hồ sơ nhân viên',
    saveAndClose: 'Lưu và đóng',
    createFailed: 'Không tạo được nhân viên. Vui lòng thử lại.',
    unexpectedError: 'Đã xảy ra lỗi không mong muốn.',
    nameRequired: 'Tên là bắt buộc',
    positionRequired: 'Vị trí là bắt buộc',
    departmentRequired: 'Phòng ban là bắt buộc',
    emailRequired: 'Email là bắt buộc',
    emailInvalid: 'Email không hợp lệ',
    phoneRequired: 'Số điện thoại là bắt buộc',
    dobRequired: 'Ngày sinh là bắt buộc',
    addressRequired: 'Địa chỉ là bắt buộc',
    startDateRequired: 'Ngày bắt đầu là bắt buộc',
    salaryRequired: 'Lương yêu cầu',
    performanceInvalid: 'Hiệu suất phải từ 0 đến 5',
    namePlaceholder: 'Nhập tên nhân viên',
    positionPlaceholder: 'ví dụ: senior_developer',
    departmentPlaceholder: 'ví dụ: engineering, finance',
    emailPlaceholder: 'nhanvien@example.com',
    phonePlaceholder: '+84 909 999 999',
    addressPlaceholder: 'Thành phố, Quốc gia',
    salaryPlaceholder: 'Nhập lương nhân viên',
    dob: 'Ngày sinh',
    address: 'Địa chỉ',
    personalInfo: 'Thông Tin Cá Nhân',
    employmentInfo: 'Thông Tin Việc Làm',
    review: 'Xem Xét & Gửi',
    personalInformation: 'Thông Tin Cá Nhân',
    employmentInformation: 'Thông Tin Việc Làm',
    reviewAndSubmit: 'Xem Xét & Gửi',
    selectPosition: 'Chọn Vị Trí',
    selectDepartment: 'Chọn Phòng Ban',
    photoTooLarge: 'Ảnh phải nhỏ hơn 5MB',
    invalidPhotoType: 'Chỉ cho phép tệp ảnh',
    successMessage: 'Thêm nhân viên thành công!',
    submitError: 'Thêm nhân viên thất bại',
    dobPlaceholder: 'Chọn ngày sinh'
  },
  // Employee Detail Modal
  employeeDetailModal: {
    download: 'Tải Xuống',
    uploadPdf: 'Tải Lên PDF',
    uploading: 'Đang tải lên...',
    delete: 'Xóa',
    basicInfo: 'Thông Tin Cơ Bản',
    contact: 'Liên Hệ',
    documents: 'Tài Liệu',
    pdfTab: 'PDF',
    requestDocsTab: 'Yêu Cầu',
    requestDocsRefresh: 'Làm Mới',
    requestDocsOpen: 'Mở',
    requestDocsPreview: 'Xem Trước',
    requestDocsDelete: 'Xóa',
    requestDocsOpenInNewTab: 'Mở trong tab mới',
    requestDocsClosePreview: 'Đóng xem trước',
    requestDocsLoading: 'Đang tải...',
    uploadRequestDoc: 'Tải Lên Tài Liệu',
    docCategory: 'Phân loại',
    categoryLeave: 'Đơn nghỉ phép',
    categoryOther: 'Khác',
    noRequestDocs: 'Chưa có tài liệu',
    requestDocsHint: 'Tải lên minh chứng nghỉ phép, chứng chỉ hoặc tài liệu hỗ trợ khác.',
    requestDocsRestricted: 'Chỉ Admin/HR/Manager mới có thể tải lên tài liệu yêu cầu.',
    requestDocUploaded: 'Tải lên tài liệu thành công!',
    quickStats: 'Thông Kê Nhanh',
    status: 'Trạng Thái',
    workDuration: 'Thời Gian Làm Việc',
    workDurationNA: 'N/A',
    yearUnit: 'năm',
    yearsUnit: 'năm',
    monthUnit: 'tháng',
    monthsUnit: 'tháng',
    durationUnitSeparator: ' ',
    durationPartSeparator: ' ',
    performance: 'Hiệu Suất',
    fullName: 'Họ và Tên',
    department: 'Phòng Ban',
    position: 'Vị Trí',
    dateOfBirth: 'Ngày Sinh',
    startDate: 'Ngày Bắt Đầu',
    salary: 'Lương',
    email: 'Email',
    phone: 'Điện Thoại',
    address: 'Địa Chỉ',
    uploadDocument: 'Tải Lên Tài Liệu',
    downloading: 'Đang tải xuống...',
    viewDocument: 'Xem Tài Liệu',
    deleteDocument: 'Xóa Tài Liệu',
    confirmDelete: 'Bạn có chắc chắn muốn xóa tài liệu này?',
    documentDeleted: 'Xóa tài liệu thành công',
    documentDeleteError: 'Xóa tài liệu thất bại',
    noDocument: 'Chưa có tài liệu',
    uploadInstructions: 'Nhấp hoặc kéo tệp PDF vào đây để tải lên',
    maxFileSize: 'Kích thước tệp tối đa: 10MB'
  },
  stats: {
    total: 'Tổng cộng',
    underReview: 'Đang Xem Xét',
    shortListed: 'Vào danh sách chọn lọc',
    interviewScheduled: 'Đã lên lịch phỏng vấn',
    offerExtended: 'Đã gửi thư mời làm việc',
    hired: 'Đã tuyển dụng',
    rejected: 'Từ chối'
  },
  // Recruitment
  recruitment: {
    position: 'Vị Trí',
    rating: 'Đánh Giá',
    statusLabel: 'Trạng Thái',
    actions: 'Hành Động',
    total: 'Tổng Cộng',
    shortListed: 'Danh Sách Chọn Lọc',
    interviews: 'Phỏng Vấn',
    offers: 'Đề Nghị',
    hired: 'Đã Tuyển Dụng',
    rejected: 'Từ Chối',
    title: 'Quy trình tuyển dụng',
    applications: 'Đơn ứng tuyển',
    candidate: 'Ứng viên',
    appliedDate: 'Ngày ứng tuyển',
    experience: 'Kinh nghiệm',
    stage: 'Giai đoạn',
    interviewScheduled: 'Đã lên lịch phỏng vấn',
    underReview: 'Đang Xem Xét',
    offerExtended: 'Đã đưa ra đề nghị',
    screening: 'Sàng lọc',
    technical: 'Kỹ thuật',
    offer: 'Đề nghị',
    years: 'Năm',
    yearsExperience: 'năm kinh nghiệm',
    jobTitle: 'Chức danh công việc',
    enterJobTitle: 'Nhập chức danh công việc',
    location: 'Địa điểm',
    enterLocation: 'Nhập địa điểm',
    employmentType: 'Hình thức làm việc',
    fullTime: 'Toàn thời gian',
    partTime: 'Bán thời gian',
    contract: 'Hợp đồng',
    internship: 'Thực tập',
    experienceLevel: 'Trình độ kinh nghiệm',
    enterExperience: 'ví dụ: 3–5 năm',
    salaryMin: 'Lương tối thiểu',
    salaryMax: 'Lương tối đa',
    description: 'Mô tả công việc',
    enterDescription: 'Nhập mô tả công việc',
    requirements: 'Yêu cầu',
    enterRequirements: 'Nhập yêu cầu công việc',
    postJob: 'Đăng tuyển',
    jobPosted: 'Đăng tuyển thành công!',
    statusUpdated: 'Cập nhật trạng thái thành công!',
    jobDetails: 'Chi tiết công việc',
    applicantInfo: 'Thông tin ứng viên',
    currentCompany: 'Công ty hiện tại',
    currentPosition: 'Vị trí hiện tại',
    education: 'Học Vấn',
    links: 'Liên Kết',
    department: 'Phòng Ban',
    postNewJob: 'Đăng Tuyển Công Việc Mới',
    viewResume: 'Xem CV',
    linkedinProfile: 'Hồ sơ LinkedIn',
    notes: 'Ghi chú',
    // Nested status translations
    status: {
      interviewscheduled: 'Đã lên lịch phỏng vấn',
      underreview: 'Đang xem xét',
      offerextended: 'Đã đưa ra đề nghị',
      hired: 'Đã tuyển dụng',
      rejected: 'Từ chối',
      shortlisted: 'Danh sách chọn lọc',
      pending: 'Đang chờ'
    },
    // Pipeline View
    subtitle: 'Quản lý quy trình tuyển dụng và theo dõi ứng viên',
    pipelineView: 'Quy trình',
    tableView: 'Bảng',
    candidates: 'ứng viên',
    moveTo: 'Chuyển đến',
    reject: 'Từ chối',
    // Pipeline stages
    pipeline: {
      title: 'Quy Trình Tuyển Dụng',
      clickToExpand: 'Nhấn vào các giai đoạn để xem ứng viên',
      noCandidates: 'Không có ứng viên trong giai đoạn này',
      screening: 'Sàng Lọc Hồ Sơ',
      screeningDesc: 'Xem xét đơn ứng tuyển ban đầu',
      shortlisted: 'Danh Sách Chọn Lọc',
      shortlistedDesc: 'Ứng viên đã vượt qua vòng sàng lọc',
      interview: 'Phỏng Vấn',
      interviewDesc: 'Biên bản phỏng vấn',
      offer: 'Đề Nghị Tuyển Dụng',
      offerDesc: 'Quyết định tuyển dụng',
      hired: 'Đã Tuyển Dụng',
      hiredDesc: 'Đã nhận việc thành công',
      rejected: 'Từ Chối',
      rejectedDesc: 'Không tiếp tục'
    },
    // Pipeline metrics
    metrics: {
      totalCandidates: 'Ứng viên',
      activeInPipeline: 'Đang xử lý',
      conversionRate: 'Chuyển đổi',
      avgTimeToHire: 'Thời gian tuyển',
      offersOut: 'Đề nghị đã gửi',
      openReqs: 'Vị trí đang mở'
    },
    // Board (2b)
    boardView: 'Bảng',
    viewMode: 'Chế độ xem',
    noApplications: 'Không tìm thấy hồ sơ nào',
    fieldsNotStored: 'Đã đăng tin, nhưng các trường sau không có trong bảng job_postings nên chưa được lưu: {fields}',
    board: {
      allDepartments: 'Tất cả phòng ban',
      openReqs: '{n} vị trí đang tuyển',
      inPlay: '{n} ứng viên đang xử lý',
      acceptedThisWeek: '{n} đề nghị được chấp nhận tuần này',
      passRate: '{n}% qua vòng',
      awaitingReply: '{n} đang chờ phản hồi',
      ofTarget: 'trên mục tiêu {n}',
      more: 'thêm {n} →',
      showLess: 'Thu gọn ↑',
      yearToDate: 'Từ đầu năm',
      ytdHires: '{n} lượt tuyển',
      ytdAvg: 'TB {n} ngày',
      createRecord: 'Tạo hồ sơ →',
      scorecard: 'Phiếu đánh giá',
      move: 'Chuyển',
      chase: 'Nhắc',
      revise: 'Sửa lại',
      panelToday: 'Phỏng vấn hôm nay {time}',
      panelOn: 'Phỏng vấn {date} {time}',
      panelUnscheduled: 'Chưa xếp lịch phỏng vấn',
      awaitingSent: 'Chờ phản hồi · đã gửi {n} ngày trước',
      tagCvParsed: 'Đã đọc CV',
      tagNeedsReview: 'Cần duyệt',
      tagAwaiting: 'Đang chờ',
      tagNegotiating: 'Đang thương lượng',
      tagSigned: 'Đã ký',
      tagScore: 'Điểm {n}'
    }
  },
  // Reports
  reports: {
    // ── Export spec sheet ──────────────────────────────────────────
    inScope: 'Trong phạm vi',
    range: 'Khoảng',
    people: 'Nhân sự',
    peopleInScope: 'nhân sự trong phạm vi',
    peopleWithoutRecords: 'Nhân sự không có dữ liệu trong kỳ',
    withoutRecords: 'không có dữ liệu',
    pendingTickerHint: 'Chấm công và nghỉ phép đang chờ duyệt',
    lastExport: 'Lần xuất gần nhất',
    by: 'bởi',
    selected: 'đã chọn',
    byUnit: 'Theo bộ phận',
    activeOnly: 'Chỉ đang làm việc',
    activeOnlyHint: 'Loại trừ nhân sự đã nghỉ việc',
    quarter: 'Quý',
    workingDays: 'ngày làm việc',
    byType: 'Theo loại',
    byStatus: 'Theo trạng thái',
    byWeek: 'Theo tuần',
    hoursShort: 'h',
    daysShort: 'ngày',
    timeAndLeave: 'chấm công & nghỉ phép',
    pendingApproval: 'Chờ duyệt',
    noApprovableRecords: 'Không có bản ghi cần duyệt trong phạm vi',
    beforeYouExport: 'Trước khi xuất',
    rowsReady: 'dòng sẵn sàng — không có vấn đề cần xử lý.',
    checkEntriesPending: 'bản ghi chấm công vẫn đang chờ duyệt',
    checkLeavePending: 'đơn nghỉ phép vẫn đang chờ duyệt',
    checkExportsAsPending: 'chúng sẽ được xuất với trạng thái CHỜ DUYỆT.',
    checkApproveFirst: 'Duyệt trước',
    checkOpenLeave: 'Mở nghỉ phép',
    checkPeopleNoRecords: 'nhân sự không có dữ liệu trong kỳ',
    checkOpenList: 'Xem danh sách',
    checkLongShifts: 'ngày chấm công trên 12 giờ cho một người',
    checkLongShiftsNote: 'thường là chấm công trùng chứ không phải ca làm thật.',
    checkLongShiftsShort: 'Trên 12h',
    checkRejectedRows: 'dòng bị từ chối trong phạm vi',
    checkRejectedNote: 'sẽ được xuất với trạng thái TỪ CHỐI.',
    checkReview: 'Xem lại',
    preview: 'Xem trước',
    rows: 'dòng',
    showing: 'Hiển thị',
    load: 'Tải thêm',
    sortedBy: 'Sắp xếp theo',
    chooseColumns: 'Chọn cột',
    previewColumns: 'Cột xem trước',
    previewColumnsNote: 'Chỉ ảnh hưởng bản xem trước. Tệp xuất luôn chứa đầy đủ các trường.',
    exportCarriesEveryRow: 'tệp xuất chứa mọi dòng trong phạm vi, không chỉ bản xem trước',
    unit: 'Bộ phận',
    approvedBy: 'Người duyệt',
    source: 'Nguồn',
    sourceTime: 'Chấm công',
    sourceLeave: 'Quản lý nghỉ phép',
    sourceTasks: 'Danh sách công việc',
    sourceGoals: 'Mục tiêu cá nhân',
    writes: 'Ghi tệp',
    recordTypes: 'loại bản ghi',
    estimated: 'ước tính',
    format: 'Định dạng',
    regular: 'Giờ thường',
    days: 'Ngày',
    bonusHours: 'Giờ Cộng Thêm',
    daysWorked: 'Số Ngày Làm Việc',
    leaveDays: 'Số Ngày Nghỉ Phép',
    title: 'Báo cáo và phân tích',
    filenamePrefix: 'Báo_Cáo_HR',
    excel: {
      charts: {
        hoursByType: 'Giờ theo loại',
        statusDistribution: 'Phân bổ trạng thái chấm công',
        taskStatusDistribution: 'Phân bổ trạng thái công việc',
        taskPriorityDistribution: 'Phân bổ ưu tiên công việc',
        goalStatusDistribution: 'Phân bổ trạng thái mục tiêu',
        goalCategoryDistribution: 'Phân bổ danh mục mục tiêu'
      },
      sheets: {
        summary: 'Tổng quan',
        performance: 'Hiệu suất nhân viên',
        allEmployeesOverview: 'Tổng quan tất cả nhân viên',
        charts: 'Biểu đồ & Chỉ số',
        timeEntries: 'Chấm công',
        tasks: 'Công việc',
        goals: 'Mục tiêu'
      },
      summaryTitle: 'Tóm tắt báo cáo nhân sự',
      generated: 'Ngày tạo',
      dateRange: 'Khoảng thời gian',
      employee: 'Nhân viên',
      reportLanguage: 'Ngôn ngữ báo cáo',
      visual: 'Trực quan',
      timeTracking: 'Tóm tắt theo dõi thời gian',
      workload: 'Tóm tắt khối lượng công việc',
      goals: 'Tóm tắt mục tiêu',
      metrics: {
        totalTimeEntries: 'Tổng số lượt ghi nhận thời gian',
        totalHours: 'Tổng số giờ đã ghi nhận',
        regularHours: 'Giờ Làm Thường',
        overtimeHours: 'Giờ Làm Thêm',
        wfhHours: 'Giờ làm việc tại nhà',
        pendingApprovals: 'Chờ phê duyệt',
        approvedEntries: 'Dữ liệu đã duyệt',
        totalTasks: 'Tổng số công việc',
        completedTasks: 'Công việc đã hoàn thành',
        inProgress: 'Đang thực hiện',
        highPriorityTasks: 'Công việc ưu tiên cao',
        estimatedHours: 'Ngày dự kiến',
        actualHours: 'Ngày thực tế',
        variance: 'Chênh lệch',
        totalGoals: 'Tổng số mục tiêu',
        completedGoals: 'Mục tiêu đã hoàn thành',
        avgProgress: 'Tiến độ trung bình'
      },
      performance: {
        header: 'Báo cáo hiệu suất',
        employeeInfo: 'Thông tin nhân viên',
        name: 'Họ tên',
        department: 'Phòng ban',
        position: 'Chức vụ',
        email: 'Email',
        reportPeriod: 'Kỳ báo cáo',
        performanceMetrics: 'Chỉ số hiệu suất',
        tableHeaders: {
          metric: 'Chỉ số',
          value: 'Giá trị',
          status: 'Trạng thái',
          notes: 'Ghi chú'
        },
        approvalRate: 'Tỷ lệ phê duyệt',
        taskCompletionRate: 'Tỷ lệ hoàn thành công việc',
        avgGoalProgress: 'Tiến độ mục tiêu trung bình',
        overallRating: 'Đánh giá hiệu suất chung',
        overallScore: 'Điểm hiệu suất tổng thể:',
        ratingLabel: 'Xếp loại:'
      },
      status: {
        high: 'Cao',
        normal: 'Bình thường',
        tracked: 'Đã theo dõi',
        pending: 'Đang chờ',
        allApproved: 'Đã duyệt tất cả',
        active: 'Đang hoạt động',
        noTasks: 'Không có công việc',
        excellent: 'Xuất sắc',
        good: 'Tốt',
        needsImprovement: 'Cần cải thiện',
        set: 'Đã thiết lập',
        onTrack: 'Đúng tiến độ',
        progressing: 'Đang tiến triển',
        behind: 'Trễ tiến độ'
      },
      rating: {
        outstanding: 'Xuất sắc',
        excellent: 'Rất tốt',
        good: 'Tốt',
        satisfactory: 'Đạt yêu cầu',
        needsImprovement: 'Cần cải thiện'
      },
      entries: 'mục',
      completedShort: 'đã hoàn thành',
      ofTotal: 'trên tổng số',
      headers: {
        dataType: 'Loại dữ liệu',
        type: 'Loại',
        hours: 'Giờ',
        status: 'Trạng thái',
        count: 'Số lượng',
        priority: 'Ưu tiên',
        employee: 'Nhân viên',
        department: 'Phòng ban',
        position: 'Chức vụ',
        date: 'Ngày',
        clockIn: 'Giờ vào',
        clockOut: 'Giờ ra',
        hourType: 'Loại giờ',
        notes: 'Ghi chú',
        createdAt: 'Ngày tạo',
        taskTitle: 'Tên công việc',
        description: 'Mô tả',
        dueDate: 'Hạn chót',
        estimatedHours: 'Ngày dự kiến',
        actualHours: 'Ngày thực tế',
        variance: 'Chênh lệch',
        goalTitle: 'Tên mục tiêu',
        category: 'Danh mục',
        progress: 'Tiến độ (%)',
        targetDate: 'Ngày mục tiêu',
        updatedAt: 'Cập nhật cuối'
      }
    },
    description: 'Tạo và xuất báo cáo chi tiết về nhân sự, hiệu suất và tuyển dụng',
    exportReport: 'Xuất báo cáo',
    selectReportType: 'Chọn loại báo cáo',
    selectEmployee: 'Chọn nhân viên',
    selectDepartment: 'Chọn phòng ban',
    applyFilters: 'Áp dụng bộ lọc',
    resetFilters: 'Đặt lại bộ lọc',
    exportFormat: 'Định dạng xuất',
    formatPdf: 'PDF',
    formatExcel: 'Excel',
    formatCsv: 'CSV',
    generating: 'Đang tạo báo cáo...',
    exporting: 'Đang xuất báo cáo...',
    reportGenerated: 'Báo cáo đã được tạo thành công!',
    reportExported: 'Báo cáo đã được xuất thành công!',
    totalOvertime: 'Tổng giờ làm thêm',
    avgPerformance: 'Hiệu suất trung bình',
    applicationsReceived: 'Đơn ứng tuyển nhận được',
    interviewsScheduled: 'Phỏng vấn đã lên lịch',
    offersExtended: 'Đề nghị đã đưa ra',
    reportsGenerated: 'Báo cáo đã tạo',
    reportsExported: 'Báo cáo đã xuất',
    dateGenerated: 'Ngày tạo',
    organization: 'Tổ chức',
    total: 'Tổng cộng',
    overtimeHours: 'Giờ Làm Thêm',
    performanceRating: 'Đánh giá hiệu suất',
    newHiresCount: 'Số nhân viên mới',
    applicationsCount: 'Số đơn ứng tuyển',
    interviewsCount: 'Số phỏng vấn',
    offersCount: 'Số đề nghị',
    dateRangeLabel: 'Khoảng thời gian',
    reportSummary: 'Tóm tắt báo cáo',
    keyMetrics: 'Chỉ số chính',
    totalHoursWorked: 'Tổng số giờ làm việc',
    totalOvertimeWorked: 'Tổng giờ làm thêm',
    averagePerformanceRating: 'Đánh giá hiệu suất trung bình',
    newEmployeesHired: 'Nhân viên mới được tuyển dụng',
    recruitmentActivities: 'Hoạt động tuyển dụng',
    applicationsReceivedCount: 'Số đơn ứng tuyển nhận được',
    interviewsConducted: 'Số phỏng vấn đã thực hiện',
    offersMade: 'Số đề nghị đã đưa ra',
    detailedMetrics: 'Chỉ số chi tiết',
    employeeName: 'Tên nhân viên',
    hoursWorked: 'Giờ làm việc',
    overtimeWorked: 'Giờ làm thêm',
    performanceScore: 'Điểm hiệu suất',
    hireDate: 'Ngày tuyển dụng',
    applicationDate: 'Ngày ứng tuyển',
    interviewDate: 'Ngày phỏng vấn',
    offerDate: 'Ngày đề nghị',
    language: 'Ngôn Ngữ',
    personalGoals: 'Mục Tiêu Cá Nhân',
    actions: 'Hành động',
    view: 'Xem',
    edit: 'Chỉnh sửa',
    delete: 'Xóa',
    reportTypes: 'Loại báo cáo',
    individualMetrics: 'Chỉ số cá nhân',
    organizationMetrics: 'Chỉ số tổ chức',
    timeEntries: 'Giờ làm việc',
    custom: 'Tùy chỉnh',
    export: 'Xuất báo cáo',
    exportScopeNote: 'Tệp xuất chứa toàn bộ bản ghi đã lọc, không chỉ 50 dòng xem trước.',
    leaveType: 'Loại',
    rate: 'Tỷ lệ',
    customReports: 'Báo cáo tùy chỉnh',
    createNewReport: 'Tạo báo cáo mới',
    selectCriteria: 'Chọn tiêu chí báo cáo',
    reportName: 'Tên báo cáo',
    reportNamePlaceholder: 'Nhập tên báo cáo',
    last7Days: '7 ngày qua',
    last30Days: '30 ngày qua',
    timeEntrys: 'Báo cáo giờ làm việc',
    attendanceRecords: 'Hồ sơ chấm công',
    performanceSummary: 'Tóm tắt hiệu suất',
    recruitmentOverview: 'Tổng quan tuyển dụng',
    performanceReport: 'Báo Cáo Hiệu Suất',
    recruitmentReport: 'Báo cáo tuyển dụng',
    customReport: 'Báo cáo tùy chỉnh',
    exportToPDF: 'Xuất sang PDF',
    reportParameters: 'Tham số báo cáo',
    individualReport: 'Báo cáo cá nhân',
    selectDateRange: 'Chọn khoảng thời gian',
    loadingReport: 'Đang tạo báo cáo...',
    exportingIncludes: 'Xuất bao gồm dữ liệu đang lọc và sắp xếp',
    prebuiltReports: 'Báo cáo có sẵn',
    reportDashboard: 'Bảng điều khiển báo cáo',
    selectReport: "Chọn báo cáo để xem chi tiết",
    employeeGrowth: 'Tăng trưởng nhân viên',
    departmentDistribution: 'Phân bố theo phòng ban',
    performanceMetrics: 'Chỉ số hiệu suất',
    salaryReport: "Báo cáo lương",
    generateReport: "Tạo báo cáo",
    exportPDF: 'Xuất PDF',
    exportExcel: 'Xuất Excel',
    filterBy: 'Lọc theo',
    allDepartments: "Tất cả phòng ban",
    human_resources: 'Nhân Sự',
    finance: 'Tài Chính',
    engineering: 'Kỹ Thuật',
    sales: 'Bán Hàng',
    marketing: 'Tiếp Thị',
    performance: "Hiệu suất",
    salary: "Lương",
    attendance: "Chấm Công",
    recruitment: "Tuyển Dụng",
    department: "Phòng Ban",
    all: 'Tất Cả',
    overview: 'Tổng Quan',
    detailedReports: 'Báo Cáo Chi Tiết',
    filters: 'Bộ Lọc',
    exportAll: 'Xuất Tất Cả',
    departmentOverview: 'Tổng Quan Phòng Ban',
    attendanceOverview: 'Tổng Quan Chấm Công',
    recruitmentFunnel: 'Phễu Tuyển Dụng',
    performanceTrend: 'Xu Hướng Hiệu Suất',
    totalEmployees: "Tổng Số Nhân Viên",
    newHires: "Nhân Viên Mới",
    turnoverRate: 'Tỷ Lệ Nghỉ Việc',
    annualRate: 'Tỷ Lệ Hàng Năm',
    avgSalary: 'Lương Trung Bình',
    satisfaction: 'Hài Lòng',
    productivity: 'Năng Suất',
    present: 'Có Mặt',
    onLeave: 'Nghỉ Phép',
    absent: 'Vắng Mặt',
    totalApplications: 'Tổng Đơn Ứng Tuyển',
    interviewed: 'Đã Phỏng Vấn',
    hired: 'Được Tuyển',
    rejected: "Bị Từ Chối",
    employees: 'Nhân Viên',
    generateCustomReport: "Tạo Báo Cáo Tùy Chỉnh",
    reportType: "Loại Báo Cáo",
    employeePerformance: "Hiệu Suất Nhân Viên",
    salaryAnalysis: "Phân Tích Lương",
    attendanceReport: "Báo Cáo Chấm Công",
    recruitmentMetrics: "Chỉ Số Tuyển Dụng",
    departmentComparison: "So Sánh Phòng Ban",
    pdf: {
      visualAnalytics: 'Phân Tích Trực Quan',
      charts: {
        hoursByType: 'Giờ theo loại',
        statusDistribution: 'Phân bổ trạng thái chấm công',
        taskStatusDistribution: 'Phân bổ trạng thái công việc',
        taskPriorityDistribution: 'Phân bổ ưu tiên công việc',
        goalStatusDistribution: 'Phân bổ trạng thái mục tiêu',
        goalCategoryDistribution: 'Phân bổ danh mục mục tiêu'
      },
      headers: {
        employee: 'Nhân viên',
        department: 'Phòng ban',
        position: 'Chức vụ',
        date: 'Ngày',
        clockIn: 'Giờ vào',
        clockOut: 'Giờ ra',
        hours: 'Số giờ',
        hourType: 'Loại giờ',
        status: 'Trạng thái',
        notes: 'Ghi chú',
        createdAt: 'Ngày tạo',
        taskTitle: 'Tên nhiệm vụ',
        description: 'Mô tả',
        priority: 'Độ ưu tiên',
        dueDate: 'Hạn chót',
        estimatedHours: 'Ngày ước tính',
        actualHours: 'Ngày thực tế',
        variance: 'Chênh lệch',
        goalTitle: 'Tên mục tiêu',
        category: 'Danh mục',
        progress: 'Tiến độ (%)',
        targetDate: 'Ngày mục tiêu',
        updatedAt: 'Cập nhật cuối'
      }
    },
    exportToExcel: "Xuất Sang Excel",
    monthlyPerformanceReview: "Đánh Giá Hiệu Suất Hàng Tháng",
    comprehensivePerformanceAnalysis: "Phân Tích Hiệu Suất Toàn Diện",
    salaryBenchmarking: "Đánh Giá Mức Lương",
    compareSalariesAcrossDepartments: "So Sánh Lương Giữa Các Phòng Ban",
    attendanceAnalytics: "Phân Tích Chấm Công",
    trackAttendancePatterns: "Theo Dõi Mô Hình Và Xu Hướng Chấm Công",
    recruitmentPipeline: "Quy Trình Tuyển Dụng",
    monitorHiringProcess: "Giám Sát Hiệu Quả Quy Trình Tuyển Dụng",
    employeeTurnover: "Nghỉ Việc Nhân Viên",
    analyzeRetentionRates: "Phân Tích Tỷ Lệ Giữ Chân Và Nghỉ Việc",
    trainingEffectiveness: "Hiệu Quả Đào Tạo",
    measureTrainingSuccess: "Đo Lường Thành Công Chương Trình Đào Tạo",
    generate: "Tạo báo cáo",
    fromLastPeriod: 'Từ Kỳ Trước',
    lastWeek: 'Tuần Trước',
    lastMonth: 'Tháng Trước',
    lastQuarter: "Quý Trước",
    lastYear: "Năm Ngoái",
    customRange: "Phạm Vi Tùy Chỉnh",
    // Date Range Options
    today: 'Hôm Nay',
    thisWeek: 'Tuần Này',
    thisMonth: 'Tháng Này',
    thisQuarter: "Quý Này",
    thisYear: 'Năm Nay',
    // Data Types
    totalEntries: 'Chấm Công',
    tasks: 'Nhiệm Vụ',
    goals: "Mục Tiêu Cá Nhân",
    employee: "Nhân Viên",
    dateRange: "Phạm Vi Thời Gian",
    dataType: "Loại Dữ Liệu",
    quickFilters: "Bộ Lọc Nhanh",
    allEmployees: "Tất Cả Nhân Viên",
    startDate: "Ngày Bắt Đầu",
    endDate: "Ngày Kết Thúc",
    recordsFound: "bản ghi được tìm thấy",
    noData: "Không có dữ liệu để hiển thị cho các tiêu chí đã chọn",
    adjustFilters: 'Thử điều chỉnh bộ lọc hoặc phạm vi ngày',
    dataPreview: 'Xem Trước Dữ Liệu',
    showingFirst50: 'Hiển thị 50 bản ghi đầu tiên',
    showingFirst50of: 'Hiển thị 50 đầu tiên trong',
    records: 'bản ghi',
    exportForAll: 'Xuất CSV để lấy tất cả bản ghi.',
    liveData: 'Dữ liệu trực tiếp từ Cơ Sở Dữ Liệu',
    subtitle: 'Xuất dữ liệu toàn diện cho chấm công, nhiệm vụ và mục tiêu cá nhân',
    errorExporting: 'Lỗi xuất dữ liệu',
    exportToCSV: "Xuất sang CSV",
    exportSuccess: 'Xuất dữ liệu thành công',
    csvExportSuccess: 'Xuất báo cáo CSV thành công với tất cả loại dữ liệu trong một tệp!',
    excelExportHint: 'Xuất tất cả loại dữ liệu kèm tóm tắt, biểu đồ và bảng chi tiết',
    pdfExportHint: 'Xuất PDF với biểu đồ trực quan, tóm tắt và bảng chi tiết cho tất cả loại dữ liệu',
    pdfExportSuccess: 'Xuất báo cáo PDF thành công!',
    reportPeriod: 'Kỳ báo cáo',
    to: "đến",
    totalHours: "Tổng số giờ",
    regularHours: "Giờ Làm Thường",
    wfh: 'Làm Việc Tại Nhà',
    overtime: 'Giờ Làm Thêm',
    leave: 'Yêu Cầu Nghỉ Phép',
    tasksDone: 'Công Việc Đã Hoàn Thành',
    completion: 'Mục Tiêu Cá Nhân Đã Hoàn Thành',
    goalProgress: 'Tiến độ mục tiêu',
    approved: "Đã phê duyệt",
    pending: "Chờ duyệt",
    completed: "Hoàn thành",
    inProgress: "Đang thực hiện",
    completionRate: "Tỷ lệ hoàn thành",
    avgProgress: "Tiến độ trung bình",
    totalGoals: 'Tổng mục tiêu',
    type: "Loại báo cáo",
    details: 'Chi tiết',
    status: "Trạng thái",
    date: 'Ngày',
    for: 'cho',
    from: "từ",
    // PDF Export specific translations
    generated: 'Tạo Lúc',
    period: 'Kỳ',
    summaryOverview: 'Tổng Quan',
    totalRecords: 'Tất Cả',
    achievedGoals: 'Mục tiêu đạt được',
    completedTasks: 'Nhiệm vụ hoàn thành',
    unknown: 'Không xác định',
    task: "Nhiệm vụ",
    priority: "Ưu tiên",
    dueDate: "Ngày đến hạn",
    goal: "Mục tiêu",
    category: "Danh mục",
    progress: "Tiến độ",
    clockIn: 'Giờ vào',
    clockOut: 'Giờ ra',
    hours: 'Giờ',
    est: 'Ước tính',
    actual: 'Thực tế',
    targetDate: 'Ngày mục tiêu',
    page: 'Trang',
    of: 'trên',
    generatedBy: "Được tạo bởi Hệ thống Quản lý HR",
    achieved: "Đã đạt được",
    priorityLow: 'Thấp',
    priorityMedium: 'Trung bình',
    priorityHigh: 'Cao',
    statusApproved: 'Đã duyệt',
    statusRejected: 'Bị từ chối',
    statusPending: 'Chờ xử lý',
    statusCompleted: 'Hoàn thành',
    statusInProgress: 'Đang thực hiện',
    statusNotStarted: 'Chưa bắt đầu'
  },
  // Status translations
  status: {
    approved: 'Đã duyệt',
    pending: 'Chờ xử lý',
    rejected: 'Bị từ chối',
    completed: 'Hoàn thành',
    active: 'Đang hoạt động',
    delayed: 'Bị trì hoãn',
    'in-progress': 'Đang thực hiện',
    'not-started': 'Chưa bắt đầu',
    achieved: 'Đã đạt được',
    'on-hold': 'Tạm dừng'
  },
  // Data Type Labels
  dataType: {
    timeEntry: 'Bảng chấm công',
    task: 'Nhiệm vụ',
    goal: 'Mục tiêu'
  },
  // Time Clock Entry
  timeClock: {
    title: 'Chấm công',
    subtitle: 'Ghi lại giờ làm việc và gửi chứng từ',
    newEntry: 'Nhập giờ công mới',
    date: 'Ngày',
    uploadFile: 'Tải Lên Tệp',
    fileTypes: 'Hỗ trợ: Hình ảnh, PDF, Văn Bản (Kích thước tối đa 50MB)',
    selectDate: 'Chọn ngày',
    clockIn: 'Giờ vào',
    clockOut: 'Giờ ra',
    hourType: 'Loại giờ công',
    proof: 'Chứng từ chấm công',
    notes: 'Ghi chú',
    optional: 'Tùy chọn',
    submit: 'Gửi giờ công',
    submitting: 'Đang gửi...',
    success: 'Đã gửi giờ công thành công!',
    confirmDelete: 'Bạn có chắc muốn xóa bản ghi này?',
    delete: 'Xóa',
    deleteOptions: 'Chọn tùy chọn xóa',
    deleteEntry: 'Xóa toàn bộ bản ghi',
    deleteProofOnly: 'Chỉ xóa tệp chứng từ',
    enterChoice: 'Nhập 1 hoặc 2',
    invalidChoice: 'Lựa chọn không hợp lệ. Vui lòng nhập 1 hoặc 2.',
    proofDeleteSuccess: 'Đã xóa tệp chứng từ thành công',
    proofDeleteError: 'Không thể xóa tệp chứng từ',
    deleteSuccess: 'Đã xóa bản ghi thành công',
    history: 'Lịch sử chấm công',
    noEntries: 'Chưa có bản ghi nào. Hãy gửi bản ghi đầu tiên!',
    time: 'Thời gian',
    hours: 'Giờ',
    type: 'Loại',
    status: 'Trạng thái',
    actions: 'Thao tác',
    weeklySummary: 'Tuần này',
    monthlySummary: 'Tháng này',
    leaveDays: 'Ngày nghỉ phép',
    requestLeave: 'Yêu cầu nghỉ phép',
    thisWeek: 'Tuần này',
    thisMonth: 'Tháng này',
    total: 'Tổng cộng',
    hrs: 'giờ',
    days: 'ngày',
    employee: 'Nhân Viên',
    includesPending: '* Bao gồm đang chờ và đã duyệt',
    notesPlaceholder: 'Thêm ghi chú hoặc nội dung bổ sung...',
    fileHelp: 'Tải lên ảnh chụp màn hình, ảnh hoặc PDF (tối đa 5MB)',
    // Khung màn hình 3a — thanh chỉ số, bộ chuyển phạm vi, hệ quả nhập liệu, chân bảng
    awaitingApproval: 'Chờ duyệt',
    missingProof: 'Thiếu chứng từ',
    dayShort: 'đ',
    scopeMine: 'Của tôi',
    scopeAdmin: 'Quản trị',
    scopeBulk: 'Hàng loạt',
    entryHint: 'Giờ công được gửi để duyệt. Đính kèm chứng từ ngay, hoặc từ bảng lịch sử bên dưới.',
    autoDetectNote: 'Cuối tuần và ngày lễ được suy ra từ ngày — chỉ đổi loại khi hôm đó là ngoại lệ.',
    adminTag: 'Quản trị',
    filingFor: 'Nhập cho: {name}',
    changeEmployee: 'Đổi nhân viên',
    willRecord: 'Mục này sẽ ghi',
    weekBecomes: 'Tuần này sẽ thành',
    remainingOf: 'Còn lại của {n}h',
    fillStandard: 'Điền {from} – {to}',
    midnightNote: 'Ca qua nửa đêm được tính vào ngày bắt đầu ca.',
    proofMissing: 'Thiếu',
    proofLoadFailed: 'Không thể hiển thị tệp này.',
    showingCount: 'Hiển thị {n} của {total} mục',
    showAll: 'Xem tất cả',
    otherTypes: '{n} loại còn lại',
    bulkExplain: 'Ghi một ngày chuẩn {from}–{to} cho mọi nhân viên đang hoạt động trong khoảng ngày. Cuối tuần được bỏ qua.',
    openBulk: 'Mở nhập hàng loạt',
    bulkScope: '{n} nhân viên đang hoạt động',
    filterByStatus: 'Trạng thái',
    allStatuses: 'Tất cả',
    noPendingEntries: 'Không có bản ghi nào đang chờ duyệt',
    reviewPendingBanner: 'Đang hiển thị các bản ghi chờ bạn duyệt',
    hourTypes: {
      regular: 'Giờ Làm Thường',
      holiday: 'Ngày Lễ',
      weekend: 'Cuối Tuần',
      overtime: 'Giờ Làm Thêm',
      bonus: 'Giờ Làm Hiệu Suất Cao',
      wfh: 'Làm Việc Tại Nhà (Trực Tuyến)',
      onLeave: 'Nghỉ Phép'
    },
    errors: {
      dateRequired: 'Vui lòng chọn ngày',
      clockInRequired: 'Vui lòng nhập giờ vào',
      clockOutRequired: 'Vui lòng nhập giờ ra',
      clockOutAfterClockIn: 'Giờ ra phải sau giờ vào',
      tooManyHours: 'Không thể vượt quá 24 giờ trong một bản ghi',
      overlapping: 'Thời gian này trùng với bản ghi khác trong cùng ngày',
      fileTooLarge: 'Kích thước tệp phải nhỏ hơn 50MB',
      invalidFileType: 'Chỉ cho phép tệp hình ảnh, PDF và tài liệu',
      submitFailed: 'Gửi giờ công thất bại. Vui lòng thử lại.',
      deleteFailed: 'Xóa giờ công thất bại. Vui lòng thử lại.',
      duplicateEntry: 'Bạn đã có mục nhập {hourType} cho ngày này. Vui lòng chỉnh sửa mục nhập hiện tại hoặc chọn loại giờ khác.'
    },
    proofUploadSuccess: 'Tải lên tệp chứng minh thành công',
    proofUploadError: 'Tải lên tệp chứng minh thất bại',
    uploadProof: 'Tải lên tệp chứng minh',
    viewProof: 'Xem hình ảnh chứng minh',
    downloadProof: 'Tải xuống tệp chứng minh',
    approve: 'Phê Duyệt',
    approvalSuccess: 'Phê duyệt giờ công thành công',
    approvalError: 'Phê duyệt giờ công thất bại',
    viewEntries: 'Xem Bản Ghi',
    myEntries: 'Bản Ghi Của Tôi',
    allEmployees: 'Tất Cả Nhân Viên',
    specificEmployee: 'Nhân Viên Cụ Thể'
  },
  // Common
  datePicker: {
    placeholder: 'Chọn ngày',
    calendar: 'Lịch',
    prevMonth: 'Tháng trước',
    nextMonth: 'Tháng sau',
    today: 'Hôm nay',
    clear: 'Xóa'
  },
  timePicker: {
    period: 'SA/CH',
    hours: 'giờ',
    minutes: 'phút',
    done: 'Xong',
    clear: 'Xóa',
    placeholder: '--:--',
    picker: 'Chọn giờ'
  },
  routeError: {
    message: 'Không mở được màn hình này. Phần còn lại của ứng dụng vẫn hoạt động — hãy chọn trang khác từ menu, hoặc tải lại.'
  },

  common: {
    notAvailable: 'Không có',
    status: 'Trạng thái',
    refresh: 'Làm mới',
    reload: 'Tải lại',
    previous: 'Trước',
    retry: 'Thử lại',
    viewDetails: 'Xem Chi Tiết',
    email: 'Email',
    phone: 'Điện Thoại',
    address: 'Địa Chỉ',
    date: 'Ngày',
    time: 'Thời Gian',
    submit: 'Gửi',
    submitting: 'Đang Gửi...',
    update: 'Cập Nhật',
    updating: 'Đang Cập Nhật...',
    results: 'Kết Quả',
    total: 'Tổng',
    employees: 'Nhân Viên',
    departments: 'Phòng Ban',
    hours: 'Giờ',
    leaveRequest: 'Yêu Cầu Nghỉ Phép',
    search: 'Tìm kiếm',
    filter: 'Lọc',
    sort: 'Sắp xếp',
    next: 'Tiếp',
    back: 'Quay lại',
    type: 'Phân Loại',
    save: 'Lưu',
    saving: 'Đang Lưu...',
    cancel: 'Hủy',
    delete: 'Xóa',
    edit: 'Chỉnh sửa',
    view: 'Xem',
    add: 'Thêm',
    close: 'Đóng',
    loading: 'Đang tải...',
    noData: 'Không có dữ liệu',
    error: 'Đã xảy ra lỗi',
    success: 'Thao tác thành công',
    confirm: 'Bạn có chắc chắn?',
    yes: 'Có',
    no: 'Không',
    accessDenied: 'Truy Cập Bị Từ Chối',
    noPermission: 'Bạn không có quyền truy cập trang này.',
    goBack: 'Quay Lại',
    week: 'Tuần',
    weeks: 'Tuần',
    month: 'Tháng',
    months: 'Tháng',
    quarter: 'Quý',
    quarters: 'Quý',
    year: 'Năm',
    years: 'Năm',
    day: 'Ngày',
    days: 'Ngày',
    hour: 'Giờ',
    minute: 'Phút',
    second: 'Giây',
    today: 'Hôm Nay',
    thisWeek: 'Tuần Này',
    thisMonth: 'Tháng Này',
    thisYear: 'Năm Này',
    allTime: 'Tất Cả Thời Gian',
    select: 'Chọn...',
    upload: 'Tải Lên',
    download: 'Tải Xuống',
    reset: 'Đặt Lại',
    optional: 'Tùy Chọn',
    fetching: 'Đang tải',
    ready: 'Xong',
    ms: 'ms',
    approve: 'Duyệt',
    decline: 'Từ Chối',
    pending: 'Chờ Duyệt',
    unknown: 'Không Rõ',
    done: 'Xong',
    noDescription: 'Chưa có mô tả',
    export: 'Xuất',
    all: 'Tất Cả'
  },
  // Theme
  theme: {
    light: 'Chế độ sáng',
    dark: 'Chế độ tối',
    toggle: 'Chuyển đổi chủ đề'
  },
  // Language
  language: {
    select: 'Chọn ngôn ngữ',
    current: 'Ngôn ngữ hiện tại'
  },
  // Time Tracking Actions
  timeTrackingActions: {
    recordTime: 'Ghi lại thời gian',
    requestLeave: 'Yêu cầu nghỉ phép',
    logOvertime: 'Ghi lại giờ làm thêm',
    exportReport: 'Xuất báo cáo'
  },
  // Recruitment Actions
  recruitmentActions: {
    view: 'Xem',
    schedule: 'Lên lịch',
    reject: 'Từ chối'
  },
  // Employee Status
  employeeStatus: {
    active: 'Hoạt động',
    inactive: 'Không hoạt động',
    onleave: 'Nghỉ phép',
    pending: 'Chờ xử lý',
    outSourceContractor: 'Nhà thầu gia công'
  },
  //Employee Position 
  employeePosition: {
    general_manager: 'Quản Lý',
    senior_developer: 'Lập Trình Viên Cao Cấp',
    hr_specialist: 'Điều Hành Nhân Sự',
    accountant: 'Kế Toán Trưởng',
    contract_manager: 'Phụ Trách Đấu Thầu',
    managing_director: 'Viện Trưởng',
    support_staff: 'Hỗ Trợ',
    expertGroup: 'Khối Chuyên Gia',
    employee: 'Nhân Viên'
  },
  // Notifications
  notifications: {
    title: 'Thông báo',
    // Chrome của sổ thông báo
    shown: 'Đang hiện',
    nShownOf: 'Hiện {shown} trên {total}',
    ofN: 'trên {n}',
    byCategory: 'Theo nhóm',
    byType: 'Theo loại',
    leavingMarksRead: 'Rời khỏi màn hình này sẽ đánh dấu mọi mục đang hiện là đã đọc.',
    loadMore: 'Tải thêm',
    loadingMore: 'Đang tải...',
    employeeAdded: 'Nhân viên đã được thêm',
    addedTo: 'được thêm vào',
    unreadCount: '{0} chưa đọc',
    allCaughtUp: 'Bạn đã cập nhật!',
    filters: 'Lọc',
    markAllRead: 'Đánh dấu tất cả đã đọc',
    deleteAll: 'Xóa tất cả',
    confirmDeleteAll: 'Bạn có chắc chắn muốn xóa tất cả thông báo?',
    total: 'Tổng',
    unread: 'Chưa đọc',
    errors: 'Lỗi',
    warnings: 'Cảnh báo',
    filterBy: 'Lọc',
    status: 'Trạng thái',
    type: 'Loại',
    category: 'Thể loại',
    allNotifications: 'Tất cả thông báo',
    unreadOnly: 'Chưa đọc',
    readOnly: 'Đã đọc',
    allTypes: 'Tất cả loại',
    info: 'Thông tin',
    success: 'Thành công',
    warning: 'Cảnh báo',
    error: 'Lỗi',
    allCategories: 'Tất cả thể loại',
    general: 'Chung',
    timeTracking: 'Theo dõi thời gian',
    performance: 'Hiệu suất',
    employee: 'Nhân viên',
    recruitment: 'Tuyển dụng',
    system: 'Hệ thống',
    noNotifications: 'Không có thông báo',
    noNotificationsFilter: 'Không có thông báo phù hợp với bộ lọc',
    noNotificationsYet: 'Bạn chưa có thông báo',
    justNow: 'Vừa rồi',
    minutesAgo: 'trước {0} phút',
    hoursAgo: 'trước {0} giờ',
    daysAgo: 'trước {0} ngày',
    markAsRead: 'Đánh dấu đã đọc',
    delete: 'Xóa',
    // Notification titles
    pendingApprovals: 'Đang Chờ Phê Duyệt',
    timeEntryApproved: 'Thời Gian Đã Được Duyệt',
    timeEntryRejected: 'Thời Gian Bị Từ Chối',
    newEmployeeAdded: 'Nhân Viên Mới Được Thêm',
    performanceReview: 'Đánh Giá Hiệu Suất',
    systemUpdate: 'Cập Nhật Hệ Thống',
    // Notification messages
    timeEntriesAwaiting: 'Bạn có {0} mục thời gian đang chờ phê duyệt',
    reviewNow: 'Xem Ngay',
    viewDetails: 'Xem Chi Tiết',
    viewAll: 'Xem Tất Cả Thông Báo',
  },
  // Recruitment Status
  recruitmentStatus: {
    active: 'Hoạt động',
    interviewscheduled: 'Đã lên lịch phỏng vấn',
    interviewScheduled: 'Đã lên lịch phỏng vấn',
    underreview: 'Đang xem xét',
    underReview: 'Đang xem xét',
    offerextended: 'Đã đề nghị',
    offerExtended: 'Đề nghị đã gửi',
    offer: 'Đề nghị',
    hired: 'Đã tuyển',
    rejected: 'Từ chối',
    screening: 'Sàng lọc',
    technical: 'Kỹ thuật',
    shortlisted: 'Vào danh sách rút gọn'
  },
  // Performance Goals
  goals: {
    completeReactProject: 'Hoàn thành dự án React',
    improveCodeQuality: 'Cải thiện chất lượng code',
    mentoringJuniorDevelopers: 'Hướng dẫn lập trình viên mới',
    apiDevelopment: 'Phát triển API',
    databaseOptimization: 'Tối ưu hóa cơ sở dữ liệu',
    backendDevelopment: 'Phát triển backend',
    teamCollaboration: 'Hợp tác nhóm'
  },
  // Review Types
  reviewTypes: {
    quarterlyReview: 'Đánh giá theo quý',
    midYearReview: 'Đánh giá giữa năm',
    annualReview: 'Đánh giá hàng năm'
  },
  // Skill Categories
  skillCategories: {
    technical: 'Kỹ thuật',
    soft: 'Kỹ năng mềm',
    leadership: 'Lãnh đạo',
    communication: 'Giao tiếp'
  },
  // Settings
  settings: {
    title: 'Cài Đặt',
    subtitle: 'Quản lý tùy chọn và cài đặt tài khoản của bạn',
    saved: 'Đã lưu!',
    export: 'Xuất',
    import: 'Nhập',
    reset: 'Đặt lại',
    saveChanges: 'Lưu Thay Đổi',
    confirmReset: 'Bạn có chắc chắn muốn đặt lại tất cả cài đặt về mặc định không?',
    importSuccess: 'Đã nhập cài đặt thành công!',
    importError: 'Không thể nhập cài đặt',
    // Tabs
    notifications: 'Thông Báo',
    appearance: 'Giao Diện',
    language: 'Ngôn Ngữ',
    privacy: 'Riêng Tư',
    work: 'Tùy Chọn Công Việc',
    // Notification Settings
    notificationPreferences: 'Tùy Chọn Thông Báo',
    emailNotifications: 'Thông Báo Email',
    emailNotificationsDesc: 'Nhận thông báo qua email',
    pushNotifications: 'Thông Báo Đẩy',
    pushNotificationsDesc: 'Nhận thông báo đẩy trong ứng dụng',
    desktopNotifications: 'Thông Báo Trên Desktop',
    desktopNotificationsDesc: 'Hiển thị thông báo trình duyệt trên desktop',
    notificationFrequency: 'Tần Suất Thông Báo',
    realtime: 'Thời gian thực',
    daily: 'Tổng hợp hàng ngày',
    weekly: 'Tóm tắt hàng tuần',
    notifyMeAbout: 'Thông báo cho tôi về',
    timeTrackingNotifications: 'Theo dõi thời gian',
    performanceNotifications: 'Đánh giá hiệu suất',
    employeeNotifications: 'Cập nhật nhân viên',
    recruitmentNotifications: 'Tuyển dụng',
    systemNotifications: 'Cập nhật hệ thống',
    // Appearance Settings
    appearanceSettings: 'Cài Đặt Giao Diện',
    theme: 'Chủ đề',
    dateFormat: 'Định dạng ngày',
    timeFormat: 'Định dạng giờ',
    itemsPerPage: 'Số mục trên mỗi trang',
    // Language & Region
    languageRegion: 'Ngôn ngữ & Khu vực',
    timezone: 'Múi giờ',
    timezoneNote: 'Hiện đang hiển thị 50 múi giờ đầu tiên',
    // Privacy Settings
    privacySettings: 'Cài Đặt Riêng Tư',
    profileVisibility: 'Hiển thị hồ sơ',
    visibilityAll: 'Mọi người',
    visibilityTeam: 'Nhóm của tôi',
    visibilityManagers: 'Chỉ quản lý',
    visibilityPrivate: 'Riêng tư',
    contactVisibility: 'Hiển thị thông tin liên hệ',
    showEmail: 'Hiển thị địa chỉ email',
    showPhone: 'Hiển thị số điện thoại',
    // Work Preferences
    workPreferences: 'Tùy Chọn Công Việc',
    defaultDashboard: 'Chế độ xem bảng điều khiển mặc định',
    overviewView: 'Tổng quan',
    detailedView: 'Chi tiết',
    compactView: 'Nhỏ gọn',
    autoClockOut: 'Tự động chấm công ra',
    autoClockOutDesc: 'Tự động chấm công ra vào một thời điểm cụ thể',
    autoClockOutTime: 'Thời gian tự động chấm công ra',
    weeklyReport: 'Báo cáo hàng tuần',
    weeklyReportDesc: 'Nhận tóm tắt hàng tuần về các hoạt động công việc của bạn',
    // Chrome của bảng thông số
    on: 'Bật',
    off: 'Tắt',
    sections: 'Nhóm cài đặt',
    unsaved: 'Chưa lưu',
    unsavedChanges: 'Thay đổi chưa lưu',
    nUnsaved: '{n} chưa lưu',
    allSaved: 'Đã lưu mọi thay đổi',
    nItems: '{n} mục',
    nothingUntilSaved: 'Không có gì hiệu lực cho đến khi lưu',
    queueEmpty: 'Bản nháp và cài đặt đã lưu đang trùng nhau. Đổi một dòng để tạo thay đổi.',
    revert: 'Hoàn tác',
    account: 'Tài khoản',
    saveN: 'Lưu {n} thay đổi',
    nothingToSave: 'Không có gì để lưu',
    loadFailed: 'Không tải được cài đặt của bạn.',
    themes: {
      light: 'Sáng',
      dark: 'Tối',
      system: 'Hệ thống'
    },
    scope: {
      notifications: 'Phạm vi: tài khoản này · áp dụng từ lần đăng nhập kế tiếp',
      topics: 'Phạm vi: mọi kênh ở trên · chủ đề đang tắt sẽ không bao giờ được gửi',
      appearance: 'Phạm vi: trình duyệt này · lưu vào tài khoản của bạn',
      language: 'Phạm vi: toàn bộ giao diện · có hiệu lực khi lưu',
      privacy: 'Phạm vi: những gì đồng nghiệp thấy trên thẻ danh bạ của bạn',
      work: 'Phạm vi: hồ sơ chấm công và trang mặc định của bạn'
    },
    note: {
      email: 'Gửi tới {email}',
      desktop: 'Trình duyệt sẽ hỏi quyền trong lần bật đầu tiên',
      frequency: 'Thời gian thực gửi ngay khi có sự kiện; bản tóm tắt gộp lại thành một tin',
      timeTracking: 'Phê duyệt, thiếu chấm công và yêu cầu minh chứng',
      performance: 'Chu kỳ đánh giá, mục tiêu và tự đánh giá',
      employee: 'Nhân viên mới, nghỉ việc và thay đổi danh bạ',
      recruitment: 'Hồ sơ ứng tuyển mới và thay đổi vòng tuyển',
      system: 'Lịch bảo trì và ghi chú phát hành',
      theme: 'Hệ thống sẽ theo thiết lập của hệ điều hành',
      dateFormat: 'Dùng cho mọi ngày hiển thị trên màn hình và trong tệp xuất',
      timeFormat: 'Áp dụng cho đồng hồ, sổ giờ công và lịch sử chấm công',
      itemsPerPage: 'Mọi bảng trong ứng dụng phân trang theo kích thước này',
      language: 'Nội dung chưa dịch sẽ hiển thị bằng tiếng Anh',
      timezone: 'Mọi mốc thời gian trên màn hình hiển thị theo múi giờ này',
      visibility: 'Quản lý và quản trị viên luôn giữ quyền truy cập cho mục đích nhân sự',
      showEmail: 'Tắt mục này chỉ ẩn khỏi danh bạ, không ẩn với bộ phận nhân sự',
      showPhone: 'Tắt mục này chỉ ẩn khỏi danh bạ, không ẩn với bộ phận nhân sự',
      dashboard: 'Chế độ mà Tổng Quan Tổ Chức mở lên',
      autoClockOutTime: 'Ca chấm công còn mở quá giờ này sẽ được đóng tại thời điểm đó'
    }
  },
  // Task Listing
  taskListing: {
    title: 'Danh Sách Nhiệm Vụ',
    individual: 'Cá nhân',
    assignDate: 'Ngày Phân Công',
    team: 'Đội nhóm',
    selectEmployee: 'Chọn nhân viên',
    dueDate: 'Hạn chót',
    startDate: 'Ngày bắt đầu',
    completionBeforeStart: 'Ngày hoàn thành không được trước ngày bắt đầu',
    manageTasks: 'Quản lý nhiệm vụ',
    assignTo: 'Phân Công Cho',
    assignTask: 'Phân Công Nhiệm Vụ',
    priorityLow: 'Thấp',
    priorityMedium: 'Trung Bình',
    priorityHigh: 'Cao',
    low: 'Thấp',
    medium: 'Trung Bình',
    high: 'Cao',
    statusPending: 'Đang chờ',
    statusInProgress: 'Đang tiến hành',
    statusCompleted: 'Đã hoàn thành',
    pending: 'Đang chờ',
    'in-progress': 'Đang tiến hành',
    teamWorkload: 'Khối lượng công việc của nhóm',
    filterByEmployee: 'Lọc theo nhân viên',
    filterByStatus: 'Lọc theo trạng thái',
    searchTasks: 'Tìm kiếm nhiệm vụ...',
    organization: 'Tổ chức',
    totalTasks: 'Tổng số nhiệm vụ',
    completed: "Đã hoàn thành",
    progress: 'Tiến Độ',
    avgQuality: 'Chất Lượng',
    avgProgress: 'Tiến Độ',
    myTasks: 'Nhiệm vụ của tôi',
    addTask: 'Thêm nhiệm vụ',
    editTask: 'Chỉnh sửa nhiệm vụ',
    taskTitle: 'Tiêu đề nhiệm vụ',
    description: 'Mô tả',
    priority: 'Ưu tiên',
    status: 'Trạng thái',
    selfAssessment: 'Tự đánh giá',
    selfAssessmentPlaceholder: 'Bạn đã thực hiện nhiệm vụ này như thế nào?',
    qualityRating: 'Đánh giá chất lượng',
    noTasks: 'Chưa có nhiệm vụ nào. Thêm nhiệm vụ đầu tiên của bạn!',
    employeeProgress: 'Tiến độ nhân viên',
    confirmDelete: 'Bạn có chắc chắn muốn xóa nhiệm vụ này?',
    tasks: 'Nhiệm Vụ',
    quality: 'Chất Lượng',
    completionDate: 'Ngày Hoàn Thành',
    createdAt: 'Ngày Tạo',
    updatedAt: 'Ngày Cập Nhật',

    // ── Sổ nhiệm vụ / chi tiết nhiệm vụ (thiết kế Industry) ────────
    assignedBy: 'Người Giao',
    titleRequired: 'Cần nhập tiêu đề nhiệm vụ',
    taskCreated: 'Đã tạo nhiệm vụ',
    taskCreateError: 'Không tạo được nhiệm vụ',
    taskUpdated: 'Đã cập nhật nhiệm vụ',
    taskUpdateError: 'Không cập nhật được nhiệm vụ',
    taskDeleted: 'Đã xóa nhiệm vụ',
    taskDeleteError: 'Không xóa được nhiệm vụ',
    newTask: 'Nhiệm vụ mới',
    export: 'Xuất dữ liệu',
    exported: 'Đã xuất {n} dòng',
    loadFailed: 'Không tải được danh sách nhiệm vụ',
    notYours: 'Nhiệm vụ này thuộc về người khác',
    markComplete: 'Đánh dấu hoàn thành',
    markedComplete: 'Đã đánh dấu hoàn thành',
    reopen: 'Mở lại',
    reopened: 'Đã mở lại nhiệm vụ',
    openTask: 'Mở',
    assignedTo: 'Phân Công Cho',
    noMatches: 'Không có kết quả nào khớp',

    // Thanh chỉ số
    open: 'Đang mở',
    dueToday: 'Đến hạn hôm nay',
    overdue: 'Quá hạn',
    unscheduled: 'Chưa có hạn',
    closedOnTime: 'Đóng đúng hạn',
    closedOnTimeHint: 'Trong số các nhiệm vụ đã đóng và có hạn chót',
    task: 'Nhiệm vụ',
    state: 'Trạng thái',
    recordOpen: 'Hồ sơ đang mở',
    grouping: 'Nhóm theo',
    byRecord: 'Theo hồ sơ',
    byDue: 'Theo hạn chót',
    byPriority: 'Theo ưu tiên',

    // Tiêu đề
    headOpen: '{n} đang mở trong tổng số {total} nhiệm vụ trên {r} hồ sơ nhân viên',
    noneInSegment: 'Không có nhiệm vụ nào ở khung nhìn này — phạm vi đang có {n} nhiệm vụ ở trạng thái khác',
    showClosedN: 'Xem {n} nhiệm vụ đã đóng',
    clearSearch: 'Xóa từ khóa tìm kiếm',
    unlinkedViewer: 'Tài khoản đăng nhập này chưa được liên kết với hồ sơ nhân viên nên không có hồ sơ nào để liệt kê nhiệm vụ. Quản trị viên có thể liên kết trong phần Quản lý người dùng.',
    headModel: 'mỗi nhiệm vụ đều phát sinh từ một hồ sơ, không nhiệm vụ nào đứng riêng',
    headMine: 'bạn phụ trách {n}',

    // Bộ lọc
    segAllOpen: 'Tất cả đang mở',
    segMine: 'Của tôi',
    segOverdue: 'Quá hạn',
    segClosed: 'Đã đóng',
    searchPlaceholder: 'Tìm nhiệm vụ, người phụ trách hoặc hồ sơ phát sinh',
    groupRecord: 'Nhóm theo hồ sơ',
    groupDue: 'Nhóm theo hạn chót',
    groupPriority: 'Nhóm theo ưu tiên',
    change: 'Đổi',

    // Bảng
    colTask: 'Nhiệm vụ',
    colSource: 'Nguồn',
    colOwner: 'Phụ trách',
    colDue: 'Hạn',
    colState: 'Trạng thái',
    done: 'Xong',
    emittedBy: 'mọi nhiệm vụ bên dưới đều phát sinh từ hồ sơ nhân viên này',
    nMoreOnRecord: 'còn {n} nhiệm vụ nữa trên hồ sơ này',
    showingRows: 'Hiển thị {shown} trên {total} · {groups} trên {allGroups} nhóm',
    showAll: 'Hiện tất cả',
    collapse: 'Thu gọn',

    // Dải nhóm
    bandOverdue: 'Đã quá hạn',
    bandOverdueNote: 'ngày đến hạn đã trôi qua',
    bandToday: 'Đến hạn hôm nay',
    bandTodayNote: 'đóng hôm nay, nếu không ngày mai sẽ thành quá hạn',
    bandWeek: 'Bảy ngày tới',
    bandWeekNote: 'trong tuần — vẫn còn xếp lịch được',
    bandLater: 'Sau đó',
    bandLaterNote: 'ngoài phạm vi tuần này',
    bandNoDate: 'Chưa có hạn',
    bandNoDateNote: 'không thể đôn đốc cho tới khi đặt hạn chót',
    bandHighNote: 'được cột quyết định xử lý trước',
    bandMediumNote: 'hàng đợi thông thường',
    bandLowNote: 'làm khi còn thời gian',

    // Trạng thái
    stateOpen: 'Đang mở',
    stateInProgress: 'Đang tiến hành',
    stateDone: 'Hoàn thành',
    stateCancelled: 'Đã hủy',
    stateOverdue: 'Quá hạn',
    stateUnscheduled: 'Chưa có hạn',

    // Nguồn
    sourceSelf: 'Tự đặt',
    sourceAssigned: 'Được giao',
    sourceSystem: 'Hệ thống',
    selfSetValue: 'Người phụ trách',
    unknownOwner: 'Hồ sơ không xác định',

    // Dòng thứ hai của mỗi hàng
    byWho: 'bởi {who}',
    bySelf: 'bởi chính người phụ trách',
    openedOn: 'mở ngày {date} {by}',
    openedUnknown: 'không ghi nhận ngày mở',
    closedOn: 'Đóng ngày {date}',
    onTime: 'đúng hạn',
    lateClose: 'đóng sau hạn chót',
    ratedN: 'đánh giá {n}/5',
    notRated: 'chưa đánh giá — hồ sơ không có điểm chất lượng cho nhiệm vụ này',
    cancelledLine: 'Đã hủy',
    oneDayLate: 'quá hạn 1 ngày',
    nDaysLate: 'quá hạn {n} ngày',
    noDueLine: 'Chưa có hạn — không thể đôn đốc và không bao giờ được tính vào số quá hạn',
    dueTodayLine: 'Đến hạn hôm nay',
    dueInDays: 'Còn {n} ngày đến hạn',
    assessmentLogged: 'đã có tự đánh giá',

    // Ô hạn chót
    noDue: 'Chưa có',
    today: 'Hôm nay',
    inNDays: 'Còn {n} ngày',
    oneDayLateShort: 'trễ 1 ngày',
    nDaysLateShort: 'trễ {n} ngày',

    // Cột quyết định
    needsDecision: 'Cần quyết định',
    nItems: '{n} mục',
    decisionSummary: '{o} quá hạn · {u} chưa có hạn để đôn đốc',
    nothingToDecide: 'Không còn nhiệm vụ nào đang mở trên các hồ sơ hiển thị.',
    costOverdue: 'Đã quá hạn {n} ngày và là mục mở lâu nhất trên hồ sơ này.',
    costToday: 'Đến hạn hôm nay; ngày mai sẽ vào danh sách quá hạn.',
    costNoDate: 'Không có hạn chót nên không có gì đôn đốc nó.',
    costSoon: 'Còn {n} ngày đến hạn, mức ưu tiên {p}.',
    costContext: '{owner} còn {n} nhiệm vụ đang mở khác trên cùng hồ sơ.',
    yourDay: 'Ngày của bạn',
    yourDayCount: '{n} đang mở · {d} đến hạn hôm nay',
    yourDayEmpty: 'Hiện chưa có nhiệm vụ nào được giao cho bạn.',
    closedAt: 'Đóng lúc {date}',

    // Chi tiết — tiêu đề và cảnh báo
    owner: 'phụ trách',
    dueLower: 'hạn',
    moveDueDate: 'Dời hạn chót',
    setDueDate: 'Đặt hạn chót',
    blockerOverdue: 'Hạn chót là {date}. Cho tới khi đóng, nhiệm vụ vẫn nằm trong số quá hạn của hồ sơ này, cùng với {n} nhiệm vụ đang mở khác.',
    blockerNoDateHead: 'Chưa có hạn — không thể đôn đốc',
    blockerNoDate: 'Không có hạn chót thì không thể xếp lịch, nhắc nhở hay báo cáo. Hãy đặt hạn, hoặc đóng nếu đã xong.',

    // Chi tiết — 01 Các bước
    panelSteps: 'Các bước',
    stepsNote: 'xong {done} trên {total} · còn {left}',
    waiting: 'Đang chờ',
    stepOpened: 'Nhiệm vụ được mở trên hồ sơ',
    stepOpenedDetail: 'Phát sinh từ hồ sơ {who} · {src}',
    srcSelfSet: 'do người phụ trách tự đặt',
    srcAssignedBy: 'do {creator} giao',
    stepScheduled: 'Đã đặt hạn chót',
    stepScheduledDone: 'Hạn {date} · mọi bước bên dưới đo theo mốc này',
    stepScheduledOpen: 'Không có hạn thì không thể đôn đốc, không tính quá hạn và không đưa vào báo cáo',
    stepStarted: 'Đã bắt đầu',
    stepStartedDone: 'Người phụ trách đã chuyển khỏi trạng thái Đang chờ',
    stepStartedOpen: 'Sau khi có hạn · người phụ trách chuyển sang Đang tiến hành',
    stepClosed: 'Đánh dấu hoàn thành',
    stepClosedDone: 'Đóng ngày {date}',
    stepClosedOpen: 'Sau khi làm xong · đóng nhiệm vụ mới giải phóng hồ sơ',
    stepAssessed: 'Đánh giá và chấm điểm',
    stepAssessedDone: 'Đánh giá {n}/5 · tính vào điểm chất lượng của hồ sơ này',
    stepAssessedOpen: 'Sau khi hoàn thành · chưa có thì hồ sơ không có điểm chất lượng cho nhiệm vụ này',

    // Chi tiết — 02 Chuỗi liên kết
    panelChain: 'Chuỗi liên kết',
    relOpenedBy: 'Mở bởi',
    relBelongsTo: 'Thuộc về',
    relSitsWith: 'Đi cùng',
    recordHolds: 'Hồ sơ này có tất cả {n} nhiệm vụ',
    nOpenTasks: '{n} nhiệm vụ đang mở trên cùng hồ sơ',
    nOfThemOverdue: '{n} trong số đó đã quá hạn — mở hồ sơ',
    noneOverdue: 'không nhiệm vụ nào quá hạn — mở hồ sơ',

    // Chi tiết — 03 Thời gian trôi qua
    panelElapsed: 'Thời gian trôi qua',
    againstWindow: 'so với khung {n} ngày mà hạn chót cho phép',
    noWindow: 'chưa có hạn — không có khung thời gian để đo',
    measuredToClose: 'tính đến thời điểm đóng',
    measuredToToday: 'tính đến hôm nay',
    withinWindow: 'Trong khung thời gian',
    pastDue: 'Vượt quá hạn chót',
    openedOnDate: 'Mở ngày {date}',

    // Chi tiết — cột phải
    record: 'Hồ sơ',
    recordNote: 'Mọi thứ nhiệm vụ này mang theo',
    recordField: 'Hồ sơ',
    setOnClose: 'Được đặt khi nhiệm vụ đóng',
    priorityHighNote: 'Xử lý trước',
    priorityMediumNote: 'Hàng đợi thông thường',
    priorityLowNote: 'Khi còn thời gian',
    activity: 'Hoạt động',
    actRated: 'Chấm điểm chất lượng {n}/5',
    actClosed: 'Đánh dấu hoàn thành',
    actChanged: 'Thay đổi gần nhất · hiện {state}',
    actAssessed: 'Đã viết tự đánh giá',
    actOpened: 'Nhiệm vụ được mở trên hồ sơ {who}',
    notes: 'Ghi chú',
    addNote: 'Thêm ghi chú hoặc quyết định…',
    noteAdded: 'Đã thêm ghi chú'
  },
  // Task Review
  taskReview: {
    title: 'Đánh giá Hiệu suất',
    notRated: 'Chưa được đánh giá',
    subtitle: 'Tiến độ chu kỳ đánh giá, phân bố điểm và phê duyệt',
    individual: 'Cá nhân',
    team: 'Đội nhóm',
    review: 'Đánh giá',
    reviewTask: 'Đánh giá Công việc',
    description: 'Mô Tả',
    qualityRatingStars: 'Xếp hạng chất lượng (1-5 sao)',
    managerEvaluationComments: 'Bình luận đánh giá của Quản lý',
    submitReview: 'Gửi Đánh Giá',
    updateStatus: 'Cập nhật Trạng thái',
    feedbackPlaceholder: 'Cung cấp phản hồi về tiến độ và chất lượng công việc...',
    selfAssessmentComments: 'Bình luận tự đánh giá',
    reviewType: 'Loại đánh giá',
    selectMonth: 'Chọn tháng',
    month: 'Tháng',
    year: 'Năm',
    viewReport: 'Xem báo cáo',
    totalTasks: 'Tổng số công việc',
    inProgress: 'Đang tiến hành',
    overdue: 'Quá hạn',
    completionRate: 'Tỷ lệ hoàn thành',
    avgQuality: 'Chất lượng TB',
    onTimePerformance: 'Hiệu suất đúng hạn',
    onTimeCompletions: 'Hoàn thành đúng hạn',
    onTimeRate: 'Tỷ lệ đúng hạn',
    priorityDistribution: 'Phân bổ ưu tiên',
    highPriority: 'Ưu tiên cao',
    mediumPriority: 'Ưu tiên trung bình',
    lowPriority: 'Ưu tiên thấp',
    qualityAssessment: 'Đánh giá chất lượng',
    ratedTasks: 'Công việc đã đánh giá',
    noTasks: 'Không tìm thấy công việc cho tháng này',
    noTeamTasks: 'Không tìm thấy công việc đội nhóm cho tháng này',
    evaluateTask: 'Đánh giá Công việc',
    selfAssessment: 'Tự đánh giá',
    qualityRating: 'Xếp hạng chất lượng',
    managerComments: 'Bình luận của Quản lý',
    commentPlaceholder: 'Cung cấp phản hồi về chất lượng công việc và mục tiêu cá nhân...',
    taskDetails: 'Chi tiết Công việc',
    currentRating: 'Xếp hạng hiện tại',
    submitEvaluation: 'Gửi Đánh giá',
    cancel: 'Hủy',
    evaluate: 'Đánh giá',
    managerEvaluation: 'Đánh giá của Quản lý',
    created: 'Đã tạo',
    due: 'Đến hạn',
    all: 'Tất cả',
    pending: 'Đang chờ',
    createGoal: 'Tạo Mục tiêu',
    createGoalSubtitle: 'Đặt một mục tiêu mới cho nhân viên',
    createPersonalGoal: 'Đặt một mục tiêu cá nhân mới',
    assignTo: 'Giao cho',
    goalTitle: 'Tiêu đề Mục tiêu',
    goalTitlePlaceholder: 'Nhập tiêu đề mục tiêu...',
    goalDescription: 'Mô tả',
    goalDescriptionPlaceholder: 'Mô tả chi tiết mục tiêu...',
    goalPermissionAdmin: 'Là quản trị viên/quản lý, bạn có thể tạo mục tiêu cho bất kỳ nhân viên nào',
    goalPermissionEmployee: 'Bạn chỉ có thể tạo mục tiêu cho chính mình',
    high: 'Cao',
    medium: 'Trung Bình',
    low: 'Thấp',
    employees: 'Nhân Viên Có Nhiệm Vụ',
    completion: 'Tỉ Lệ Việc Hoàn Thành',
    quality: 'Chất Lượng',
    task: 'Nhiệm Vụ',
    progress: 'Tiến Độ',
    organization: 'Tổ chức',
    chooseEmployee: 'Chọn nhân viên',
    selectEmployeeToView: 'Chọn nhân viên để xem mục tiêu và nhiệm vụ của họ',
    tasks: 'Nhiệm vụ',
    assignedBy: 'Phân công bởi',
    assignedTo: 'Phân công cho',
    dueDate: "Ngày đến hạn",
    status: "Trạng thái",
    priority: "Ưu tiên",
    action: 'Hành động',
    noTasksFound: 'Không tìm thấy nhiệm vụ nào',
    selectEmployee: "Chọn nhân viên",
    taskList: 'Danh sách nhiệm vụ',
    employeeSelfAssessment: 'Tự đánh giá của nhân viên',
    employeeBreakdown: 'Phân tích nhân viên',
    total: 'Tổng cộng',
    completedTasks: 'Nhiệm vụ đã hoàn thành',
    inProgressTasks: 'Nhiệm vụ đang tiến hành',
    overdueTasks: 'Nhiệm vụ quá hạn',
    avgQualityRating: 'Đánh giá chất lượng TB',
    category: 'Danh Mục',
    targetDate: 'Ngày Mục Tiêu',
    notes: 'Ghi Chú',
    createdAt: 'Ngày Tạo',
    updatedAt: 'Ngày Cập Nhật',
    completed: "Nhiệm Vụ Đã Hoàn Thành",
    // Bảng điều khiển chu kỳ đánh giá
    cycle: 'Chu kỳ',
    company: 'Toàn công ty',
    scope: 'Phạm vi',
    inScope: 'Trong phạm vi',
    inScopeCount: '{n} nhân viên trong phạm vi',
    signedOff: 'Đã phê duyệt',
    signedOffThisWeek: 'Đã phê duyệt trong 7 ngày qua',
    avgScore: 'Điểm TB',
    calibration: 'hiệu chỉnh',
    calibrationLabel: 'Hiệu chỉnh',
    daysOut: 'còn {n} ngày',
    daysPast: 'quá {n} ngày',
    cycleCompletion: 'Mức hoàn thành chu kỳ',
    ofSignedOff: '{n} trên {total} đã phê duyệt',
    thisWeek: 'tuần này',
    projectionDone: 'Mọi đánh giá trong phạm vi đều đã được phê duyệt.',
    projectionRemaining: 'Còn {n} đánh giá cần phê duyệt trước buổi hiệu chỉnh ngày {date}.',
    projectionRate: 'Với tốc độ này, chu kỳ sẽ kết thúc',
    beforeCalibration: 'sớm hơn buổi hiệu chỉnh {n} ngày.',
    afterCalibration: 'muộn hơn buổi hiệu chỉnh {n} ngày.',
    stageSelf: 'Tự đánh giá',
    stageManager: 'Quản lý đánh giá',
    stageCalibration: 'Hiệu chỉnh',
    stageSignedOff: 'Đã phê duyệt',
    complete: 'hoàn tất',
    nOutstanding: 'còn {n} chưa xong',
    nOverdue: '{n} quá hạn',
    nToFile: 'còn {n} cần nộp',
    session: 'buổi',
    nWaitOnYou: '{n} đang chờ bạn',
    noneWaitOnYou: 'không có mục nào chờ bạn',
    scoreDistribution: 'Phân bố điểm',
    nReviews: '{n} đánh giá',
    distributionCaption: 'Điểm trung bình {avg} tập trung nhiều nhất tại {peak} — {pct}% số đánh giá đạt mức đó trở lên',
    distributionEmptyCaption: 'Chưa có đánh giá nào được chấm điểm trong chu kỳ này',
    noScores: 'Chưa có mục nào được chấm điểm trong chu kỳ này.',
    averageByDepartment: 'Điểm trung bình theo phòng ban',
    scaleRange: 'thang {from} – {to}',
    departmentCaption: 'Đường kẻ dọc là điểm trung bình toàn tổ chức, {avg} — {n} phòng ban nằm dưới mức này',
    departmentEmptyCaption: 'Điểm trung bình phòng ban sẽ hiện ra khi các đánh giá được chấm điểm',
    noDepartmentScores: 'Chưa phòng ban nào có đánh giá được chấm điểm trong chu kỳ này.',
    awaitingSignOff: 'Đang chờ bạn phê duyệt',
    oldestWaited: 'Mục lâu nhất đã chờ {n} ngày',
    calibrationCloses: 'hiệu chỉnh kết thúc ngày {date}',
    nothingAwaiting: 'Không có mục nào chờ chữ ký của bạn trong chu kỳ này.',
    sinceLastCycle: 'so với',
    scoreUnchanged: 'điểm không đổi',
    flaggedForPromotion: 'được đề xuất thăng chức',
    signOff: 'Phê duyệt',
    sendBack: 'Trả lại',
    waitedDays: 'đã chờ {n} ngày',
    overdueManagerReviews: 'Đánh giá của quản lý quá hạn',
    noOverdueReviews: 'Mọi đánh giá của quản lý đều còn trong hạn.',
    nLate: '{n} trễ hạn',
    remindAll: 'Nhắc cả {n} người',
    remindNoAccounts: 'Không quản lý nào trong số đó có tài khoản để nhận thông báo.',
    reminderTitle: 'Đánh giá của quản lý còn tồn đọng',
    reminderBody: '{n} đánh giá của {dept} đã quá hạn cho chu kỳ {cycle}.',
    remindedMessage: 'Đã nhắc {n} quản lý.',
    signedOffMessage: 'Đã phê duyệt {name}.',
    sentBackMessage: 'Đã trả lại {name} để chỉnh sửa.',
    overallRating: 'Tổng thể',
    strengths: 'Điểm mạnh',
    areasForImprovement: 'Cần cải thiện',
    achievements: 'Thành tích',
    noWrittenReview: 'Không có nhận xét bằng văn bản nào được ghi lại cho đánh giá này.',
    competency: {
      technical_skills_rating: 'Chuyên môn',
      communication_rating: 'Giao tiếp',
      leadership_rating: 'Lãnh đạo',
      teamwork_rating: 'Làm việc nhóm',
      problem_solving_rating: 'Giải quyết vấn đề'
    }
  },
  // 3d — màn hình bấm giờ trực tiếp
  punchClock: {
    title: 'Bấm Giờ',
    shift: 'ca',
    scope: 'Phạm vi',
    scopeMine: 'Của tôi',
    scopeFloor: 'Toàn sàn',
    sync: 'Đồng bộ',
    // thanh chỉ số
    onTheFloor: 'Đang làm',
    ofHeadcount: '{n} trên {total} nhân viên',
    onBreak: 'Nghỉ giữa giờ',
    hoursToday: 'Giờ hôm nay',
    lateToday: 'Đến muộn',
    unclosed: 'Chưa chốt',
    // khối chính
    countingFrom: 'Đang tính giờ từ {time}',
    notPunchedIn: 'Chưa bấm giờ vào',
    onClock: 'đã làm {n}',
    breakTotal: 'nghỉ {n}',
    readyToPunch: 'hôm nay chưa ghi nhận gì',
    regularHours: 'Giờ làm thường',
    overtimeAfter: 'Tăng ca sau {time}',
    startBreak: 'Bắt đầu nghỉ',
    endBreak: 'Kết thúc nghỉ',
    punchIn: 'Bấm giờ vào',
    punchOut: 'Bấm giờ ra',
    consequenceIn: 'Bấm vào sẽ bắt đầu tính giờ cho ca {time}',
    consequencePlain: 'Bấm ra lúc này ghi {n} chờ duyệt',
    consequenceOvertime: 'Bấm ra lúc này ghi {n} tăng ca',
    punchedIn: 'Đã bấm giờ vào lúc {time}',
    punchedOut: 'Đã bấm giờ ra. Ghi {n} chờ duyệt.',
    noEmployee: 'Tài khoản này chưa gắn với hồ sơ nhân viên.',
    filedLive: 'Bấm giờ trực tiếp',
    filedWithBreak: 'Bấm giờ trực tiếp · nghỉ {n}',
    dayStripLabel: 'Hôm nay từ 08:00 đến 19:00',
    // tuần
    thisWeek: 'Tuần này',
    ofContract: 'của {n}h',
    weekMine: 'Giờ của bạn',
    weekFloor: 'Trung bình mỗi người có đi làm',
    weekCaption: 'vạch ngang là ngày hợp đồng {n}h',
    contractDay: 'Ngày hợp đồng, 8h',
    dayMon: 'HAI',
    dayTue: 'BA',
    dayWed: 'TƯ',
    dayThu: 'NĂM',
    dayFri: 'SÁU',
    daySat: 'BẢY',
    daySun: 'CN',
    // nhật ký
    log: 'Nhật ký bấm giờ',
    logCaption: 'Mỗi sự kiện đều giữ nguồn gốc · điều chỉnh vẫn giữ bản gốc',
    logEmpty: 'Hôm nay chưa có lần bấm giờ nào.',
    colTime: 'Giờ',
    colEvent: 'Sự kiện',
    colSource: 'Nguồn',
    colNote: 'Ghi chú',
    eventIn: 'Bấm giờ vào',
    eventOut: 'Bấm giờ ra',
    eventBreakStart: 'Bắt đầu nghỉ',
    eventBreakEnd: 'Kết thúc nghỉ',
    eventRunning: 'Đang tính',
    onTime: 'Đúng giờ',
    sourceFiled: 'Nhập tay',
    sourceLive: 'Bấm tại đây',
    pendingApproval: '{n} bản ghi đang chờ duyệt',
    // cột tình trạng sàn
    floorStatus: 'Tình trạng sàn',
    nWorking: '{n} đang làm',
    needDecision: '{n} lần bấm giờ cần người quyết định',
    allClean: 'Không có việc gì trên sàn cần quyết định.',
    impossibleSpan: 'Khoảng thời gian dài bất thường cho một ca',
    closeAt: 'Chốt lúc {time}',
    enterByHand: 'Nhập tay',
    wouldClose: 'Việc chốt một lần bấm giờ nằm ở màn hình Chấm công.',
    wouldEdit: 'Việc sửa tay nằm ở màn hình Chấm công.',
    lateMeta: '{n} người · muộn nhất {m}',
    noneLate: 'Mọi người đều trong thời gian cho phép',
    noPunch: 'Hôm nay chưa bấm giờ',
    everyonePunched: 'Toàn bộ danh sách đều đã bấm giờ',
    workingByDept: 'Đang làm theo bộ phận',
    rationale: 'Đồng hồ 76px cùng một nút đặc là trọng tâm của màn hình: mọi thứ khác chỉ là căn cứ cho một quyết định duy nhất — khi nào bấm giờ.'
  },
  performanceAppraisalPage: {
    title: "Đánh Giá Hiệu Suất",
    overallPerformance: "Hiệu Suất Tổng Thể",
    goalsAchieved: "Mục Tiêu Đạt Được",
    reviewsThisPeriod: "Đánh Giá Trong Kỳ Này",
    avgSkillRating: "Đánh Giá Kỹ Năng TB",
    currentGoals: "Mục Tiêu Hiện Tại",
    addGoal: "Thêm Mục Tiêu",
    overview: "Tổng Quan",
    goals: "Mục Tiêu",
    reviews: "Đánh Giá"
  },
  employeeDetailTabs: {
    basicInformation: "Thông Tin Cơ Bản",
    contact: "Liên Hệ",
    documents: "Tài Liệu"
  },
  employeeEditForm: {
    editEmployee: "Chỉnh Sửa Nhân Viên",
    contactInformation: "Thông Tin Liên Hệ",
    employmentDetails: "Chi Tiết Việc Làm",
    email: "E-Mail",
    phone: "Điện Thoại",
    telephone: "Số Điện Thoại",
    address: "Địa Chỉ",
    department: "Phòng Ban",
    abteilung: "Phòng Ban",
    startDate: "Ngày Bắt Đầu",
    startdatum: "Ngày Bắt Đầu",
    dateOfBirth: "Ngày Sinh",
    performance: "Hiệu Suất",
    leistung: "Hiệu Suất",
    cancel: "Hủy Bỏ",
    abbrechen: "Hủy Bỏ",
    save: "Lưu",
    speichern: "Lưu"
  },
  // Department Pie Chart Names
  departments: {
    all: 'Tất Cả',
    legal_compliance: 'Pháp Lý',
    technology: 'Công Nghệ',
    internal_affairs: 'Quản Lý Nội Bộ',
    human_resources: 'Nhân Sự',
    office_unit: 'Khối Văn Phòng',
    board_of_directors: 'Hội Đồng Quản Trị',
    finance: 'Tài Chính',
    engineering: 'Kỹ Thuật',
    sales: 'Bán Hàng',
    marketing: 'Tiếp Thị',
    design: 'Thiết Kế',
    part_time_employee: 'Khối Chuyên Gia'
  },
  // Control Panel
  controlPanel: {
    refresh: 'Làm Mới',
    refreshing: 'Đang làm mới...',
    refreshData: 'Làm mới Dữ liệu',
    lastUpdated: 'Cập nhật lần cuối',
    updateAvailable: 'Cập nhật mới có sẵn!',
    updateAvailableDesc: 'Phiên bản mới đã sẵn sàng để tải xuống. Vui lòng truy cập trang GitHub để tải phiên bản mới nhất.',
    downloadLink: 'Tải xuống tại đây',
    demoModeDisabled: 'Chức Năng Bị Khoá',
    title: 'Bảng Điều Khiển Quản Trị',
    subtitle: 'Quản lý cài đặt hệ thống và tài khoản người dùng',
    role: 'Vai Trò',
    switchToPrefix: 'Chuyển sang',
    demoAdminLabel: 'Quản trị Viên',
    demoEmployeeLabel: 'Nhân Viên',
    userUuid: 'UUID Người Dùng',
    employeeId: 'Mã Nhân Viên',
    changePassword: 'Đổi Mật Khẩu',
    changeOwnPassword: 'Đổi Mật Khẩu Của Bạn',
    resetOtherUserPassword: 'Đặt Lại Mật Khẩu Nhân Viên Khác',
    readManual: 'Đọc Hướng Dẫn',
    logout: 'Đăng Xuất',
    confirmLogout: 'Bạn có chắc chắn muốn đăng xuất?',
    allFieldsRequired: 'Tất cả các trường là bắt buộc',
    passwordTooShort: 'Mật khẩu phải có ít nhất 6 ký tự',
    passwordsDontMatch: 'Mật khẩu không khớp',
    passwordChanged: 'Đổi mật khẩu thành công!',
    passwordChangeError: 'Lỗi khi đổi mật khẩu',
    currentPassword: 'Mật Khẩu Hiện Tại',
    newPassword: 'Mật Khẩu Mới',
    confirmPassword: 'Xác Nhận Mật Khẩu',
    needHelp: 'Cần Trợ Giúp?',
    helpText: 'Xem hướng dẫn để biết chi tiết về cách sử dụng hệ thống.',
    standardAccess: 'Quyền truy cập người dùng tiêu chuẩn',
    visitAnalytics: 'Phân Tích Lượt Truy Cập',
    visitAnalyticsDesc: 'Theo dõi và phân tích lượt truy cập vào hệ thống của bạn.',
    visitAnalyticsSubtitle: 'Theo dõi lượt truy cập, nguồn giới thiệu và trình duyệt',
    visitAnalyticsTitle: 'Phân Tích Lượt Truy Cập',
    visitAnalyticsHelp: 'Xem hướng dẫn để biết cách sử dụng phân tích lượt truy cập.',
    visit: {
      demoCount: 'Lượt Truy Cập Demo',
      authorized: 'Đăng Nhập Uỷ Quyền',
      total: 'Tổng',
      last24h: '24 giờ qua',
      distinctIps: 'IP riêng biệt',
      noVisits: 'Chưa có lượt truy cập nào được ghi nhận.',
      unknownIp: 'IP không xác định',
      ref: 'Ref',
      referrer: 'Nguồn giới thiệu',
      referrerDesc: 'Nguồn giới thiệu là trang web hoặc ứng dụng đã giới thiệu người dùng đến hệ thống của bạn.',
      userAgent: 'Trình duyệt',
      userAgentDesc: 'Trình duyệt là phần mềm mà người dùng sử dụng để truy cập hệ thống của bạn.',
      lastVisit: 'Lần truy cập cuối',
      lastVisitDesc: 'Lần truy cập cuối là thời điểm người dùng đã truy cập hệ thống lần cuối.',
      visitsByDay: 'Lượt truy cập theo ngày',
      visitsByDayDesc: 'Biểu đồ này hiển thị số lượt truy cập hàng ngày trong khoảng thời gian đã chọn.',
      visitsByReferrer: 'Lượt truy cập theo nguồn giới thiệu',
      visitsByReferrerDesc: 'Biểu đồ này hiển thị số lượt truy cập theo từng nguồn giới thiệu trong khoảng thời gian đã chọn.',
      visitsByBrowser: 'Lượt truy cập theo trình duyệt',
      visitsByBrowserDesc: 'Biểu đồ này hiển thị số lượt truy cập theo từng trình duyệt trong khoảng thời gian đã chọn.',
      visitsByIp: 'Lượt truy cập theo IP',
      visitsByIpDesc: 'Biểu đồ này hiển thị số lượt truy cập theo từng địa chỉ IP trong khoảng thời gian đã chọn.'
    },
    roles: {
      admin: 'Quản Trị Viên',
      hrManager: 'Quản Lý Nhân Sự',
      manager: 'Quản Lý',
      employee: 'Nhân Viên',
      viewer: 'Người Xem',
      demoAdmin: 'Quản Trị Viên-Demo',
      hrAdmin: 'Quản Trị Viên Nhân Sự'
    },
    roleDesc: {
      admin: 'Quyền truy cập toàn bộ hệ thống với tất cả các đặc quyền quản trị bao gồm quản lý người dùng, cài đặt hệ thống và kiểm soát dữ liệu hoàn toàn',
      hrAdmin: 'Quản lý nhân sự hoàn chỉnh với quản trị nhân viên, quyền truy cập bảng lương và cấu hình hệ thống',
      hrManager: 'Quản lý nhân viên, đánh giá hiệu suất, tuyển dụng và các hoạt động nhân sự',
      manager: 'Giám sát thành viên nhóm, phê duyệt chấm công và quản lý đánh giá hiệu suất',
      employee: 'Truy cập thông tin cá nhân, gửi bản ghi giờ làm và xem dữ liệu hiệu suất của bản thân',
      viewer: 'Quyền truy cập chỉ xem đối với báo cáo, bảng điều khiển và thông tin hệ thống hạn chế',
      demoAdmin: 'Chế độ demo sử dụng localstorage để mô phỏng quyền quản trị; phiên bản chính thức sử dụng cơ sở dữ liệu backend để đồng bộ dữ liệu'
    },
    uploadAvatar: 'Tải lên ảnh đại diện',
    avatarUpdated: 'Cập nhật ảnh đại diện thành công!',
    avatarError: 'Lỗi khi tải lên ảnh đại diện',
    resetUserPassword: 'Đặt Lại Mật Khẩu Người Dùng',
    selectUser: 'Chọn Người Dùng',
    chooseUser: '-- Chọn một người dùng --',
    enterNewPassword: 'Nhập mật khẩu mới',
    confirmNewPassword: 'Xác nhận mật khẩu mới',
    resetPassword: 'Đặt Lại Mật Khẩu',
    selectUserFirst: 'Vui lòng chọn người dùng trước',
    userNotFound: 'Không tìm thấy người dùng',
    confirmResetPassword: 'Bạn có chắc chắn muốn đặt lại mật khẩu cho',
    passwordResetSuccess: 'Đặt lại mật khẩu thành công cho',
    passwordResetError: 'Lỗi khi đặt lại mật khẩu. Bạn có thể cần quyền truy cập vai trò dịch vụ quản trị viên.',
    errorFetchingUsers: 'Lỗi khi tải người dùng',
    warning: 'Cảnh báo',
    adminResetWarning: 'Điều này sẽ thay đổi mật khẩu cho người dùng đã chọn. Họ sẽ cần sử dụng mật khẩu mới để đăng nhập.',
    resetEmployeePassword: 'Đặt Lại Mật Khẩu Nhân Viên',
    selectEmployee: 'Chọn Nhân Viên',
    chooseEmployee: '-- Chọn một nhân viên --',
    employeeNotFound: 'Không tìm thấy nhân viên',
    confirmResetEmployeePassword: 'Bạn có chắc chắn muốn đặt lại mật khẩu cho nhân viên',
    passwordResetSuccessEmployee: 'Đặt lại mật khẩu thành công cho nhân viên',
    errorFetchingEmployees: 'Lỗi khi tải nhân viên',
    noEmployeesFound: 'Không tìm thấy nhân viên',
    manageEmails: 'Quản lý Email người dùng',
    restoreDemoData: "Khôi phục dữ liệu demo",
    restoreDemoDataDescription: "Khôi phục dữ liệu demo mặc định cho các loại dữ liệu cụ thể. Thao tác này sẽ đặt lại mọi thay đổi bạn đã thực hiện.",
    demoTimeEntries: "Bản ghi thời gian",
    demoGoals: "Mục tiêu",
    demoTasks: "Nhiệm vụ",
    demoReviews: "Đánh giá",
    demoSkills: "Kỹ năng",
    demoLeaveRequests: "Yêu cầu nghỉ phép",
    demoDataRestored: "{type} đã được khôi phục về mặc định",
    allDemoDataRestored: "Tất cả dữ liệu demo đã được khôi phục về mặc định",
    restoreAllDemoData: "Khôi phục tất cả dữ liệu demo",
    switchToDemoEmployee: 'Chuyển Sang Nhân Viên',
    switchToDemoAdmin: 'Chuyển Sang Quản Trị Viên',
    demoRoleOnly: 'Tính năng này chỉ khả dụng ở chế độ demo',
    // Bảng Điều Khiển theo hệ thiết kế industry: dòng phạm vi và hệ quả.
    mode: 'Chế Độ',
    modeDemo: 'Demo',
    modeLive: 'Trực Tiếp',
    demoSession: 'Phiên demo — các thao tác chỉ được mô phỏng',
    liveSession: 'Phiên trực tiếp',
    signedInAs: 'Đang đăng nhập với',
    identity: 'Danh Tính',
    identityScope: 'Phạm vi: chỉ phiên đăng nhập này',
    credentials: 'Thông Tin Đăng Nhập',
    credentialsScope: 'Phạm vi: chỉ tài khoản của bạn',
    credentialsScopeAdmin: 'Phạm vi: tài khoản của bạn · và mọi tài khoản bạn quản trị',
    change: 'Thay Đổi',
    ownPasswordNote: 'Có hiệu lực ngay; thiết bị này vẫn giữ đăng nhập',
    demoLockedNote: 'Bị khóa ở chế độ demo — không ghi gì vào cơ sở dữ liệu',
    userPasswordNote: '{n} tài khoản trong phạm vi — họ đăng nhập bằng mật khẩu mới ngay lập tức',
    userPasswordNoteIdle: 'Đăng xuất tài khoản được chọn khỏi mọi thiết bị đang dùng',
    employeePasswordNote: '{n} hồ sơ nhân viên có tài khoản đăng nhập riêng',
    employeePasswordNoteIdle: 'Chỉ hồ sơ nhân viên liên kết với tài khoản mới đặt lại được',
    togglePassword: 'Hiện hoặc ẩn mật khẩu',
    visitScope: 'Phạm vi: mọi phiên truy cập ứng dụng',
    session: 'Phiên Làm Việc',
    demoSessionNote: 'Dữ liệu demo nằm trong trình duyệt này. Không có gì gửi tới cơ sở dữ liệu.',
    liveSessionNote: 'Các thao tác này rời khỏi màn hình. Mọi thứ phía trên chỉ thuộc về bạn.',
    switchRole: 'Đổi Vai Trò',
    endSession: 'Kết Thúc Phiên',
    logoutNote: 'Đăng xuất thiết bị này. Các biểu mẫu chưa lưu sẽ bị hủy.'
  },
  // Kiểm Soát Chính Sách — cài đặt trình bày như bản đặc tả kỹ thuật.
  policyControls: {
    title: 'Kiểm Soát Chính Sách',
    version: 'Phiên bản chính sách',
    lastEditedBy: 'sửa lần cuối bởi',
    appliesTo: 'áp dụng cho {n} nhân viên',
    versionHistory: 'Lịch Sử Phiên Bản',
    publishN: 'Ban hành {n} thay đổi',
    nothingToPublish: 'Không có gì để ban hành',
    sections: 'Các mục chính sách',
    policy: 'Chính sách',
    users: 'Người dùng',
    roles: 'Vai trò',
    pendingChanges: 'Thay đổi chờ duyệt',
    lastEdit: 'Sửa lần cuối',
    organisation: 'Tổ chức',
    nItems: '{n} mục',
    nothingUntilPublished: 'Không có gì hiệu lực cho tới khi ban hành',
    queueEmpty: 'Bản ban hành và bản nháp đang trùng nhau. Sửa một quy tắc để mở thay đổi.',
    approve: 'Phê Duyệt',
    decline: 'Từ Chối',
    accepted: 'Đã duyệt',
    revert: 'Hoàn Tác',
    queued: 'Đang chờ',
    auditTrail: 'Nhật ký thay đổi',
    on: 'Bật',
    off: 'Tắt',
    affects: 'Ảnh hưởng {n}',
    lowRisk: 'Rủi ro thấp',
    yourEdit: 'Bạn sửa',
    yourEditReason: 'Sửa trên màn hình này — có hiệu lực khi bạn ban hành.',
    you: 'Bạn',
    openCases: 'mở các trường hợp',
    weekday: 'Ngày thường',
    weekend: 'Cuối tuần',
    holiday: 'Ngày lễ',
    thirdWorkingDay: 'Ngày làm việc thứ 3',
    elevenDays: '11 ngày',
    file: 'Tệp',
    encoding: 'Mã hóa',
    item: {
      otCap: 'Trần tăng ca',
      otCapReason: 'Trần Thị Lan Anh đề xuất — chuyền Sơn Trà chạy sáu ngày mỗi tuần cho tới khi đơn hàng tháng Chín xuất xưởng.',
      lateGrace: 'Thời gian trễ được bỏ qua',
      lateGraceReason: 'Đỗ Bảo Long đề xuất — xe đưa đón 07:30 hay tới muộn vào sáng mưa.',
      teamLead: 'Vai trò · Tổ trưởng',
      teamLeadGain: 'Được thêm "Duyệt tăng ca" · 9 người'
    },
    sec: {
      workingTime: 'Giờ làm việc',
      overtime: 'Tăng ca & lương',
      leave: 'Chính sách nghỉ phép',
      roles: 'Vai trò & quyền',
      cycles: 'Chu kỳ đánh giá',
      notifications: 'Thông báo',
      data: 'Dữ liệu & lưu trữ'
    },
    scope: {
      allDepartments: 'Phạm vi: toàn bộ phòng ban',
      overtime: 'Phạm vi: toàn bộ phòng ban · 1 ngoại lệ cho Kỹ thuật',
      leave: 'Phạm vi: toàn bộ phòng ban · áp dụng mức tối thiểu luật định',
      roles: 'Phạm vi: 6 vai trò · 248 nhân viên',
      cycles: 'Phạm vi: toàn bộ phòng ban · H1 2026 đang mở',
      notifications: 'Phạm vi: toàn bộ nhân viên · email và trong ứng dụng',
      data: 'Phạm vi: toàn bộ hồ sơ · lưu trú dữ liệu tại Việt Nam'
    },
    row: {
      standardDay: 'Ngày làm việc chuẩn',
      standardDayNote: 'Vượt mức này là tăng ca — hôm qua có 41 người vượt',
      workingDays: 'Số ngày làm việc mỗi tháng',
      workingDaysNote: 'Là mẫu số của mọi chỉ số tháng trên bảng điều khiển — tháng 7/2026 ra 26',
      coreHours: 'Khung giờ lõi',
      coreHoursNote: 'Họp ngoài khung này cần ngoại lệ — tuần này có 3 cuộc',
      lateGrace: 'Thời gian trễ được bỏ qua',
      lateGraceNote: 'Tuần này có 7 người dùng đến nó',
      autoClose: 'Tự động chốt lượt chấm công',
      autoCloseNote: 'Tháng trước đã chốt 14 lượt bị quên vào nửa đêm',
      weekendWork: 'Cuối tuần tính là ngày làm',
      weekendWorkNote: 'Bật lên sẽ đưa tháng từ 26 lên 30 ngày làm việc',
      otCap: 'Trần tăng ca hằng tháng',
      otCapNote: 'Tháng này có 3 người vượt trần',
      otApproval: 'Tăng ca cần phê duyệt',
      otApprovalNote: '23 quản lý nắm quyền duyệt · 41 yêu cầu đã xử lý trong tháng',
      multipliers: 'Hệ số tăng ca',
      multipliersNote: 'Do hợp đồng quy định — bộ phận lương đọc số này, màn hình chỉ báo cáo lại',
      payPeriod: 'Kỳ lương chốt vào',
      payPeriodNote: 'Bản ghi nộp sau đó rơi sang tháng kế — tháng 7 có 6 bản',
      annualLeave: 'Ngày phép năm',
      annualLeaveNote: 'Năm nay đã có 9 người dùng hết',
      carryOver: 'Trần chuyển phép',
      carryOverNote: 'Ở mức này, 12 người sẽ mất ngày phép vào 31 tháng 12',
      leaveNotice: 'Báo trước tối thiểu',
      leaveNoticeNote: 'Quý này có 4 yêu cầu không tuân thủ',
      sickNote: 'Cần giấy nghỉ ốm sau',
      sickNoteNote: 'Năm nay đã nộp 17 giấy theo quy tắc này',
      holidays: 'Ngày lễ',
      holidaysNote: 'Lịch lễ luật định Việt Nam — 11 ngày, tháng 8 không có ngày nào',
      rolesInUse: 'Vai trò đang dùng',
      rolesInUseNote: 'Quản trị · Nhân sự · Quản lý · Tổ trưởng · Nhân viên · Người xem',
      managersLeave: 'Quản lý được duyệt nghỉ phép',
      managersLeaveNote: '23 quản lý đang có quyền · 41 lượt duyệt trong tháng',
      teamLeadOt: 'Tổ trưởng được duyệt tăng ca',
      teamLeadOtNote: '9 người sẽ được thêm quyền — đang chờ ban hành',
      session: 'Thời lượng phiên',
      sessionNote: 'Tháng trước có 148 phiên bị cắt ở mức này',
      twoFactor: 'Xác thực hai lớp cho quản trị',
      twoFactorNote: '4 tài khoản quản trị đã đăng ký; tắt đi sẽ hủy toàn bộ',
      digest: 'Bản tin hằng ngày',
      digestNote: '231 người nhận; 17 người đã tắt',
      punchReminder: 'Nhắc chấm công sau',
      punchReminderNote: 'Tuần này đã gửi 34 lần',
      escalate: 'Chuyển cấp yêu cầu chưa trả lời sau',
      escalateNote: 'Tháng này có 6 yêu cầu chuyển lên trưởng phòng',
      weekendNotify: 'Thông báo cuối tuần',
      weekendNotifyNote: 'Bật lên sẽ chạm tới 248 người trong ngày nghỉ',
      timeEntries: 'Lưu bản ghi chấm công',
      timeEntriesNote: 'Bản ghi cũ nhất là từ tháng 3/2023 — rút ngắn sẽ xóa nó',
      leaveRecords: 'Lưu hồ sơ nghỉ phép',
      leaveRecordsNote: 'Luật lao động yêu cầu 60 tháng — thấp hơn thì bản xuất hết an toàn cho kiểm toán',
      auditLog: 'Lưu nhật ký kiểm toán',
      auditLogNote: 'Cửa sổ này đang lưu 4.812 bản ghi',
      anonymise: 'Ẩn danh người đã nghỉ việc',
      anonymiseNote: 'Tháng sau có 3 người đến hạn khi tác vụ chạy',
      exportFormat: 'Định dạng xuất',
      exportFormatNote: 'Bộ phận lương đọc tệp này vào ngày làm việc thứ 3 mỗi kỳ'
    }
  },
  // Production Help Center (vi)
  prodHelp: {
    title: 'Hướng Dẫn Sử Dụng',
    highlight: 'Hướng',
    subtitle: 'Mẹo Vặt',
    lede: 'Bộ hướng dẫn ngắn gọn dành riêng: phát hành an toàn, duy trì ổn định và phản ứng nhanh khi có sự cố.',
    checklist: 'Danh sách kiểm tra vận hành',
    cards: 'Hướng dẫn',
    item: {
      observability: 'Bảng điều khiển và cảnh báo đang hoạt động',
      rollbacks: 'Kế hoạch rollback đã được kiểm thử',
      backups: 'Sao lưu đã được xác minh'
    },
    observability: {
      title: 'Khả năng quan sát: log, metric, trace',
      description: 'Đảm bảo ngân sách lỗi, log có cấu trúc, bảng điều khiển độ trễ 95/99 và lấy mẫu trace phù hợp cho production.'
    },
    rollouts: {
      title: 'Triển khai an toàn và feature flag',
      description: 'Sử dụng triển khai dần dần với health check, playbook rollback và flag để nhanh chóng vô hiệu hóa các đường dẫn mã rủi ro.'
    },
    security: {
      title: 'Bảo mật và quản lý bí mật',
      description: 'Xoay vòng khóa, áp dụng đặc quyền tối thiểu, bật audit log và yêu cầu MFA cho vai trò quản trị.'
    },
    performance: {
      title: 'Ngân sách hiệu năng',
      description: 'Theo dõi thời gian cold start và hot path. Đặt ngân sách cho độ trễ API, truy vấn SQL và TTI frontend.'
    },
    resilience: {
      title: 'Sao lưu và khả năng phục hồi',
      description: 'Xác minh sao lưu tự động, chiến lược DR, giới hạn tốc độ và circuit breaker cho các phụ thuộc phía dưới.'
    },
    tags: {
      reliability: 'độ tin cậy',
      metrics: 'chỉ số',
      release: 'phát hành',
      safety: 'an toàn',
      security: 'bảo mật',
      performance: 'hiệu năng',
      resilience: 'khả năng phục hồi'
    }
  },
  // Demo Help Center
  help: {
    containerLabel: 'Trung Tâm Trợ Giúp',
    title: 'Hướng Dẫn Tính Năng Nâng Cao Chế Độ Dùng Thử',
    subtitle: 'Hướng dẫn các tính năng chính và mẹo chuyên nghiệp',
    videoSection: 'Phần Video',
    videos: {
      title: 'Hướng Dẫn Bằng Video',
      subtitle: 'Học qua các video hướng dẫn ngắn và mẹo nâng cao.'
    },
    demoRestrictions: 'Hạn Chế Bản Demo',
    demo: {
      title: 'QUAN TRỌNG: Hạn Chế Bản Demo!',
      body: 'DỮ LIỆU ĐƯỢC LƯU TRỮ CỤC BỘ - Không Đồng Bộ Qua Các Thiết Bị. Dữ liệu có thể được khôi phục trong Bảng Điều Khiển.'
    },
    features: {
      section: 'Tính năng Nâng Cao & Theo Ngữ Cảnh',
      defaultTitle: 'Tính Năng Ẩn & Mẹo Chuyên Nghiệp',
      contextTitle: 'Mẹo theo ngữ cảnh cho {context}',
      restricted: 'BỊ HẠN CHẾ',
      empty: 'Không tìm thấy mẹo nâng cao cho ngữ cảnh: "{context}". Hiển thị tất cả các tính năng bên dưới.',
      // Per-feature entries (match HELP_FEATURES ids used by components)
      feature_1: {
        title: 'Tính năng ẩn: Chỉnh Sửa Hàng Loạt',
        description: 'Bạn có thể chỉnh sửa tới 50 bản ghi cùng lúc. Chọn hộp kiểm bên cạnh mục và nhấp vào biểu tượng ✏️ ở đầu bảng. Tính năng này bị vô hiệu hóa trong bản demo hiện tại.'
      },
      feature_2: {
        title: 'Mẹo theo ngữ cảnh: Bộ Lọc Nhanh',
        description: 'Sử dụng biểu tượng kính lúp (🔍) bên cạnh tiêu đề cột để lọc ngay theo các giá trị duy nhất của cột đó. Không cần mở bảng lọc đầy đủ.'
      },
      feature_3: {
        title: 'Hạn Chế Demo: Tính Bền Dữ Liệu',
        description: 'Mọi thay đổi bạn thực hiện (tạo, chỉnh sửa, xóa) sẽ bị mất khi bạn làm mới trang. Đây là một hạn chế quan trọng trong môi trường demo.'
      },
      feature_4: {
        title: 'Khó Tìm: Tùy Chọn Người Dùng',
        description: 'Truy cập cài đặt bố cục bảng điều khiển bằng cách nhấp avatar hồ sơ ở góc trên bên phải. Bạn có thể chuyển sang Chế độ Tối và điều chỉnh mật độ bảng ở đây.'
      }
    },
    // Individual feature translations (map to HELP_FEATURES ids)
    features_map: {
      feature_1: {
        title: 'Tính năng ẩn: Chỉnh Sửa Hàng Loạt',
        description: 'Bạn có thể chỉnh sửa tới 50 bản ghi cùng lúc. Chọn hộp kiểm bên cạnh mục và nhấp vào biểu tượng ✏️ ở đầu bảng. Tính năng này bị vô hiệu hóa trong bản demo hiện tại.'
      },
      feature_2: {
        title: 'Mẹo theo ngữ cảnh: Bộ Lọc Nhanh',
        description: 'Sử dụng biểu tượng kính lúp (🔍) bên cạnh tiêu đề cột để lọc ngay theo các giá trị duy nhất của cột đó. Không cần mở bảng lọc đầy đủ.'
      },
      feature_3: {
        title: 'Hạn Chế Demo: Tính Bền Dữ Liệu',
        description: 'Mọi thay đổi bạn thực hiện (tạo, chỉnh sửa, xóa) sẽ bị mất khi bạn làm mới trang. Đây là một hạn chế quan trọng trong môi trường demo.'
      },
      feature_4: {
        title: 'Khó Tìm: Tùy Chọn Người Dùng',
        description: 'Truy cập cài đặt bố cục bảng điều khiển bằng cách nhấp avatar hồ sơ ở góc trên bên phải. Bạn có thể chuyển sang Chế độ Tối và điều chỉnh mật độ bảng ở đây.'
      }
    },
    // Tag translations (keys are normalized by removing spaces and lowercasing)
    tags: {
      advanced: 'Nâng Cao',
      hidden: 'Ẩn',
      demorestricted: 'Bản Demo Hạn Chế',
      contextual: 'Theo Ngữ Cảnh',
      quickaccess: 'Truy Cập Nhanh',
      restriction: 'Hạn Chế',
      demo: 'Demo',
      'hard-to-find': 'Khó Tìm',
      personalization: 'Cá Nhân Hóa'
    },
    showcase: {
      section: 'Hướng dẫn sản phẩm dạng hoạt hình',
      label: 'Hướng dẫn sản phẩm dạng hoạt hình',
      badge: 'Demo ứng dụng trực tiếp',
      title: 'Xem 4 Chức Năng Chính Đang Vận Hành',
      subtitle: 'Theo dõi luồng từ ghi nhận thời gian đến báo cáo, với dữ liệu di chuyển giữa các mô-đun theo thời gian thực.',
      control: 'Bảng điều khiển',
      sync: 'Mọi thứ luôn được đồng bộ',
      syncDesc: 'Các mục nhập, tệp và mục tiêu cập nhật bảng điều khiển và báo cáo ngay lập tức.',
      live: 'Trực tiếp',
      time: {
        title: 'Quản lý thời gian',
        desc: 'Theo dõi chấm công, phê duyệt, làm thêm giờ và nhật ký công việc tại một nơi.'
      },
      upload: {
        title: 'Tải lên tệp',
        desc: 'Lưu trữ an toàn hợp đồng, giấy tờ tùy thân và bằng chứng với lịch sử sẵn sàng cho kiểm toán.'
      },
      dashboard: {
        title: 'Bảng điều khiển & biểu đồ',
        desc: 'Trực quan hóa mức độ sử dụng, chuyên cần và chỉ số sức khỏe theo thời gian thực.'
      },
      tasks: {
        title: 'Công việc & mục tiêu',
        desc: 'Gán người phụ trách, thời hạn và danh sách kiểm tra được đồng bộ với dữ liệu nhân sự.'
      },
      reporting: {
        title: 'Báo cáo',
        desc: 'Xuất PDF/CSV và lên lịch bản tổng hợp định kỳ cho quản lý.'
      }
    }
  },
  // Email Management
  emailManagement: {
    title: 'Quản lý Email người dùng',
    description: 'Quản lý nhiều địa chỉ email cho mỗi người dùng. Mỗi email có thể có mật khẩu riêng nhưng xác thực là cùng một người dùng.',
    addEmail: 'Thêm Email',
    addEmailFor: 'Thêm Email cho',
    emailAddress: 'Địa chỉ Email',
    authUserId: 'ID người dùng Auth',
    authUserIdHelp: 'Lấy từ bảng điều khiển Auth của Supabase sau khi tạo tài khoản',
    linkEmail: 'Liên kết Email',
    unlink: 'Hủy liên kết',
    primary: 'Chính',
    setPrimary: 'Đặt làm chính',
    confirmUnlink: 'Bạn có chắc chắn muốn hủy liên kết {{email}}?',
    cannotRemoveLastEmail: 'Không thể xóa email cuối cùng',
    emailLinked: 'Liên kết email thành công',
    emailUnlinked: 'Hủy liên kết email thành công',
    primaryEmailSet: 'Cập nhật email chính thành công',
    errorLinking: 'Lỗi liên kết email',
    errorUnlinking: 'Lỗi hủy liên kết email',
    errorSettingPrimary: 'Lỗi đặt email chính',
    errorLoadingUsers: 'Lỗi tải người dùng',
    fillAllFields: 'Vui lòng điền tất cả các trường'
  },
  // Personal Goals
  personalGoals: {
    setTargetDate: 'Đặt ngày mục tiêu',
    goalCreated: 'Mục tiêu, đã được tạo thành công',
    errorCreatingGoal: 'Lỗi khi tạo mục tiêu',
    personalGoals: 'Mục tiêu cá nhân',
    noGoals: 'Chưa có mục tiêu cá nhân nào. Tạo mục tiêu đầu tiên của bạn!',
    editGoal: 'Chỉnh sửa mục tiêu',
    goalUpdated: 'Mục tiêu đã được cập nhật thành công',
    errorUpdatingGoal: 'Lỗi khi cập nhật mục tiêu',
    deleteGoal: 'Xóa mục tiêu',
    confirmDeleteGoal: 'Bạn có chắc chắn muốn xóa mục tiêu này?',
    goalDeleted: 'Mục tiêu đã được xóa thành công',
    errorDeletingGoal: 'Lỗi khi xóa mục tiêu',
    subtitle: 'Tiến độ công việc hàng tháng và đánh giá chất lượng',
    individual: 'Cá Nhân',
    team: 'Nhóm',
    taskStatus: 'Trạng Thái Công Việc',
    completedTasks: 'Công Việc Hoàn Thành',
    totalTasks: "Tổng Công Việc",
    overdue: 'Quá Hạn',
    completionRate: 'Tỷ Lệ Hoàn Thành',
    avgQuality: 'Chất Lượng TB',
    onTimePerformance: 'Hiệu Suất Đúng Hạn',
    onTimeCompletions: 'Hoàn Thành Đúng Hạn',
    onTimeRate: 'tỷ lệ đúng hạn',
    priorityDistribution: 'Phân Bố Ưu Tiên',
    highPriority: 'Ưu Tiên Cao',
    mediumPriority: 'Ưu Tiên Trung Bình',
    lowPriority: 'Ưu Tiên Thấp',
    qualityAssessment: 'Đánh Giá Chất Lượng',
    ratedTasks: 'Công Việc Đã Đánh Giá',
    noTasks: 'Không tìm thấy công việc nào trong tháng này',
    noTeamTasks: 'Không tìm thấy công việc nhóm nào trong tháng này',
    evaluateTask: 'Đánh Giá Công Việc',
    selfAssessment: 'Tự Đánh Giá',
    qualityRating: 'Xếp Hạng Chất Lượng',
    managerComments: 'Nhận Xét Của Quản Lý',
    commentPlaceholder: 'Cung cấp phản hồi về chất lượng và hiệu suất công việc...',
    taskDetails: 'Chi Tiết Công Việc',
    dueDate: 'Ngày Đến Hạn',
    currentRating: 'Xếp Hạng Hiện Tại',
    submitEvaluation: 'Gửi Đánh Giá',
    cancel: 'Hủy',
    evaluate: 'Đánh Giá',
    managerEvaluation: 'Đánh Giá Của Quản Lý',
    created: 'Tạo',
    createGoal: "Tạo Mục Tiêu",
    createGoalSubtitle: 'Đặt mục tiêu mới cho nhân viên',
    createPersonalGoal: 'Đặt mục tiêu cá nhân mới',
    assignTo: 'Giao Cho',
    goalPermissionAdmin: 'Với tư cách quản trị viên/quản lý, bạn có thể tạo mục tiêu cho bất kỳ nhân viên nào',
    goalPermissionEmployee: 'Bạn chỉ có thể tạo mục tiêu cho chính mình',
    all: 'Tất Cả',
    title: "Đánh Giá Hiệu Suất Công Việc",
    overallRating: 'Đánh giá tổng thể',
    reviewPeriod: 'Kỳ đánh giá',
    goals: 'Mục tiêu và đối tượng',
    achievements: 'Thành tích chính',
    strengths: 'Điểm mạnh',
    areasForImprovement: 'Lĩnh vực cần cải thiện',
    skillsAssessment: 'Đánh giá kỹ năng',
    rating: 'Đánh giá',
    excellent: 'Xuất sắc',
    good: 'Tốt',
    average: 'Trung bình',
    needsImprovement: 'Cần cải thiện',
    comments: 'Nhận xét',
    save: 'Lưu đánh giá',
    lastReview: 'Đánh giá cuối',
    nextReview: 'Đánh giá tiếp theo',
    q1_2025: 'Q1 / 2025',
    q2_2025: 'Q2 / 2025',
    q3_2025: 'Q3 / 2025',
    q4_2025: 'Q4 / 2025',
    q1_2026: 'Q1 / 2026',
    q2_2026: 'Q2 / 2026',
    q3_2026: 'Q3 / 2026',
    q4_2026: 'Q4 / 2026',
    overallPerformance: 'Hiệu suất tổng thể',
    goalsCompleted: 'Mục tiêu hoàn thành',
    reviewsThisPeriod: "Đánh Giá Trong Kỳ Này",
    avgSkillRating: "Đánh giá kỹ năng trung bình",
    completed: "Hoàn thành",
    inProgress: "Đang thực hiện",
    pending: "Chờ xử lý",
    // Review Types
    weekly: 'Hàng tuần',
    monthly: 'Hàng tháng',
    quarterly: "Hàng quý",
    midYear: "Giữa năm",
    annual: "Hàng năm",
    probation: 'Thử việc',
    adHoc: 'Đột xuất',
    // Period Types
    periodType: 'Loại kỳ',
    byMonth: 'Theo tháng',
    byQuarter: 'Theo quý',
    byYear: 'Theo năm',
    year: 'Năm',
    quarter: 'Quý',
    month: 'Tháng',
    q1: 'Q1 (Tháng 1-3)',
    q2: 'Q2 (Tháng 4-6)',
    q3: 'Q3 (Tháng 7-9)',
    q4: 'Q4 (Tháng 10-12)',
    reviewPeriodPlaceholder: 'Ví dụ: Q4 2024',
    reviewType: 'Loại đánh giá',
    ratingUpdated: 'Đã cập nhật đánh giá hiệu suất thành công!',
    ratingUpdateError: 'Không thể cập nhật đánh giá hiệu suất',
    reviewCreatedSuccess: 'Tạo đánh giá thành công!',
    reviewCreatedError: 'Không thể tạo đánh giá',
    skillUpdateError: 'Không thể cập nhật đánh giá kỹ năng',
    beginner: 'Mới bắt đầu',
    intermediate: 'Trung cấp',
    advanced: 'Nâng cao',
    technicalSkills: 'Kỹ năng kỹ thuật',
    // Tabs
    overview: 'Tổng quan',
    goalsTab: 'Mục tiêu',
    reviewsTab: 'Đánh giá',
    // Goals section
    currentGoals: 'Mục tiêu hiện tại',
    addGoal: 'Thêm mục tiêu',
    addNewGoal: 'Thêm mục tiêu mới',
    performanceGoals: 'Mục tiêu hiệu suất',
    progress: 'Tiến độ',
    in_progress: 'đang thực hiện',
    clickToRate: 'Nhấp vào sao để cập nhật đánh giá nhân viên',
    deadline: 'Hạn chót',
    due: "Đến hạn",
    complete: 'hoàn thành',
    viewDetails: 'Xem chi tiết',
    // Reviews section
    performanceReviews: 'Đánh giá hiệu suất',
    newReview: "Đánh giá hiệu suất mới",
    by: 'bởi',
    viewFullReview: 'Xem đánh giá đầy đủ',
    status: "Trạng thái",
    actions: 'Hành động',
    edit: 'Chỉnh sửa',
    view: 'Xem',
    progressSaved: 'Tiến độ đã được lưu thành công!',
    progressSaveError: 'Không thể lưu tiến độ',
    goalTitle: "Tiêu đề mục tiêu",
    goalTitlePlaceholder: "Nhập tiêu đề mục tiêu",
    goalDescription: "Mô tả mục tiêu",
    goalDescriptionPlaceholder: "Mô tả chi tiết mục tiêu...",
    category: 'Danh mục',
    priority: "Ưu tiên",
    low: "Thấp",
    medium: "Trung bình",
    high: "Cao",
    critical: 'Quan trọng',
    targetDate: "Ngày mục tiêu",
    cancelled: 'Đã hủy',
    onHold: 'Tạm dừng',
    reviewDetails: 'Chi tiết đánh giá hiệu suất',
    reviewer: 'Người đánh giá',
    reviewDate: 'Ngày đánh giá',
    ratings: 'Đánh giá',
    reviewComments: 'Bình luận đánh giá',
    noComments: 'Chưa có bình luận',
    errorFetchingComments: 'Lỗi khi tải bình luận',
    general: "Chung",
    technical: "Kỹ năng kỹ thuật",
    leadership: "Lãnh đạo",
    project: "Dự án",
    communication: "Giao tiếp",
    timeManagement: 'Quản lý thời gian',
    problemSolving: "Giải quyết vấn đề",
    creativity: 'Sáng tạo',
    teamwork: "Làm việc nhóm",
    adaptability: 'Thích nghi',
    emotionalIntelligence: 'Trí tuệ cảm xúc',
    criticalThinking: 'Tư duy phản biện',
    professionalDevelopment: "Phát triển chuyên nghiệp",
    goalDeletedSuccess: 'Xóa mục tiêu thành công!',
    goalDeletedError: 'Không thể xóa mục tiêu',
    goalsInProgress: 'Mục Tiêu Đang Thực Hiện',

    // ---- Personal Goals 2c: đánh giá đối chiếu ----
    history: 'Lịch Sử',
    period: 'Kỳ',
    employee: 'Nhân Viên',
    reviewDue: 'Hạn đánh giá',
    exportReview: 'Xuất đánh giá',
    withCompany: 'gắn bó',
    yearsShort: 'n',
    monthsShort: 'th',
    assessmentLead: 'Tự đánh giá là phần tô, quản lý là vạch dấu, trung vị công ty là nét mảnh.',
    self: 'Tự đánh giá',
    manager: 'Quản lý',
    median: 'Trung vị',
    mgrShort: 'QL',
    noManagerShort: 'chưa có QL',
    adjustRatings: 'Chỉnh điểm',
    gapsToDiscuss: '{count} điểm cần trao đổi',
    ratedAboveSelf: 'quản lý chấm cao hơn',
    ratedBelowSelf: 'thấp hơn',
    noGaps: 'Tự đánh giá và quản lý chênh nhau dưới nửa điểm ở mọi kỹ năng.',
    noManagerRatings: 'Kỳ này chưa có điểm của quản lý.',

    // Mục tiêu
    onTrack: 'Đúng tiến độ',
    atRisk: 'Có rủi ro',
    closed: 'Đã đóng',
    daysLeft: 'còn {n} ngày',
    overdueDays: 'trễ {n} ngày',
    expectedBy: 'theo tiến độ phải đạt {n}%',
    inProgressLower: 'đang thực hiện',
    completedThisYear: 'đã hoàn thành',
    noReviews: 'Chưa có đánh giá nào được ghi nhận.',

    // Cột phải
    reviewCycle: 'Chu kỳ đánh giá',
    closes: 'Chốt ngày',
    byDate: 'trước',
    notYet: 'Chưa có',
    awaitingSchedule: 'Chưa xếp lịch',
    stepSelfAssessment: 'Đã nộp tự đánh giá',
    stepManagerRating: 'Quản lý đã chấm điểm',
    stepCalibration: 'Họp đối chiếu',
    stepSignOff: 'Ký duyệt & mục tiêu quý sau',
    ratingHistory: 'Lịch sử điểm',
    noRatingHistory: 'Chưa có quý nào được chấm điểm.',
    managerNote: 'Ghi chú của quản lý',
    noManagerNote: 'Kỳ này chưa có nhận xét bằng văn bản.',
    acknowledge: 'Xác nhận',
    acknowledged: 'Đã xác nhận',
    reply: 'Phản hồi',
    yourReply: 'Phản hồi của bạn',
    replyPrompt: 'Phản hồi của bạn về đánh giá này:',
    replyError: 'Không thể lưu phản hồi'
  },
  // Morphing Showcase
  morphingShowcase: {
    title: 'Tích Hợp Morphing Flubber',
    subtitle: 'Hoạt ảnh morphing SVG mượt mà được hỗ trợ bởi Flubber',
    interactiveTimeIcon: 'Biểu Tượng Thời Gian Tương Tác',
    clickToMorph: 'Nhấp để chuyển đổi giữa đồng hồ và lịch',
    clock: 'Đồng Hồ',
    calendar: 'Lịch',
    autoMorphingIcon: 'Biểu Tượng Tự Động Chuyển Đổi',
    autoMorphDescription: 'Tự động chuyển đổi giữa đồng hồ và lịch mỗi 3 giây',
    statusIndicators: 'Chỉ Báo Trạng Thái',
    uiElements: 'Các Yếu Tố Giao Diện',
    useCases: 'Trường Hợp Sử Dụng Trong Phần Mềm HR',
    timeTracking: 'Theo Dõi Thời Gian',
    timeTrackingDesc: 'Chuyển đổi giữa trạng thái vào/ra, hiển thị chuyển đổi thời gian',
    statusUpdates: 'Cập Nhật Trạng Thái',
    statusUpdatesDesc: 'Chuyển đổi mượt mà giữa các trạng thái và chỉ báo công việc',
    uiFeedback: 'Phản Hồi Giao Diện',
    uiFeedbackDesc: 'Hoạt ảnh hấp dẫn cho các hành động và thay đổi trạng thái',
    features: 'Tính Năng',
    smoothPathInterpolation: 'Nội suy đường dẫn mượt mà',
    autoPlaySupport: 'Hỗ trợ tự động phát',
    themeAwareColors: 'Màu sắc theo chủ đề',
    optimizedPerformance: 'Hiệu suất được tối ưu hóa',
    manualControl: 'Điều khiển thủ công',
    responsiveDesign: 'Thiết kế đáp ứng',
    customizableTiming: 'Thời gian có thể tùy chỉnh',
    multipleEasingFunctions: 'Nhiều hàm làm mềm'
  },
  // Flubber Demo
  flubberDemo: {
    title: 'Demo Morphing SVG Flubber',
    subtitle: 'Chuyển đổi đường dẫn SVG mượt mà với nội suy Flubber',
    basicShapes: 'Hình Dạng Cơ Bản',
    timeCalendar: 'Thời Gian & Lịch',
    uiElements: 'Các Yếu Tố Giao Diện',
    statusIcons: 'Biểu Tượng Trạng Thái',
    workRelated: 'Liên Quan Đến Công Việc',
    smoothTransitions: 'Chuyển Đổi Mượt Mà',
    smoothTransitionsDesc: 'Flubber tạo ra các nội suy mượt mà giữa các đường dẫn SVG, xử lý các hình dạng phức tạp một cách dễ dàng.',
    autoPlay: 'Tự Động Phát',
    autoPlayDesc: 'Tự động chuyển đổi qua các hình dạng với khoảng thời gian và thời lượng có thể tùy chỉnh.',
    customizable: 'Có Thể Tùy Chỉnh',
    customizableDesc: 'Kiểm soát màu sắc, kích thước, hoạt ảnh và hàm làm mềm để tích hợp giao diện hoàn hảo.',
    usageExample: 'Ví Dụ Sử Dụng',
    features: 'Tính Năng',
    smoothPathInterpolation: 'Nội suy đường dẫn mượt mà sử dụng Flubber',
    autoPlayIntervals: 'Tự động phát với khoảng thời gian có thể tùy chỉnh',
    themeAwareColors: 'Màu sắc theo chủ đề',
    optimizedPerformance: 'Hiệu suất được tối ưu hóa với requestAnimationFrame',
    manualShapeSelection: 'Lựa chọn hình dạng thủ công',
    responsiveAccessible: 'Đáp ứng và dễ tiếp cận',
    customizableDuration: 'Thời lượng hoạt ảnh có thể tùy chỉnh',
    easingFunctions: 'Các hàm làm mềm cho chuyển động tự nhiên'
  },
  demoEmployees: {
    'demo-emp-1': {
      name: 'Quản Trị Viên',
      location: 'Trụ Sở Chính'
    },
    'demo-emp-2': {
      name: 'Kỹ Sư Công Nghệ',
      location: 'Trụ Sở Chính'
    },
    'demo-emp-3': {
      name: 'Nhà Thiết Kế UI/UX',
      location: 'Từ Xa'
    },
    'demo-emp-4': {
      name: 'Chuyên Viên Tiếp Thị',
      location: 'Trụ Sở Chính'
    },
    'demo-emp-5': {
      name: 'Nhân Viên Kinh Doanh',
      location: 'Từ Xa'
    }
  },
  locations: {
    headquarters: 'Trụ Sở Chính',
    remote: 'Từ Xa'
  },
  demoTasks: {
    'task-1': {
      title: 'Phân Tích Báo Cáo Q4',
      description: 'Phân tích toàn diện về các chỉ số hiệu suất hàng quý và xu hướng - LƯU Ý: Dữ Liệu Demo Này Không Chỉnh Sửa Được. Vui Lòng Tự Thêm Dữ Liệu Để Thử Nghiệm'
    },
    'task-2': {
      title: 'Cập Nhật Sổ Tay Nhân Viên',
      description: 'Sửa đổi và cập nhật sổ tay nhân viên với các chính sách làm việc từ xa - LƯU Ý: Dữ Liệu Demo Này Không Chỉnh Sửa Được. Vui Lòng Tự Thêm Dữ Liệu Để Thử Nghiệm'
    },
    'task-3': {
      title: 'Sửa Lỗi Đăng Nhập',
      description: 'Gỡ lỗi và giải quyết vấn đề đăng nhập của người dùng trên thiết bị di động - LƯU Ý: Dữ Liệu Demo Này Không Chỉnh Sửa Được. Vui Lòng Tự Thêm Dữ Liệu Để Thử Nghiệm'
    },
    'task-4': {
      title: 'Thiết Kế Dashboard Mới',
      description: 'Tạo wireframes và mockups cho dashboard phân tích - LƯU Ý: Dữ Liệu Demo Này Không Chỉnh Sửa Được. Vui Lòng Tự Thêm Dữ Liệu Để Thử Nghiệm'
    },
    'task-5': {
      title: 'Đánh Giá Hiệu Quả Chiến Dịch',
      description: 'Phân tích kết quả và ROI chiến dịch marketing Q3 - LƯU Ý: Dữ Liệu Demo Này Không Chỉnh Sửa Được. Vui Lòng Tự Thêm Dữ Liệu Để Thử Nghiệm'
    },
    'task-6': {
      title: 'Hướng Dẫn Khách Hàng Mới',
      description: 'Hoàn thành quy trình onboarding cho khách hàng doanh nghiệp mới - LƯU Ý: Dữ Liệu Demo Này Không Chỉnh Sửa Được. Vui Lòng Tự Thêm Dữ Liệu Để Thử Nghiệm'
    },
    'task-7': {
      title: 'Tài Liệu API',
      description: 'Viết tài liệu API toàn diện cho phiên bản 2.0 - LƯU Ý: Dữ Liệu Demo Này Không Chỉnh Sửa Được. Vui Lòng Tự Thêm Dữ Liệu Để Thử Nghiệm'
    }
  },
  demoGoals: {
    'goal-1': {
      title: 'Cải Thiện Hiệu Quả Nhóm',
      description: 'Tăng năng suất của nhóm lên 20% trong quý tới - LƯU Ý: Dữ Liệu Demo Này Không Chỉnh Sửa Được. Vui Lòng Tự Thêm Dữ Liệu Để Thử Nghiệm'
    },
    'goal-2': {
      title: 'Học React Native',
      description: 'Thành thạo React Native để phát triển ứng dụng di động - LƯU Ý: Dữ Liệu Demo Này Không Chỉnh Sửa Được. Vui Lòng Tự Thêm Dữ Liệu Để Thử Nghiệm'
    },
    'goal-3': {
      title: 'Chứng Chỉ UX',
      description: 'Đạt chứng chỉ Google UX Thiết Kế Chuyên Nghiệp - LƯU Ý: Dữ Liệu Demo Này Không Chỉnh Sửa Được. Vui Lòng Tự Thêm Dữ Liệu Để Thử Nghiệm'
    },
    'goal-4': {
      title: 'Tăng Khách Hàng Tiềm Năng',
      description: 'Đạt tăng 30% khách hàng tiềm năng qua các chiến dịch marketing - LƯU Ý: Dữ Liệu Demo Này Không Chỉnh Sửa Được. Vui Lòng Tự Thêm Dữ Liệu Để Thử Nghiệm'
    },
    'goal-5': {
      title: 'Ký Hợp Đồng Doanh Nghiệp',
      description: 'Ký 5 hợp đồng doanh nghiệp trị giá trên $500K mỗi hợp đồng - LƯU Ý: Dữ Liệu Demo Này Không Chỉnh Sửa Được. Vui Lòng Tự Thêm Dữ Liệu Để Thử Nghiệm'
    }
  },
  demoReviews: {
    'review-1': {
      reviewPeriod: 'Q4 2024',
      strengths: 'Kỹ năng kỹ thuật xuất sắc, làm việc nhóm rất tốt',
      areasForImprovement: 'Có thể cải thiện việc viết tài liệu',
      achievements: 'Dẫn dắt quá trình chuyển đổi sang kiến trúc mới',
      comments: 'Hiệu suất tuyệt vời trong quý này'
    },
    'review-2': {
      reviewPeriod: 'Q4 2024',
      strengths: 'Giao tiếp và giải quyết vấn đề xuất sắc',
      areasForImprovement: 'Chủ động hơn trong các cuộc họp',
      achievements: 'Cải tiến hệ thống thiết kế UI',
      comments: 'Đóng góp tuyệt vời cho đội'
    }
  },
  demoSkills: {
    'skill-1': {
      name: 'React',
      category: 'Kỹ thuật',
      proficiencyLevel: 'Chuyên gia'
    },
    'skill-2': {
      name: 'TypeScript',
      category: 'Kỹ thuật',
      proficiencyLevel: 'Nâng cao'
    },
    'skill-3': {
      name: 'UI/UX Design',
      category: 'Thiết kế',
      proficiencyLevel: 'Chuyên gia'
    }
  },
  demoFeedback: {
    'feedback-1': {
      type: 'Đánh giá đồng nghiệp',
      text: 'Hợp tác rất tốt trong dự án. Rất hữu ích và có nhiều kiến thức.'
    },
    'feedback-2': {
      type: 'Đánh giá của quản lý',
      text: 'Làm việc xuất sắc với hệ thống thiết kế mới. Tiếp tục phát huy nhé!'
    }
  },
  demoJobs: {
    'job-1': {
      title: 'Lập trình viên Frontend Senior',
      description: 'Chúng tôi đang tìm một lập trình viên Frontend có kinh nghiệm để tham gia đội và phát triển các ứng dụng web tuyệt vời.',
      location: 'Làm việc từ xa',
      salaryRange: '$90k - $120k',
      requirements: {
        0: 'React',
        1: 'TypeScript',
        2: 'Tailwind CSS'
      }
    },
    'job-2': {
      title: 'Nhà thiết kế sản phẩm',
      description: 'Tham gia đội thiết kế của chúng tôi để tạo ra trải nghiệm người dùng tuyệt vời và định hình tương lai sản phẩm.',
      location: 'New York',
      salaryRange: '$80k - $100k',
      requirements: {
        0: 'Figma',
        1: 'UI/UX',
        2: 'Lập mô hình (Prototyping)'
      }
    }
  },
  demoApplicants: {
    'app-1': {
      firstName: 'John',
      lastName: 'Doe',
      education: 'Cử nhân Khoa học Máy tính',
      currentPosition: 'Nhà phát triển Cao cấp'
    },
    'app-2': {
      firstName: 'Jane',
      lastName: 'Smith',
      education: 'Thạc sĩ Thiết kế',
      currentPosition: 'Trưởng nhóm UX'
    }
  },
  demoApplications: {
    'appl-1': {
      status: 'Đang xem xét',
      notes: 'Ứng viên mạnh với nền tảng kỹ thuật xuất sắc'
    },
    'appl-2': {
      status: 'Đã lên lịch phỏng vấn',
      notes: 'Portfolio xuất sắc và kỹ năng thiết kế tốt'
    }
  },
  demoInterviews: {
    'int-1': {
      status: 'Đã lên lịch',
      type: 'Phỏng vấn kỹ thuật',
      notes: 'Xem xét portfolio và đánh giá kỹ thuật'
    }
  },
  demoNotifications: {
    'notif-1': {
      title: "Chào mừng đến với hệ thống HR",
      message: "Chào mừng bạn đến chế độ demo của hệ thống HR. Hãy thoải mái khám phá mọi tính năng!",
      actionLabel: "Đi đến bảng điều khiển"
    },
    'notif-2': {
      title: "Nhiệm vụ mới được giao",
      message: "Bạn được giao một nhiệm vụ mới: Xem xét hiệu suất Q3",
      actionLabel: "Xem nhiệm vụ"
    },
    'notif-3': {
      title: "Đến hạn đánh giá hiệu suất",
      message: "Các đánh giá hiệu suất hàng quý sẽ đến hạn vào tuần tới.",
      actionLabel: "Xem đánh giá"
    }
  },
  // Translation Studio (manual UGC translations)
  translationStudio: {
    // Số liệu trên thanh chạy
    strings: 'Chuỗi',
    inQueue: 'Trong hàng đợi',
    locales: 'Ngôn ngữ',
    coverage: 'Độ phủ',
    selected: 'Đang chọn',
    translateAll: 'Dịch tất cả',
    confirmTranslateAll: 'Thay thế toàn bộ bản dịch hiện có của văn bản này bằng kết quả dịch máy? Không có gì được lưu cho đến khi bạn nhấn Lưu.',
    sourceCountHint: 'Bản ghi đã dịch xong / tổng số bản ghi',
    resizePanel: 'Thay đổi kích thước bảng nguồn',
    accessDenied: 'Từ chối truy cập: Cần quyền quản trị viên',
    downloading: 'Đang tải mô hình ngôn ngữ',
    suggestAll: 'Gợi ý các ô trống',
    suggestHint: 'Chỉ điền vào ô trống. Không có gì được lưu cho đến khi bạn xem lại.',
    suggested: 'Đã điền gợi ý',
    needsDownload: 'Gợi ý ngôn ngữ này sẽ tải mô hình về trước',
    notConfigured: 'Thiếu bảng bản dịch. Hãy áp dụng migration 018_hr_ugc_translations.sql (supabase db push) để bật chức năng lưu.',
    title: 'Xưởng Dịch Thuật',
    subtitle: 'Bản dịch do người viết cho công việc, mục tiêu, đánh giá và đơn xin nghỉ. Chúng được ưu tiên hơn bản dịch tự động ở mọi nơi.',
    sources: 'Nguồn',
    fields: 'Trường',
    sourceTasks: 'Công việc',
    sourceGoals: 'Mục tiêu cá nhân',
    sourceReviews: 'Đánh giá hiệu suất',
    sourceLeave: 'Đơn xin nghỉ',
    status: 'Trạng thái',
    filter_all: 'Tất cả',
    filter_untranslated: 'Chưa dịch',
    filter_translated: 'Hoàn tất',
    filter_stale: 'Đã lỗi thời',
    searchPlaceholder: 'Tìm văn bản, bản ghi hoặc nhân viên',
    empty: 'Không có mục nào khớp với bộ lọc.',
    loadError: 'Không tải được nội dung cần dịch.',
    selectPrompt: 'Chọn một đoạn văn bản bên trái để dịch.',
    saved: 'Đã lưu',
    removed: 'Đã xóa',
    saveError: 'Không lưu được',
    stale: 'nguồn đã thay đổi',
    machineDraft: 'Bản nháp tự động',
    remove: 'Xóa',
    draftUnavailable: 'Chưa có mô hình trên thiết bị cho ngôn ngữ này.',
    noLocalModel: 'Trình duyệt này không có công cụ dịch trên thiết bị nên không tạo được bản nháp tự động. Bạn vẫn có thể tự viết bản dịch.',
    demoUnavailable: 'Bản dịch được lưu trên máy chủ và không khả dụng ở chế độ demo.',
    field_title: 'Tiêu đề',
    field_description: 'Mô tả',
    field_self_assessment: 'Tự đánh giá',
    field_comments: 'Nhận xét',
    field_notes: 'Ghi chú',
    field_success_criteria: 'Tiêu chí thành công',
    field_strengths: 'Điểm mạnh',
    field_areas_for_improvement: 'Điểm cần cải thiện',
    field_achievements: 'Thành tích',
    field_employee_comments: 'Nhận xét của nhân viên',
    field_reason: 'Lý do',
    field_rejection_reason: 'Lý do từ chối',
  },
};
